...
normalize_sql_server_identifiers <- function(sql_text) {
  ...
  # geri döner.
  # DİZE SABİTLERİ VE YORUMLAR MASKELENİR.
  #
  # Desen `[...]` biçimindeki HER belirteci yeniden yazıyor, dize sabitlerini
  # ve yorumları atlamıyordu: `WHERE Aciklama = '[Proje Adı]'` ifadesi
  # `'"Proje Adı"'` oluyor, KARŞILAŞTIRMA DEĞERİ değişiyor, yüklem hiçbir
  # satırla eşleşmiyor ve analiz HATASIZ biçimde boş bir kütle raporluyordu.
  # Maskeleme -> yeniden yazma -> geri koyma sırası bunu kapatır.
  metin <- enc2utf8(sql_text)
  # MASKE DESENİ ÜÇ SINIFI KAPSAR:
  #   1) tek tırnaklı dize sabitleri,
  #   2) ÇİFT TIRNAKLI tanımlayıcılar,
  #   3) satır (`--`) ve ÇOK SATIRLI blok yorumları.
  #
  # (2) EKSİKTİ: çift tırnak içine alınmış köşeli parantezli bir tanımlayıcı
  # yeniden yazıldığında ikilenmiş tırnak üretiliyor ve SQL Server sorguyu
  # REDDEDİYORDU.
  #
  # (3) TEK SATIRLA SINIRLIYDI: PCRE'de nokta karakteri satır sonunu KAPSAMAZ,
  # bu yüzden blok yorumu birden fazla satıra yayıldığında maskelenmiyordu.
  # Yorumun içindeki tek tırnak o zaman SAHTE bir dize sabiti başlatıyor ve
  # GERÇEK SQL metnini maskeliyordu; tanımlayıcı yeniden yazımı o bölümü
  # atlıyor, geri konan ifade amaçlanandan FARKLI oluyordu. Satır sonunu da
  # kapsayan karakter sınıfı bunu kapatır.
  kalip_maske <- paste0(
    "'(?:[^']|'')*'",
    "|\"(?:[^\"]|\"\")*\"",
    "|--[^\\r\\n]*",
