...
run_claude_code <- function(prompt,
  ...
  # Çalışma dizini kontrolü. `user_id` GEÇİLİR: kimlik biliniyorsa izinli kökler
  # o kullanıcıya daraltılır; kimlik yoksa `allow_system_temp` ÜST kökleri değil
  # yalnızca bağlantı testi kökünü açar (bkz. cc_policy_allowed_workdir_roots).
  workdir_policy <- cc_policy_validate_workdir(
    workdir, user_id = user_id, allow_system_temp = TRUE
  )
  ...
  args <- cc_policy_build_cli_args(
    ...
    if (isTRUE(akis$limit_exceeded)) {
      ...
      # `duration` SÖZLEŞMESİ sayısal saniyedir; `difftime` nesnesi birim
      # taşıyor ve aritmetik/DB tüketicilerine beklenmeyen tür gidiyordu.
      return(cc_output_limit_result(
        round(as.numeric(difftime(Sys.time(), baslangic, units = "secs")), 1)
      ))
    }
...
test_claude_code_connection <- function(cli_path = NULL, model = NULL,
  ...
  test_sonuc <- run_claude_code(
   ...
   model = model,
    timeout_sec = 60L,
    cli_path = durum$path, api_key = api_key, user_id = user_id
  )
