...
cc_report_output_sync_failure <- function(ctx, outputs) {
  ...
  # `finalize_streaming()` KENDİ `sendCustomMessage()` çağrılarını yapar; kapanmış
  # oturumda hata fırlatıp bu raporlayıcıdan çıkıyor, `then()` zinciri reddediliyor
  # ve `promises::catch()` özel "Aktarım Hatası" durumunu genel "Çıktı İşleme
  # Hatası" kaydının üzerine yazıyordu.
...
cc_report_output_scan_truncation <- function(ctx, outputs) {
  ...
  # Kapanmış oturumda `finalize_streaming()` hata fırlatır; korunmadığında bu
  # durum kaydı genel "Çıktı İşleme Hatası" ile eziliyordu.
  try(cc_finalize_if_active(
    ctx$rv, ctx$env$request_id, ctx$finalize_streaming,
...
cc_dispatch_run_output_processing <- function(ctx) {
  ...
  # gönderimi ÖLÜMCÜL OLMAYAN sayar. Eski dal guard/lease'i kaldırıp worker'ı hiç
  # göndermiyordu: kullanıcı sekmeyi kapattığında üretilen çıktılar kaynak
  # klasöre hiç aktarılmıyor ve tamamlanmış çalıştırma "çıktı işleme hatası"
  # olarak raporlanıyordu. Hata YALNIZCA loglanır; guard/lease temizliği
  # aşağıdaki promise/deadline yoluna aittir.
  ...
  if (inherits(asama_hatasi, "condition")) {
    cc_log_warn(paste(
      CLAUDE_CODE_LOG_PREFIX,
      "[OUTPUT_SYNC] Çıktı aşaması bildirilemedi;",
      "çıktı işleme devam ediyor:",
      conditionMessage(asama_hatasi)
    ))
  }
  ...
  # denetimini geçiyor ve YENİ isteğin lease'ini siliyordu. (Codex katmanındaki
  # etkin tanımla BİREBİR aynı sözleşme.)
  gonderilen_request_id <- ctx$env$request_id
  gonderilen_lease <- as.character(ctx$env$runtime_lease %||% "")[1]
  if (is.na(gonderilen_lease)) gonderilen_lease <- ""
  ...
  tracked_future_promise(
    ...
    promises::then(function(outputs) {
      ...
      # AŞAMA BİLDİRİMİ ÖLÜMCÜL DEĞİLDİR: kullanıcı sekmeyi kapattığında
      # `session$sendCustomMessage()` hata veriyor, bu hata `then()` zincirini
      # reddediyor ve `promises::catch()` içindeki
      # `cc_report_output_processing_failure()` çıktıları BAŞARIYLA aktarılmış
      # bir çalıştırmayı `failed` olarak kalıcılaştırıyordu (üstelik
      # `cc_finish_streaming_run()` hiç çalışmıyordu).
      ...
      # Kapanmış oturumda `sendCustomMessage()` hata verir; koruma olmadan
      # tamamlanmış çalıştırma `catch()` dalına düşüp `failed` kalıcılaşıyordu.
  ...
  if (inherits(gonderim, "condition")) {
    ...
    if (cc_is_active_run(ctx$rv, gonderilen_request_id)) {
