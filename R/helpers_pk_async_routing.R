...
mergen_pk_force_bounded_sync <- function(stop_check = NULL,
  ...
  # HEDEF ORTAM SARMALAYICININ DEĞİL, HAT ÇEKİRDEĞİNİNDİR: tek-çıkış kancası
  # kurulduğunda `environment(pk_analiz_process_request)` KURULUM çerçevesidir
  # ve oraya yazılan sınırlı yürütücüyü çekirdek HİÇ görmez (sınırsız
  # materyalizasyon kalır, Durdur/son tarih kapısı uygulanmazdı).
  .cekirdek_adi <- ".pk_analiz_process_request_without_exit_observer"
  cekirdek <- if (exists(.cekirdek_adi, mode = "function", inherits = TRUE)) {
    get(.cekirdek_adi, mode = "function", inherits = TRUE)
  } else pk_analiz_process_request
  hedef <- environment(cekirdek)
  ...
  # ÖNCEKİ BAĞLAMANIN VARLIĞI AYRI İZLENİR.
  #
  # `get0()` tanımsız sembol için `NULL` döner; geri yükleyici yalnızca
  # `is.function(eski)` iken yazdığı için sınırlı yürütücü `hedef` içinde SÜREÇ
  # ÖMRÜ BOYUNCA bağlı kalıyordu. Sonraki senkron istekler o zaman TAMAMLANMIŞ
  # bir isteğin `stop_check` kapanışını okuyan bir `stage_gate` ile çalışırdı.
  eski_vardi <- exists("execute_pk_sql_unicode", envir = hedef, inherits = FALSE)
  ...
  function() {
    if (isTRUE(eski_vardi) && is.function(eski)) {
      try(assign("execute_pk_sql_unicode", eski, envir = hedef), silent = TRUE)
    } else if (exists("execute_pk_sql_unicode", envir = hedef, inherits = FALSE)) {
      # `hedef` içinde YEREL bir bağlama YOKTU: sarmalayıcı KALDIRILIR (ebeveyn
      # tanımı yine görünür olur, gölge bağlama kalmaz).
      try(rm("execute_pk_sql_unicode", envir = hedef), silent = TRUE)
    }
