...
cc_policy_permission_mode <- function(settings_data = NULL) {
  ...
  if (!is.null(settings_data) && !is.null(settings_data$claude_code_permission_mode)) {
    ...
    if (!is.na(settings_mode) && nzchar(settings_mode)) {
      ...
      # Merkezi mod TANIMSIZ/GEÇERSİZ olduğunda CLI kendi varsayılanını kullanır;
      # bu `default` katılığındadır. Eskiden `is.na(merkezi_puan)` dalı oturum
      # ayarını KOŞULSUZ kabul ediyor, böylece geçersiz bir değer bilinmeyen-mod
      # yedeğiyle `acceptEdits`e düşüp CLI varsayılanını GEVŞETEBİLİYORDU.
      # Oturum ayarı ayrıca GEÇERLİ olmak zorundadır.
      karsilastirma_puani <- if (is.na(merkezi_puan)) 2L else merkezi_puan
      if (!is.na(ayar_puan) && ayar_puan >= karsilastirma_puani) {
  ...
  # acceptEdits` olduğu için `acceptEdits` ÜÇÜNÜN EN GEVŞEĞİDİR; bir yazım
  # hatası (`acceptEdit`, `Accept_Edits`) otomatik düzenlemeyi SESSİZCE
  # açıyordu. Tanımsız/geçersiz merkezi mod `default` katılığında sayılır
  # (bu dosyadaki oturum-daraltma mantığı da aynı varsayımı kullanır), bu
  # yüzden CLI'nin kendi varsayılanı korunur.
  log_warn(paste(
    CLAUDE_CODE_LOG_PREFIX,
    "Bilinmeyen Claude Code permission_mode değeri:",
    raw_mode,
    "- CLI varsayılanı (default) kullanılacak."
  ))
