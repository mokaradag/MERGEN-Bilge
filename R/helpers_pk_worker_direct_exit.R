...
if (isTRUE(.pk_worker_direct_exit_mode) &&
  ...
  pk_analiz_process_request <- function(user_prompt, chat_history, session,
    ...
    if (exists("pk_analysis_observe", mode = "function", inherits = TRUE)) {
      ...
      # YEDEK YOL DA LİSTE GÜVENLİDİR VE KAPALI BAŞARISIZ OLUR: eski yedek (`else { sonuc }`) tam olarak yukarıda anlatılan kusuru geri getiriyordu (yardımcı yoksa LİSTE doğrudan sınıflandırıcıya gidiyordu). Metin çıkarılamadığında doğru davranış yeni bağlantı AÇMAMAKTIR; yanlış pozitifin bedeli yalnızca opsiyonel telemetrinin atlanmasıdır.
      cikarilabildi <- exists("pk_direct_exit_text", mode = "function", inherits = TRUE)
      sonuc_metni <- if (cikarilabildi) {
        tryCatch(pk_direct_exit_text(sonuc), error = function(e) NULL)
      } else NULL

      db_hatasi <- if (is.null(sonuc_metni)) {
        TRUE
      } else {
        exists("pk_direct_exit_is_db_failure", mode = "function", inherits = TRUE) &&
          isTRUE(tryCatch(pk_direct_exit_is_db_failure(sonuc_metni), error = function(e) TRUE))
      }
      ...
      try(pk_analysis_observe(session, baglanti$conn, list(
        ...
        # `durduruldu` BURADA HER ZAMAN FALSE'TUR: iptal edilen istek yukarıda telemetriye HİÇ ULAŞMADAN erken döner; sabit değer, ölü dalın "durdurulan koşular da kaydediliyor" izlenimini engeller.
        ...
        outcome = if (exists("pk_direct_exit_outcome", mode = "function", inherits = TRUE)) {
          pk_direct_exit_outcome(sonuc, stopped = FALSE, error = istisna)
        } else if (istisna) "Hata" else "DogrudanYanit",
