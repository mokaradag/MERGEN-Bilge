...
mergen_build_send_message_prompt_snapshot <- function(prompt_text, session_files) {
  ...
  if (length(current_session_files) > 0) {
    yollar <- vapply(
      ...
      # `%||%` YALNIZCA `NULL` için yedeğe düşer: `f$path` BOŞ DİZE olduğunda
      # `f$datapath` hiç kullanılmıyor, sonraki `<yol-yok>:<index>` anahtarı aynı
      # dosyanın takma adlarını AYRI tutuyor ve `uploaded_names` onları dosya
      # listesinde ve Kaynakça'da tekrarlıyordu.
      function(f) {
        yol <- as.character(f$path %||% "")[1]
        if (length(yol) == 1L && !is.na(yol) && nzchar(yol)) return(yol)
        as.character(f$datapath %||% "")[1]
      },
      character(1)
    )
    yollar[is.na(yollar)] <- ""
    ...
    yol_kimligi <- vapply(seq_along(yollar), function(i) {
      ...
      # AYRAÇ VE HARF KATLAMA YALNIZCA WINDOWS'TA uygulanır: POSIX'te ters slash
      # SIRADAN bir yol karakteridir ve `dir/a.csv` ile `dir\\a.csv` FARKLI
      # dosyalardır; koşulsuz dönüşüm ikincisini tekilleştirmeyle düşürüyordu.
      if (identical(.Platform$OS.type, "windows")) {
        tolower(sub("/+$", "", gsub("\\\\", "/", yol)))
      } else {
        sub("/+$", "", yol)
      }
    }, character(1))
