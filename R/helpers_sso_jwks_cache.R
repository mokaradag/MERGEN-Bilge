...
#' JWKS yönlendirme güven politikası (saf karar yardımcısı)
#'
#' `SSO_CONFIG$jwks_endpoint` değeri `httr::GET()` ile çekilir ve dönen anahtarlar
#' DOĞRUDAN imza doğrulamasını besler (CWE-494). `httr` yönlendirmeleri varsayılan
#' olarak İZLER; yapılandırılmış bir `https://` uç noktası `http://` adresine ya da
#' BAŞKA bir konağa yönlendirildiğinde anahtarlar güvenilmeyen bir kaynaktan
#' geliyordu. Operatörün AÇIKÇA yapılandırdığı `http://` uç noktası desteklenir
#' (on-prem dağıtım), ancak HTTPS -> HTTP DÜŞÜRMESİ ve konak değişimi reddedilir.
#'
#' @param istenen_url Yapılandırılmış (güvenilen) JWKS adresi
#' @param son_url Yanıtın ETKİN adresi (`resp$url`); bilinmiyorsa boş
#' @return TRUE kabul edilebilir, FALSE reddedilir
.sso_jwks_redirect_kabul_edilir <- function(istenen_url, son_url) {
  istenen <- as.character(istenen_url %||% "")[1]
  son <- as.character(son_url %||% "")[1]
  if (is.na(istenen) || !nzchar(istenen)) return(FALSE)
  # Etkin adres BİLİNMİYORSA yönlendirme kanıtı yoktur; davranış değişmez.
  if (is.na(son) || !nzchar(son)) return(TRUE)
  if (identical(istenen, son)) return(TRUE)

  ayir <- function(u) {
    p <- tryCatch(httr::parse_url(u), error = function(e) NULL)
    if (!is.list(p)) return(NULL)
    list(
      sema = tolower(as.character(p$scheme %||% "")[1]),
      konak = tolower(as.character(p$hostname %||% "")[1])
    )
  }
  ...
  if (is.na(a$konak) || !nzchar(a$konak) || !identical(a$konak, b$konak)) return(FALSE)
  # Yapılandırılmış şema `https` ise düşürme kabul edilmez.
  if (identical(a$sema, "https") && !identical(b$sema, "https")) return(FALSE)
  TRUE
}
...
sso_fetch_jwks <- function(jwks_url, timeout_secs = 10) {
  tryCatch({
    ...
    etkin_url <- tryCatch(as.character(resp$url %||% "")[1], error = function(e) "")
    if (!isTRUE(.sso_jwks_redirect_kabul_edilir(jwks_url, etkin_url))) {
      log_warn("JWKS yönlendirmesi güven politikasını ihlal etti; anahtarlar kullanılmadı: {jwks_url}")
      return(NULL)
    }
    ...
    keys <- parsed[["keys"]]
    # BİÇİM DENETİMİ önbelleğe almadan ÖNCE yapılır. `{"keys":"invalid"}` gibi
    # geçerli ama HATALI BİÇİMLİ bir yanıt eski denetimden geçiyor,
    # `sso_find_jwk_by_kid()` içindeki `k[["kid"]]` erişimi karakter değerde
    # hata fırlatıyor ve çözümleyici belgelenen `NULL` yerine İSTİSNA üretiyordu.
    # Hatalı biçimli girdiler ATLANIR; hiç geçerli JWK kalmazsa `NULL` döner.
    if (!is.list(keys) || length(keys) == 0) return(NULL)
    gecerli <- Filter(function(k) is.list(k) && length(k) > 0L, keys)
    if (length(gecerli) == 0) {
      log_warn("JWKS yanıtı geçerli JWK nesnesi içermiyor: {jwks_url}")
      return(NULL)
    }
    gecerli
  }, error = function(err) {
...
sso_find_jwk_by_kid <- function(keys, kid) {
  if (!is.list(keys) || length(keys) == 0) return(NULL)
  for (k in keys) {
    # Nesne OLMAYAN giriş atlanır: `k[["kid"]]` karakter değerde hata fırlatır.
    if (!is.list(k)) next
    k_kid <- k[["kid"]]
...
sso_resolve_signing_key <- function(token, config = SSO_CONFIG,
  ...
  # SKALER OLMAYAN `kid` REDDEDİLİR (fail-closed). Başlık doğrulanmamıştır;
  # `"kid": ["a","b"]` gibi bir değer ilk-eleman bayt denetimini geçiyor, ancak
  # AŞAĞIDAKİ arama çağrılarına HAM vektör geçtiği için `sso_find_jwk_by_kid()`
  # içindeki `nzchar(kid)` uzunluk > 1 döndürüyor ve `if` koşulu R 4.2+ ile
  # "the condition has length > 1" hatasıyla düşüyordu (fail-closed dönmek
  # yerine istisna).
  if (!is.null(kid) && (!is.atomic(kid) || length(kid) != 1L)) {
    log_warn("JWT başlığındaki kid değeri skaler değil; reddedildi (fail-closed).")
    return(NULL)
  }
  ...
  # Aramaya YALNIZCA doğrulanmış skaler geçilir; `kid` yoksa NULL geçilir ki
  # `sso_find_jwk_by_kid()` ilk imzalama anahtarına düşebilsin.
  arama_kid <- if (nzchar(kid_metin)) kid_metin else NULL
  ...
  jwk <- sso_find_jwk_by_kid(keys, arama_kid)
