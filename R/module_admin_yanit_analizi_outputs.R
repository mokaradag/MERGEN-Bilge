...
admin_yanit_outputs <- function(output, ya_data, etiket_sayilari, refresh) {
    ...
    output$ya_yorum_tablo <- DT::renderDT({
      ...
      data$onizleme <- htmltools::htmlEscape(ifelse(
        ...
        # Üç nokta YALNIZCA gerçekten kesildiğinde eklenir; aksi hâlde "Evet"
        # yanıtı "Evet..." görünüp gizli metin varmış izlenimi veriyordu.
        paste0(
          substr(data$YanitOnizleme, 1, 120),
          ifelse(nchar(data$YanitOnizleme) > 120, "...", "")
        ),
        "-"
      ))
