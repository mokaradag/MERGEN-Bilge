...
fileClickObserversInit <- function(input, session, settings_data, api_config,
  ...
  observeEvent(input$analysis_file_clicked, {
	...
	# KİMLİK ÇÖZÜMLEME HATASI da KÖK HATASIDIR. `0L`'e düşmek, aşağıdaki
	# `tiklayan_uid > 0L` kapısını atlıyor, `izinli_kokler` yalnızca `www` kökünü
	# içeriyor ve `kok_hatasi` FALSE kalıyordu: kullanıcı yükleme klasöründeki
	# GEÇERLİ bir dosya kalıcı görünen "Geçersiz dosya yolu." hatasıyla
	# reddediliyordu (geçici "lütfen tekrar deneyin" yerine).
	kok_hatasi <- FALSE
	tiklayan_uid <- tryCatch(
	  ...
	  error = function(e) {
	    kok_hatasi <<- TRUE
	    log_warn(
	      "[ANALYSIS_FILE] Etkin kullanıcı kimliği çözülemedi: {msg}",
	...
	# KÖK ÇÖZÜMLEME HATASI SESSİZCE YUTULMAZ. `mergen_user_upload_dir()` hata
	# verdiğinde eski kod `""` üretiyor, `Filter(nzchar, ...)` bunu düşürüyor ve
	# kullanıcı yükleme kökü izinli kökler listesinden çıkıyordu: geçerli bir
	# kullanıcı dosyası "Geçersiz dosya yolu." ile reddedilebiliyordu. Durum
	# AYRIŞTIRILIR: kök çözülemediyse uyarı loglanır ve kullanıcıya geçici bir
	# hata bildirilir (sessiz ret yerine).
	kullanici_koku <- ""
	if (isTRUE(tiklayan_uid > 0L)) {
	  kullanici_koku <- tryCatch(
	    as.character(mergen_user_upload_dir(tiklayan_uid))[1],
	    error = function(e) {
	      kok_hatasi <<- TRUE
	      log_warn(
	        "[ANALYSIS_FILE] Kullanıcı yükleme kökü çözülemedi: {msg}",
	        msg = conditionMessage(e)
	      )
	      ""
	    }
	  )
	  if (is.na(kullanici_koku) || !nzchar(kullanici_koku)) {
	    if (!kok_hatasi) {
	      kok_hatasi <- TRUE
	      log_warn("[ANALYSIS_FILE] Kullanıcı yükleme kökü boş döndü; kök listesine eklenmedi.")
	    }
	    kullanici_koku <- ""
	  }
	}
	...
	if (!any(hedef_anahtar == kok_anahtarlari |
	  ...
	  # Kök ÇÖZÜLEMEDİĞİ için reddedilen geçerli dosya, gerçek bir traversal
	  # denemesinden AYRI bildirilir; kullanıcı yeniden denemesi gerektiğini bilir.
	  if (isTRUE(kok_hatasi)) {
	    showToast(session, "Dosya klasörünüz şu anda çözümlenemedi; lütfen tekrar deneyin.", "warning")
	    log_warn(
	      "[ANALYSIS_FILE] Kök çözümlenemediği için yol doğrulanamadı: {hedef_anahtar}",
	      hedef_anahtar = hedef_anahtar
	    )
	    return(invisible(NULL))
	  }
	  showToast(session, "Geçersiz dosya yolu.", "error")
