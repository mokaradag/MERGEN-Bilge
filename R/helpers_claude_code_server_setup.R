...
cc_bind_server_setup <- function(input,
  ...
  observeEvent(input$yerel_klasor, {
    ...
    dosya_sayisi <- 0L
    reddedilen <- 0L
    # Sarkan-bağlantı sondasının ÜST DİZİN listelemesi TEK olay boyunca
    # önbelleklenir; dosya başına listeleme Shiny olay döngüsünü O(N^2)
    # maliyetle blokluyordu.
    listeleme_onbellegi <- new.env(parent = emptyenv())
    for (i in seq_len(nrow(dosyalar))) {
      ...
      hedef <- cc_setup_yerel_yukleme_hedefi(
        calisma_alani, goreceli,
        listeleme_onbellegi = listeleme_onbellegi
      )
      if (is.null(hedef)) {
