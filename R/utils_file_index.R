...
.file_index_scan_bounded <- function(base_path, pattern) {
  if (exists("cc_scan_directory_bounded", mode = "function", inherits = TRUE)) {
    ...
    if (is.list(sonuc)) {
      ...
      # KISMİ sonuç İŞARETLENİR: aksi hâlde eksik indeks TAM indeks gibi TTL
      # boyunca önbelleğe alınıyor ve taramanın ulaşamadığı MEVCUT dosyalar
      # dakikalar boyunca "bulunamadı" dönüyordu.
      return(structure(eslesen, scan_partial = isTRUE(sonuc$truncated)))
    }
  }
...
.file_index_walk_bounded <- function(base_path, pattern,
  ...
  # KISMİ sonuç İŞARETLENİR (sınırlı tarayıcı yolundaki `scan_partial` ile aynı
  # sözleşme): yalnızca uyarı loglamak, eksik indeksin TAM indeks gibi TTL
  # boyunca önbelleğe alınmasını engellemiyordu.
...
.build_basename_index <- function(base_path, pattern = "\\.(docx|docm|doc|pdf|pptx|ppt|xlsx|xls|csv|txt|json|md|r|py|log)$", force = FALSE) {
  ...
  if (force || is_stale || is.null(ent) || is.null(ent$map)) {
    ...
    if (isTRUE(attr(all_files, "scan_failed"))) {
      if (is.list(ent) && !is.null(ent$map)) {
        # Geri çekilme damgası ESKİ girdi için de YAZILIR. Eskiden `ent` doğrudan
        # döndürülüyor, `scan_failed_at` hiç yazılmıyor ve yukarıdaki 30 sn'lik
        # geri çekilme devreye girmiyordu: `search_file_in_folder()` adım 1-4'te
        # yeniden tarama başlatıyor, tek bir dosya tıklaması Shiny sürecini
        # ~80 sn bloke ediyordu. `ts` TAZELENMEZ; indeks yine eski kabul edilir.
        ent$scan_failed_at <- now
        .FILE_INDEX_CACHE[[key]] <- ent
        return(ent)
      }
    ...
    # indeks boş indeksten iyidir) ancak `ts` TAZELENMEZ; kısa geri çekilme
    # damgası, aynı istekteki sonraki adımların taramayı yeniden başlatmasını
    # engellerken bir sonraki isteğin yeniden taramasına izin verir.
    .FILE_INDEX_CACHE[[key]] <- if (kismi) {
      list(ts = NULL, map = map, scan_failed_at = now)
    } else {
      list(ts = now, map = map)
    }
  }
