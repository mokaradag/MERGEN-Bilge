...
mergen_pk_run_sync <- function(ctx) {
  tryCatch({
    ...
    # KALICI LOG ORTAK ZİNCİRDEN GEÇER: `pk_safe_log_text()` HEM parola/anahtar HEM bağlantı KİMLİKLERİNİ (DSN/UID/Server) maskeler ve redaktör yoksa KAPALI BAŞARISIZ olur. Tek başına `redact_sensitive_text()` bir ODBC istisnasının DSN'ini kalıcı loga taşıyordu.
    # REDAKTÖR HATASI İSTEĞİ DÜŞÜREMEZ (PR #705, P3): bu satır DIŞTAKİ
    # `tryCatch`in `error` işleyicisi İÇİNDE çalışır, buradaki hata YAKALANMAZ
    # ve çağırana yayılır; senkron yollar sarmalamadan çağırdığı için istek
    # düşer ve `values$is_sending` ASILI KALIR. Redaktör düşerse ham metin
    # loga TAŞINMAZ (kapalı başarısız).
    ham <- tryCatch(
      if (exists("pk_safe_log_text", mode = "function", inherits = TRUE)) {
        pk_safe_log_text(ham)
      } else {
        "[redaktor yuklenmedi]"
      },
      error = function(x) "[redaktor hatasi - ham metin gizlendi]"
    )
