...
# YÜKLEME DENETİMİ KULLANILAN TÜM KİLİT YARDIMCILARINI kapsar. Yalnızca
# `.cc_codex_acquire_dir_lock()` doğrulandığında, eksik `reap`/`touch`
# yardımcısı yükleme anında değil TERFİ sırasında (`kirayi_tazele()` /
# `.kilidi_birak()` içinde) hata veriyordu; o noktada geçici dosya ve kilit
# dizini diskte kalabiliyordu.
for (.cc_codex_gerekli_kilit in c(".cc_codex_acquire_dir_lock",
                                  ".cc_codex_reap_dir_lock",
                                  ".cc_codex_touch_dir_lock",
                                  ".cc_codex_lock_owner_token")) {
  if (!exists(.cc_codex_gerekli_kilit, mode = "function", inherits = TRUE)) {
    stop(
      paste0("Codex runtime kilit katmanı eksik: ", .cc_codex_gerekli_kilit),
      call. = FALSE
    )
  }
}
rm(.cc_codex_gerekli_kilit)
...
mirror_directory_to_local_workspace <- function(source_dir,
  ...
  if (!is.null(err)) {
    # SAHİPLİK DOĞRULANMADAN GERİ YÜKLEME YAPILMAZ. `target_dir` bu noktada
    # `backup` konumuna taşınmış olabilir ve hata, kilidi BAŞKA bir worker'ın
    # devralmasından kaynaklanmış olabilir. O worker bu arada KENDİ `target_dir`
    # ağacını terfi ettirdiyse `restore()` onu siler ve eski girdiyi geri
    # koyardı. Sahiplik kaybedilmişse eski worker `target_dir` üzerinde HİÇBİR
    # işlem yapmaz; `backup` diskte kalır ve bayat runtime temizliğine bırakılır.
    sahiplik_var <- identical(.cc_codex_lock_owner_token(lock_dir), kilit_jetonu)
    if (isTRUE(sahiplik_var)) {
      if (promoted || had_target) restore()
    } else {
      try(
        log_warn(paste(
          "[RUNTIME_WORKDIR] Terfi kilidi sahipliği kaybedildi;",
          "geri yükleme YAPILMADI (yedek korunuyor):", backup
        )),
        silent = TRUE
      )
    }
