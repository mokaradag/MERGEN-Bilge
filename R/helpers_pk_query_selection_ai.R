...
pk_select_llm_invoke <- function(messages, cfg, session = NULL, llm_fn = NULL,
  ...
  .select_plan <- pk_select_effective_timeout(cfg$timeout_sec)
  etkin_timeout <- .select_plan$timeout_sec
...
pk_select_confirmed_decision <- function(session, prompt, library_index, chat_key) {
  ...
  if (!is.null(gereksinimler) &&
    ...
    if (is.list(kontrol) && isTRUE(kontrol$asserted) &&
      ...
      return(.pk_select_decision(
        kontrol$status,
        message_tr = paste0(
          "Seçtiğiniz analiz, sorunuzun gerektirdiği alanları sağlamıyor; ",
          "bu nedenle çalıştırılmadı. Lütfen sorunuzu yeniden ifade edin."
        ),
        query_id = kimlik,
  ...
  .pk_select_decision(
    ...
    reason = "Kullanıcı sunulan seçenekler arasından bu analizi açıkça seçti.",
    capability_status = "not_asserted",
    selection_method = "user_confirmed",
    disclosures = "Seçim kullanıcının açık onayıyla yapıldı; model seçimi çalıştırılmadı."
  )
}
...
pk_select_run <- function(user_prompt, library, chat_history = NULL,
  ...
  uyum <- pk_retrieval_agreement(
    ...
    library = library, exclusion_prompt = user_prompt  # OLUMSUZ KANIT YALNIZCA GÜNCEL SORUYA AİTTİR (bkz. `pk_retrieval_agreement`).
  )
  ...
  # BOZUK GEÇİŞ B YANITI OPERATÖRE BİLDİRİLİR (PR #705, P3): onarım denemeleri
  # tükenince akış buraya düşer; karar GÜVENLİDİR ama bu bildirim YALNIZCA
  # `.pk_select_pass_failure_decision()` yolunda ekleniyor ve operatöre HİÇ
  # ULAŞMIYORDU. Yönlendirmeyi tipli yola çevirmek çipleri sözlüksel kesişime
  # indirger ve örtüşmesi olmayan sorularda kullanıcı SEÇENEKSİZ kalırdı.
