...
sttServer <- function(id, parent_session, settings) {
  moduleServer(id, function(input, output, session) {
    ...
    observeEvent(input$audio_chunk, {
      ...
      # BÜTÇE DOĞRULAMALARDAN SONRA TÜKETİLİR. Eskiden kapı geçildiği anda
      # sıfırlanıyordu: GEÇERSİZ bir `audio_chunk` (dizi/boş/çok büyük/kuyruk
      # dolu) erken dönerken bütçe zaten harcanmış oluyor, kayıtçının hemen
      # ardından gönderdiği GEÇERLİ final parça reddediliyor ve konuşmanın son
      # bölümü kayboluyordu.
      rv$expect_final_chunk <- FALSE
