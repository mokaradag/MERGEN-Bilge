...
# `sql_variant` SINIRSIZ DEĞİLDİR: tek bir değer en fazla 8.016 bayttır ve bu, AYNI depodaki metadata üreticisi sözleşmesinde de kanıtlanmış üst sınır olarak modellenmiştir; LOB listesinde tutmak sınırlı bir sonucu getirmeden reddediyordu.
# SQL Server sürücüsü bunu `SQL_SS_VARIANT` (-150) bildirir, -98 yalnızca bazı sürücülerde görülür (PR #719 inceleme, P3): tek kod sabitlenince tanımlayıcı YOKKEN sütun `__unknown__` dalına düşüyor ve sınırsız-LOB opt-in'i kapalıyken sonuç `unknown_type_code` ile REDDEDİLİYORDU.
.PK_ODBC_SQL_VARIANT_CODE <- c(-150L, -98L)
.PK_SQL_VARIANT_MAX_BYTES_RUNTIME <- 8016

# `hierarchyid` DE SINIRSIZ DEĞİLDİR: tek bir değer SQL Server'da en fazla 892 bayttır; tip adı `sinirsiz_tipler` listesinde durduğu için `type = "text"` dönüyor, `.pk_result_type_is_lob()` bunu LOB sayıyor ve
# `pk_sql_plan_chunk_rows()` -- operatör `MERGEN_PK_ALLOW_UNBOUNDED_LOB` açmadıkça -- `refuse = TRUE` / `unbounded_lob_column` üretiyordu.
# Yani satır genişliği KANITLANABİLİRKEN analiz tek satır bile çekilmeden `too_large` ile reddediliyordu; `sql_variant` için zaten düzeltilmiş olan hata sınıfının aynısı.
...
.pk_sql_metadata_field <- function(column_info, adaylar) {
  # YERELDEN BAĞIMSIZ KATLAMA ZORUNLUDUR: Türkçe yerelde `tolower("I")` NOKTASIZ `ı` üretir, `PRECISION` gibi bir sürücü alan adı `precısıon` olur, aday listesiyle EŞLEŞMEZ ve `boyut` `NA` kalır.
  # Değişken genişlikli sütun o zaman `__unproven__` sınıflanır ve sınırlı yürütücü Türkçe VM'de aynı sorguyu `too_large` diye reddederken CI'da başarılı olur.
  ...
  # ADAY SIRASI ÖNCELİKTİR.
  #
  # `which(alanlar %in% adaylar)` konumları `names(column_info)` SIRASINDA
  # döndürüyordu; `adaylar` sırası hiç etkili değildi. `precision` alanını
  # `column_size`/`max_length` ÖNCESİNDE bildiren bir DBI arka ucunda sayısal
  # hassasiyet karakter uzunluğu sanılıyor, `pk_sql_columns_from_metadata()`
  # YANLIŞ alandan `__bounded__` genişlik üretiyordu.
  for (aday in adaylar) {
    idx <- which(alanlar == aday)
    if (length(idx)) return(column_info[[idx[1L]]])
  }
  NULL
}
...
pk_sql_describe_result_schema <- function(conn, sql_text, call_fn = NULL) {
  ...
  # `error_type` DE SEÇİLİR: `sys.dm_exec_describe_first_result_set` bir toplu
  # ifadeyi betimleyemediğinde (ör. yerel `#temp` tablosu) HATA FIRLATMAZ; TEK
  # bir satır döner ve `error_type` doludur, `name`/`system_type_name` NULL'dur.
  # Sütun seçilmediği için o satır SÜTUN TANIMI sanılıyor,
  # `.pk_sql_column_from_descriptor()` onu `__unproven__` sınıflandırıyor ve
  # `pk_sql_plan_chunk_rows()` HİÇ veri çekmeden sonucu reddediyordu.
  sorgu <- paste(
    "SELECT name, system_type_name, max_length, error_type, is_hidden",
  ...
  # HATA SATIRI TANIMLAYICI DEĞİLDİR: betimleyici yok sayılır ve çağıran (kanıtlanmamış genişlik) yoluna düşer.
  if ("error_type" %in% names(cerceve) && any(!is.na(cerceve$error_type))) {
    return(NULL)
  }

  # GİZLİ SÜTUN SONUÇTA YOKTUR: `FOR BROWSE` anahtar sütunlarını `is_hidden = 1` işaretler ve `dbFetch()` onları DÖNDÜRMEZ; genişliklerini toplamak gerçekte sığan bir sonucu daha küçük parçalara böler ya da tamamen reddederdi. Hata satırı denetiminden SONRA süzülür.
  if ("is_hidden" %in% names(cerceve)) {
    gizli <- suppressWarnings(as.integer(cerceve$is_hidden))
    cerceve <- cerceve[is.na(gizli) | gizli != 1L, , drop = FALSE]
    if (nrow(cerceve) == 0L) return(NULL)
  }
...
.pk_sql_column_from_descriptor <- function(satir) {
  # `%||%` YALNIZCA `NULL` ATLAR: `sys.dm_exec_describe_first_result_set` ifadeyi tam betimleyemediğinde `system_type_name` SQL NULL döner, DBI bunu `NA_character_` yapar ve `grepl()` `NA` yayar.
  # Sonraki `if (NA)` "missing value where TRUE/FALSE needed" ile sınıflandırmayı DÜŞÜRÜRDÜ; kanıtlanmamış tip zaten aşağıda güvenli tarafta ("text"/sınırsız) biter.
...
#'
#' `name` TİP ÇÖZÜMÜNDE yok sayılır (kolon takma adı bir tip adına benzediği için sonuç reddedilmemelidir) ama TANILAMA için tanıma taşınır.
#'
...
pk_sql_columns_from_metadata <- function(column_info, schema = NULL) {
  # SÜRÜCÜ TANIMLAYICISI VARSA O KULLANILIR: sayısal ODBC kodları MAX ile sınırlı değişken genişliği ayırt edemez, tanımlayıcı ayırt eder.
  if (is.list(schema) && length(schema) > 0L) {
    # ADLAR TANILAMAYA TAŞINIR (PR #719 incelemesi, P3): boyut planlayıcısının reddi (`unbounded_lob_column`, `unproven_variable_width_column`, `unknown_type_code`) adsız girdide `col_3` gibi KONUMSAL etiket basıyordu ve operatör hangi sütunun reddi tetiklediğini göremiyordu.
    return(lapply(schema, function(s) {
      tanim <- .pk_sql_column_from_descriptor(s)
      ad <- suppressWarnings(as.character(s$name %||% "")[1])
      if (length(ad) == 1L && !is.na(ad) && nzchar(ad)) tanim$name <- ad
      tanim
    }))
  }
  ...
  adlar <- .pk_sql_metadata_field(column_info, c("name", "column_name", "field.name"))  # TANILAMA için ad; tip çözümünde KULLANILMAZ
  tanimlar <- lapply(seq_len(nrow(column_info)), function(i) {
    ...
    if (!is.na(kod)) {
      # Sayısal ODBC kodu: LOB kodları üst sınır ÜRETMEZ; `sql_variant` (-98) KANITLANMIŞ 8.016 bayt üst sınırı taşır.
      if (kod %in% .PK_ODBC_SQL_VARIANT_CODE) {
      ...
      # `[[` DEĞİL `[`: adlandırılmış atomik vektörde OLMAYAN bir ad `[[` ile "subscript out of bounds" fırlatır.
      # Desteklenmeyen bir ODBC tip kodu bu yüzden `__unknown__` dalına HİÇ ulaşamıyor, planlayıcı çağrısı da tek-satır getirmeye düşmek yerine ABORT ediyordu.
  ...
  if (is.null(adlar)) return(tanimlar)
  Map(function(t, a) {
    a <- suppressWarnings(as.character(a)[1])
    if (length(a) == 1L && !is.na(a) && nzchar(a)) t$name <- a
    t }, tanimlar, adlar)
}
