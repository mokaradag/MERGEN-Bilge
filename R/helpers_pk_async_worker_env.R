...
.pk_async_file_digest <- function(path) {
  ...
  # ÇARPIM `numeric` YAPILIR: `as.integer(ham) * seq_along(ham)` TAMSAYI
  # aritmetiğidir ve ~8 MB üzerindeki dosyalarda tek tek çarpımlar taşarak
  # `NA` üretir; parmak izi ":NA" olur ve İÇERİĞİ FARKLI ama boyutu aynı iki
  # dosya AYNI görünürdü. Taşmayan girdilerde değer BİT BAZINDA aynıdır.
  agirlikli <- sum(as.numeric(as.integer(ham)) * seq_along(ham)) %% 2147483647
  paste0(boyut, ":", format(agirlikli, scientific = FALSE))
}
...
.pk_async_sql_paths_from_source <- function(repo_root, files) {
  ...
  for (dosya in file.path(repo_root, adaylar)) {
    ...
    # BAĞLANTI HER YOLDA KAPANIR: hata işleyicisi KENDİ çerçevesinde çalışır, `con` orada tanımlı değildir.
    metin <- tryCatch({
      con <- file(dosya, open = "rb")
      ham <- tryCatch(readBin(con, what = "raw", n = boyut), finally = close(con))
...
pk_async_bootstrap_fingerprint <- function(repo_root, files) {
  ...
  # SON ÇARE İÇERİĞİ TAŞIR (PR #705, P3): `nchar()` özeti dosya İÇERİĞİNİ atar
  # ve iki farklı checkout aynı toplam karakter sayısını verdiğinde parmak izleri
  # ÇAKIŞIR; işçi ÖNCEKİ revizyonu sürdürürdü. `identical()` karşılaştırması
  # uzun dizeden etkilenmez.
  paste0(length(hedefler), ":", ham)
}
...
pk_async_worker_commit_env <- function(stage, target = globalenv()) {
  ...
  # ATOMİK COMMIT: DOKUNULACAK HER BAĞLAMA ÖNCE YEDEKLENİR.
  #
  # Eskiden silme/atama doğrudan `target` üzerinde yapılıyor, başarısızlık
  # denetimi ise TÜM mutasyonlardan SONRA çalışıyordu: kilitli/yazılamayan tek
  # bir bağlama `ok = FALSE` üretse de BAŞARILI olan mutasyonlar YERİNDE
  # kalıyordu. Kalıcı işçi o noktadan sonra eski ve yeni yardımcıların
  # KARIŞIMINI taşıyor ve sonraki isteklerde tutarsız güvenlik mantığı
  # çalıştırabiliyordu. Artık kısmi commit GERİ ALINIR.
  dokunulan <- unique(c(atilacak, yeni))
  vardi <- dokunulan[vapply(dokunulan, exists, logical(1),
                            envir = target, inherits = FALSE)]
  yedek <- new.env(parent = emptyenv())
  for (ad in vardi) {
    alindi <- try(assign(ad, get(ad, envir = target, inherits = FALSE), envir = yedek),
                  silent = TRUE)
    # YEDEKLENEMEYEN bağlama için geri alma GARANTİ EDİLEMEZ: commit hiç
    # başlatılmaz (kısmi bir duruma girmektense hiç değiştirmemek güvenlidir).
    if (inherits(alindi, "try-error")) {
      return(list(ok = FALSE, names = yeni, failed = paste0(ad, " (yedeklenemedi)")))
    }
  }
  for (ad in atilacak) {
  ...
  if (length(basarisiz)) {
    # GERİ ALMA: önceki revizyon aynen yerine konur, commit sırasında YENİ
    # oluşturulmuş semboller kaldırılır. Sahiplik listesi de GÜNCELLENMEZ;
    # böylece bir sonraki bootstrap eski isimleri kendi sahipliğinde görür.
    for (ad in vardi) {
      try(assign(ad, get(ad, envir = yedek, inherits = FALSE), envir = target),
          silent = TRUE)
    }
    for (ad in setdiff(dokunulan, vardi)) {
      if (exists(ad, envir = target, inherits = FALSE)) {
