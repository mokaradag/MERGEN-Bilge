...
# Silme sonucundan YEREL DURUM kararını üretir (saf; Shiny/IO içermez).
#   abort   = fiziksel silme OLMADI -> tablo/bağlam/indeks korunur.
#   partial = dosya SİLİNDİ ama indeks/artefakt temizliği kısmi kaldı -> yerel
#             durum temizlenir, eksik temizlik AYRI bildirilir.
# `deleted_physical` ile `delete_failed` BİRLİKTE TRUE olabilir; eski koşul bu
# durumda erken dönüp indirilemeyen bir satır bırakıyordu ve sonraki istekler
# var olmayan yolu referans alıyordu.
fm_delete_local_state_plan <- function(silme) {
  silindi <- isTRUE(silme$deleted_physical)
  basarisiz <- isTRUE(silme$delete_failed)
  list(abort = basarisiz && !silindi, partial = basarisiz && silindi)
}
...
fm_attempt_delete_candidate <- function(durum, candidate, source_label,
  ...
  # KISA ZAMAN AŞIMI: silme Shiny gözlemcisinden çalışır ve kilit beklemesindeki
  # `Sys.sleep()` TEK R sürecini bloklar. `confirm_delete_file` en fazla üç
  # doğrudan aday + `resolve` + `fallback` denediği için varsayılan 5 saniyelik
  # bekleme aynı süreçteki DİĞER oturumları saniyelerce geciktirebiliyordu.
  # `require_lock = FALSE` olduğu için kilit alınamazsa mevcut kilitsiz geri
  # dönüş davranışı KORUNUR; bu değer yalnızca bekleme süresini kısaltır.
  if (exists(".file_store_with_index_lock", mode = "function", inherits = TRUE)) {
    .file_store_with_index_lock(sil(), timeout_sec = 0.5, require_lock = FALSE)
  } else {
    sil()
  }
}
...
fm_rollback_failed_upload <- function(sonuc, ctx) {
  ...
  if (is.function(ctx$rollback_uploaded_file)) {
    geri <- try(ctx$rollback_uploaded_file(sonuc$name, exact_path = sonuc$dest),
                silent = TRUE)
...
# `exact_path` geri almayı DEĞİŞMEZ kimliğe bağlar: yalnızca ada bakan kaldırma,
# aynı adla güncel başka bir yükleme varken onu siliyordu.
fm_make_upload_rollback <- function(unregister_fn, remove_row_fn) {
  function(filename, exact_path = NULL) {
    a <- try(unregister_fn(filename), silent = TRUE)
    b <- try(remove_row_fn(filename, quiet = TRUE, exact_path = exact_path), silent = TRUE)
...
fm_clear_user_bucket_safely <- function(uid, fm_debug = NULL) {
  # ÇÖZÜLEMEYEN KİMLİK KAPALI-BAŞARISIZDIR. `NULL` kimlikte `TRUE` dönmek,
  # kalıcı kovadaki dosyalar diskte KALIRKEN çağıranın modül durumunu boşaltıp
  # "diskten de temizlendi" bildirmesine yol açıyordu.
  if (is.null(uid) || !nzchar(as.character(uid)[1])) {
    if (is.function(fm_debug)) fm_debug("clear_files_failed", "kullanıcı kimliği çözülemedi")
    return(invisible(FALSE))
  }
  if (exists("fm_valid_user_id", mode = "function", inherits = TRUE) &&
      !isTRUE(fm_valid_user_id(uid))) {
    if (is.function(fm_debug)) fm_debug("clear_files_failed", "kullanıcı kimliği geçersiz")
    return(invisible(FALSE))
  }
  sonuc <- tryCatch(
