...
#' BAĞIMSIZ çalışır.
.PK_RETRIEVAL_NOT_FOR_FRAME <- enc2utf8(c("bu", "sorgu", "sorgusu", "sorgular", "için", "icin", "değil", "degil", "değildir", "degildir", "kullanılmaz", "kullanilmaz", "uygun", "uygundur", "amacıyla", "amaciyla", "amaçla", "amacla", "yönelik", "yonelik", "ilişkin", "iliskin", "konusunda", "hakkında", "hakkinda", "şeklinde", "seklinde", "olarak", "asla", "hiçbir", "hicbir"))  # UTF-8 SABİTLENİR (PR #705 inceleme, P2): Türkçe Windows VM `source(..., encoding = "UTF-8")` ile bu sabitleri YEREL kod sayfasına çevirir, `pk_tr_fold()` ise UTF-8 işaretli metin döndürür; iki taraf eşleşmeyince `setdiff()` çerçeve sözcüklerini DÜŞÜREMİYOR ve küratörün `not_for` dışlaması VM'de hiç ateşlenmiyordu. OLUMSUZ CÜMLE ÇERÇEVESİ. `not_for` bir CÜMLE olduğunda ("Bu sorgu gerçekleşen maliyetler için değildir") `all(belirtecler %in% istem)` ASLA eşleşmiyordu: istem "gerçekleşen maliyetler" olsa bile "bu"/"sorgu"/"için"/"değildir" belirteçleri istemde yoktur, `pk_retrieval_excluded_ids()` kimliği atlıyor ve küratörün AÇIK dışlaması hiç ateşlenmiyordu. Bu dilbilgisel çerçeve sözcükleri düşürülür; İÇERİK sözcüklerinin TAMAMI hâlâ istemde geçmek zorundadır, dolayısıyla eski yanlış-pozitif koruması korunur. Türkçe'de çekim ekleri ve serbest sözcük sırası yüzünden bitişik ifade (substring) eşleşmesi yerine belirteç KÜMESİ kullanılır; ASCII'leştirilmiş biçimler de listelenir çünkü `pk_tr_fold()` yalnızca küçük harfe çevirir, aksanları düşürmez.
...
pk_retrieval_agreement <- function(index, prompt, selected_id, top_n, library = NULL,
  ...
  dislama_istemi <- exclusion_prompt %||% prompt  # OLUMSUZ KANIT YALNIZCA GÜNCEL SORUYA AİTTİR: sözlüksel puanlama için istemi konuşma geçmişiyle GENİŞLETMEK bilinçlidir, ama aynı genişletme `not_for` dışlamasına uygulandığında ÖNCEKİ bir turun jetonları `not_for` ifadesini tamamlayıp GEÇERLİ bir seçimi dışlıyordu.
  ...
  if (!is.null(library)) {
    tum_dislanan <- pk_retrieval_excluded_ids(library, dislama_istemi)
