...
# KATMAN 6 KULLANILABİLİR Mİ? (süreç başına BİR KEZ uyarır)
#
# `stringdist` `required_packages` üyesidir ve geçerli bir dağıtımda HER ZAMAN
# vardır. Yine de eksik olduğunda tek bir düzenleme-mesafesi hesabı TÜM varlık
# çözümlemesini İSTİSNAYA çevirmemelidir: kısa listeye giren ve katman 1-5'te
# eşleşmeyen HER aday buraya ulaşır, yani tek eksik paket her isteği düşürürdü.
# KAPALI BAŞARISIZ davranış doğru olanıdır: katman 6 eşleşme ÜRETEMEZ, istek
# netleştirme çiplerine düşer. Testler de paketi opsiyonel sayıyordu; artık
# çalışma zamanı ve test beklentisi AYNI.
.pk_entity_edit_available <- function() {
  if (requireNamespace("stringdist", quietly = TRUE)) return(TRUE)
  if (!isTRUE(.pk_entity_score_state$stringdist_warned)) {
    .pk_entity_score_state$stringdist_warned <- TRUE
    try(warning(
      "pk_entity_score: 'stringdist' yok; katman 6 (düzenleme mesafesi) DEVRE DIŞI.",
      call. = FALSE
    ), silent = TRUE)
  }
  FALSE
...
.pk_entity_score_state <- new.env(parent = emptyenv())
.pk_entity_score_state$stringdist_warned <- FALSE
...
.pk_entity_edit_counts <- function(a, b) {
  ...
  if (!.pk_entity_edit_available()) return(NULL)
