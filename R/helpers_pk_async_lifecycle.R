...
.mergen_pk_chip_labels <- function(chips) {
  ...
  etiketler <- vapply(chips, function(cip) {
    # ADAYLAR SIRAYLA DENENİR. `%||%` YALNIZCA `NULL` atlar; `name` alanı
    # `NA_character_` geldiğinde eski kod `label` adayını HİÇ denemiyor,
    # doğrudan `id`/`value` değerine düşüyordu (PR #719 inceleme, P3).
    # ÇÖZÜMLEYİCİ ÇİPLERİ `label`/`value` taşır; yalnızca `name`/`id` okununca
    # etiketler BOŞ kalıyor ve tıklanabilir seçenek HİÇ gösterilmiyordu.
    adaylar <- if (is.list(cip)) {
      list(cip[["name"]], cip[["label"]], cip[["id"]], cip[["value"]])
    } else {
      list(cip)
    }
    for (aday in adaylar) {
      deger <- suppressWarnings(trimws(as.character(aday)[1]))
      if (length(deger) == 1L && !is.na(deger) && nzchar(deger)) return(deger)
    }
    ""
  }, character(1))
