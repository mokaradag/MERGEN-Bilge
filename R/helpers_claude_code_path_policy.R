...
cc_policy_allowed_workdir_roots <- function(user_id = NULL,
  ...
  if (isTRUE(allow_system_temp)) {
    ...
    if (isTRUE(kullanici_kapsamli)) {
      ...
      # ÜST KÖKLER AÇILMAZ: eskiden `claude_code_runtime` ve
      # `claude_code_workspaces` ÜST kökleri ekleniyordu; `cc_policy_path_inside_roots()`
      # yalnızca kök önekine baktığı için BAŞKA bir kullanıcının `user_<id>` alt
      # ağacı çalışma dizini olarak kabul edilebiliyordu (IDOR).
      configured <- c(configured, cc_runtime_user_dir(kullanici_no))
    }

    # BAĞLANTI TESTİ KÖKÜ HER İKİ DALDA da izinlidir. Bu kök kullanıcıdan
    # BAĞIMSIZDIR ve hiçbir `user_<id>` alt ağacının üstü DEĞİLDİR, bu yüzden
    # IDOR yüzeyi açmaz. Yalnızca `else` dalında eklendiğinde, Yapılandırma
    # sayfası POZİTİF `cc_test_uid` geçirdiğinde `cc_policy_validate_workdir()`
    # bağlantı testi dizinini reddediyor ve CLI hiç çalışmadan test
    # "Çalışma dizini güvenlik ilkesi tarafından engellendi" ile düşüyordu.
    configured <- c(configured, cc_policy_temp_workspace_root())
  }
...
#
# KÖK, KULLANICI ÇALIŞMA ALANLARININ ÜSTÜ DEĞİLDİR. Önceden `claude_code_workspaces`
# ÜST dizini döndürülüyordu; o kök izinli sayıldığında altındaki her
# `user_<id>` alt ağacı da çalışma dizini olarak kabul edilebiliyordu. Ayrı bir
# alt klasör kullanıldığı için hiçbir kullanıcının alt ağacı bu yolla erişilemez.
CC_POLICY_TEMP_WORKSPACE_DIR <- "_baglanti_testi"

cc_policy_temp_workspace_root <- function() {
  yol <- file.path(tempdir(), "claude_code_workspaces", CC_POLICY_TEMP_WORKSPACE_DIR)
  if (!dir.exists(yol)) {
...
# `listeleme_onbellegi`: TEK yükleme olayı boyunca ÜST DİZİN listelemelerini
# önbellekleyen bir ortam (isteğe bağlı). Sarkan-bağlantı sondası her dosya için
# `list.files()` çağırdığında, tarayıcıdan gelen tek klasör yüklemesi aynı dizine
# N dosya koyduğu için maliyet O(N^2) ad karşılaştırması + N dizin listelemesi
# oluyordu; bu iş Shiny olay döngüsünde çalıştığı için aynı R sürecindeki TÜM
# oturumlar bloke oluyordu (UNC'de her listeleme ayrıca ağ turu).
# Önbellek BAYATLIĞI güvenlidir: yalnızca BU döngünün yazdığı adlar eksik
# kalabilir ve o adlar için `file.exists()` zaten TRUE döner (sonda hiç
# çalışmaz). Listeleme okunamadığında sonuç KAPALI-BAŞARISIZ olarak saklanır.
cc_setup_yerel_yukleme_hedefi <- function(calisma_alani, goreceli,
                                          listeleme_onbellegi = NULL) {
  ...
  if (!var_mi && dir.exists(ust_dizin)) {
    onbellek_anahtari <- anahtar(ust_dizin)
    girisler <- NULL
    if (is.environment(listeleme_onbellegi) &&
        nzchar(onbellek_anahtari) &&
        !is.null(listeleme_onbellegi[[onbellek_anahtari]])) {
      girisler <- listeleme_onbellegi[[onbellek_anahtari]]
    } else {
      girisler <- try(
        suppressWarnings(list.files(ust_dizin, all.files = TRUE, no.. = TRUE)),
        silent = TRUE
      )
      girisler <- if (inherits(girisler, "try-error")) NA_character_ else anahtar(girisler)
      if (is.environment(listeleme_onbellegi) && nzchar(onbellek_anahtari)) {
        listeleme_onbellegi[[onbellek_anahtari]] <- girisler
      }
    }
    # `NA_character_` sentinel'i listelemenin OKUNAMADIĞINI bildirir; karar
    # kapalı-başarısızdır.
    if (length(girisler) == 1L && is.na(girisler[1])) return(NULL)
    if (anahtar(taban) %in% girisler) return(NULL)
  }
