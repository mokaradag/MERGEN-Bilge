...
.pk_result_type_is_lob <- function(sql_type, max_length = NA) {
  ...
  if (tip %in% .PK_VARWIDTH_TYPES) {
    # BEYAN EDİLEN `MAX` İŞARETİ DE KANITTIR.
    #
    # `.pk_result_type_key()` parantezli kısmı atar, bu yüzden
    # `"nvarchar(max)"` yalnızca `"nvarchar"` olur ve LOB kararı SADECE
    # `max_length < 0` değerine kalıyordu. `list(type = "nvarchar(max)",
    # max_length = NA)` gibi bir bildirimde `pk_column_width_upper_bound()`
    # sınırsız der ama sütun `lob_columns` listesine GİRMEZ;
    # `pk_sql_plan_chunk_rows()` o zaman tek satırlık parçaya düşerdi.
    ham <- suppressWarnings(as.character(sql_type)[1])
    if (length(ham) == 1L && !is.na(ham) &&
        grepl("\\(\\s*max\\s*\\)", ham, ignore.case = TRUE, perl = TRUE)) {
      return(TRUE)
    }
    uzunluk <- suppressWarnings(as.numeric(max_length)[1])
...
pk_sql_plan_chunk_rows <- function(column_info, chunk_rows = 5000L,
  ...
  if (!isTRUE(genislik$bounded)) {
    ...
    # BİLİNMEYEN TİP KODU DA KANITSIZDIR (PR #705 incelemesi, P3): tip kodu ne
    # `.PK_ODBC_FIXED_TYPE_BYTES` tablosunda ne de LOB listesindedir, bayt/karakter
    # oranı BİLİNMEZ ve tek hücre tavanı aşabilir. `metadata_unavailable` ile
    # `unproven_variable_width_column` aynı riski reddederken bu dal
    # `dbFetch(n = 1)` çağrısını refuse ETMEDEN yetkilendiriyordu.
    bilinmeyen <- vapply(sutunlar, function(s) {
