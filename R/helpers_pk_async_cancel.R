...
# SÜREÇ-YEREL ETKİN JETON KAYDI: yaş tabanlı temizlik İPTAL EDİLMİŞ AMA HÂLÂ
# ETKİN bir isteğin jetonunu siliyordu (son tarihi `max_age_sec` üzerindeki bir
# istekte bloke olan işlem geri döndüğünde `pk_async_stage_gate()` isteği "iptal
# edilmemiş" okuyup analize DEVAM ediyordu). Anahtar dosya ADIdır; hem
# `pk_cancel_token_path()` hem `list.files()` aynı adı üretir. Süreç çökerse
# kayıt da yok olur, sızan dosyalar yine yaşa göre toplanır.
.pk_cancel_active_tokens <- new.env(parent = emptyenv())
...
pk_cancel_token_signal <- function(token_path) {
  ...
  # ETKİN olarak kaydet: bayat temizlik bu jetona DOKUNMAZ.
  if (isTRUE(ok)) assign(basename(yol), TRUE, envir = .pk_cancel_active_tokens)
...
pk_cancel_token_clear <- function(token_path) {
  ...
  # TERMİNAL DURUM: kayıt düşer, dosya artık bayat sayılabilir.
  anahtar <- basename(yol)
