...
.pk_select_column_line <- function(ad, cm) {
  ...
  # `%||%` YALNIZCA `NULL` ATLAR: `label` `NA_character_` olduğunda etiket `NA` kalıyor ve `sprintf()` sütun adı yerine "NA" metnini basıyordu (Geçiş A yolu `NA` etiketleri zaten düşürür).
  etiket <- suppressWarnings(as.character(cm$label)[1])
  if (length(etiket) != 1L || is.na(etiket) || !nzchar(etiket)) etiket <- ad
