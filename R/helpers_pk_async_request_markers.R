...
# kullanılır (kimlikler süreç içinde pratikte benzersizdir ve her iki işaretin
# yönü de güvenlidir).
...
.pk_marker_note <- function(slot, key) {
  ...
  .pk_request_marker_store[[slot]] <- utils::tail(unique(c(mevcut, key)), 500L)
  invisible(TRUE)
}
...
mergen_pk_invalidate_requests <- function(session, request_ids) {
  ...
  # KAYBOLMAZ.
  for (kimlik in kimlikler) .pk_marker_note("abandoned", .pk_marker_key(session, kimlik))
