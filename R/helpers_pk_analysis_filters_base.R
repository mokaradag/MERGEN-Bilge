...
# `nzchar()` GİRDİSİ HER ZAMAN UZUNLUK-1 OLMALIDIR (PR #705 incelemesi, P2).
#
# `.pk_filter_scalar_token()` eksik/boş alan için `NULL` döner; `nzchar(NULL)`
# `logical(0)` üretir ve `if (...)` / `&&` bunu "argument is of length zero"
# hatasıyla ÇÖKERTİR. Bu gövde izole test/hata ayıklama oturumlarında doğrudan
# source edildiği için koruma burada da gereklidir.
.pk_filter_scalar_chr <- function(x) {
  deger <- .pk_filter_scalar_token(x)
  if (is.null(deger) || length(deger) != 1L || is.na(deger)) "" else as.character(deger)[1]
}
...
# v1 MOTORUNUN DESTEKLEDİĞİ İŞLEMLER (istem modele TAM OLARAK bunları bildirir).
# Liste dışındaki bir işlem sessizce eşitliğe çevrilmez; yaprak DÜŞÜRÜLÜR.
PK_V1_SUPPORTED_FILTER_OPS <- c("exact_match", "contains", "greater_than", "less_than")
  ...
  tryCatch({
    ...
    if (length(filters) > 0) {
      filters <- lapply(filters, function(f) {
        # ATOMİK ÖĞE YAPRAK DEĞİLDİR. `simplifyVector = FALSE` altında
        # `"filters": ["ProjeKodu"]` karakter skalerlerinden oluşan bir liste
        # üretir; `f$column` atomik vektör üzerinde HATA fırlatır ve dıştaki
        # tryCatch TÜM yanıtı `error` durumuna çevirirdi. Aşağıdaki ikinci
        # normalleştirme döngüsü zaten `is.list(f)` koruması uygular.
    ...
    if (length(filters) > 0) {
      ...
      # SÜTUN ÖNCE TEK KARAKTER DEĞERE İNDİRGENİR.
      #
      # Ayrıştırıcı `simplifyVector = FALSE` kullandığı için
      # `"column": ["ProjeAdi","ProjeKodu"]` LİSTE olarak gelir; `nzchar(f$column)`
      # o zaman "invalid 'x' type in 'nzchar'" ile hata verir, dıştaki
      # `tryCatch` TÜM yanıtı `error` durumuna çevirir ve v2 motoru diğer
      # yaprakları geçerli olan bir yanıtı REDDEDERDİ. Artık yalnızca BOZUK
      # yaprak düşer.
      filters <- lapply(filters, function(f) {
        if (!is.list(f) || .pk_filter_base_is_group(f)) return(f)
        if (!is.null(f$column)) f$column <- .pk_filter_scalar_token(f$column)
        f
      })
...
apply_smart_filters <- function(data, filter_instructions, user_prompt) {
  ...
  # BOŞ YETKİLİ ÇERÇEVE DE `count` SORUSUNU CEVAPLAR: doğru cevap `Adet = 0`
  # olduğu halde boş çerçeve aşağı akışta "veri yok" diye anlatılıyordu
  # (çalışma zamanı gövdesiyle AYNI sözleşme).
  if (nrow(data) == 0) {
    agg_bos <- .pk_filter_base_ascii_lower(
      .pk_filter_scalar_token(filter_instructions$aggregation)
    )
    if (identical(agg_bos, "count")) {
      ...
      return(data.frame(Sonuc = aciklama, Adet = 0L, stringsAsFactors = FALSE))
    }
    return(data.frame())
  }
  ...
  # ALAN LİSTE GELEBİLİR: `simplifyVector = FALSE` altında `"filter_expression":
  # []` veya bir nesne LİSTE olarak gelir ve `nzchar()` "invalid 'x' type"
  # hatası fırlatır. Çalışma zamanı gövdesiyle AYNI biçimde önce skalere indir.
  .ifade <- .pk_filter_scalar_chr(filter_instructions$filter_expression)
  if (nzchar(.ifade)) {
  ...
  if (!applied_expression_success) {
    if (!is.null(filters) && length(filters) > 0) {
      ...
      for (f in filters) {
        ...
        # SÜTUN TEK KARAKTER DEĞERE İNDİRGENİR: liste biçimli `column`,
        # `col %in% names(dt)` ifadesini uzunluğu >1 bir vektöre çevirir ve
        # R 4.3+ bunu `&&` içinde HATA sayar (çalışma zamanı gövdesiyle AYNI).
        col <- .pk_filter_scalar_chr(f$column)
        val <- f$value
        op <- .pk_filter_scalar_chr(f$operation %||% "exact_match")
        ...
        op <- .pk_filter_base_ascii_lower(op)
        if (!(op %in% PK_V1_SUPPORTED_FILTER_OPS)) {
          cat(sprintf("[SMART_FILTER] Islem v1 motorunda desteklenmiyor, atlandi: %s\n", op))
          next
        }
        ...
        if (nzchar(col) && col %in% names(dt)) {
          ...
          if (is.character(col_vals) || is.factor(col_vals)) {
            ...
            # 2^53 yuvarlamasını engeller (çalışma zamanı gövdesiyle AYNI).
            operand <- pk_filter_numeric_operand(col_vals, val_str)
            # REDDEDİLEN OPERAND YAPRAĞI SESSİZCE DÜŞÜRMEZ. `integer64` bir
            # sütuna "1500.0"/"15,5" geldiğinde `ok = FALSE` döner; eski gövde
            # `dt`'ye HİÇ dokunmuyor ve `exact_match` SIFIR satır yerine TÜM
            # satırları geri veriyordu (çalışma zamanı gövdesi yaprağı düşürür).
            if (!isTRUE(operand$ok)) {
              cat("[SMART_FILTER] Deger sayisal bicime donusturulemedi, filtre atlandi.\n")
              next
            }
