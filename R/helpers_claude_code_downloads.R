...
collect_claude_code_generated_downloads <- function(tool_uses,
                                                    ...
                                                    session_token = "",
                                                    cancel_fn = NULL) {
  # PAYLAŞILAN GÖRÜNÜRLÜK PENCERESİ KEŞFİ DE KAPSAR. Pencere yalnızca
  # `stage_claude_code_downloads()` içinde açıldığında, ondan ÖNCE çalışan
  # `list_claude_code_generated_file_paths()` bütçesiz kalıyor;
  # `cc_path_visibility_budget_remaining()` `NULL` döndüğü için
  # `cc_wait_for_path_visible()` HER aday için tam `file_settle_total_ms`
  # harcıyordu (araç kullanımı başına üç aday). Çok sayıda Write/Edit üreten bir
  # çalıştırmada worker dakikalarca meşgul kalıyordu.
...
cc_stage_tool_use_write_paths_as_downloads <- function(tool_uses,
  ...
  # Yedek keşif yolu da AYNI paylaşılan görünürlük penceresinde çalışır.
  cc_with_path_visibility_budget({
  allowed_roots <- cc_policy_allowed_output_roots(
    user_id = user_id,
    workdir = runtime_workdir %||% source_workdir
