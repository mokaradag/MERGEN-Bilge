...
# BÜTÇE NOTU PAYI (karakter).
#
# Not, modelin GÖRDÜĞÜ nihai yüke girer; hem v1 hem v2 yolu aynı payı ayırır.
# Tek yerde tutulur, aksi hâlde iki yol sessizce ayrışır.
.PK_PROMPT_NOTE_ALLOWANCE <- 220L
...
pk_prompt_fit_payload <- function(summary_text, preview_data, assemble, budget = NULL) {
  ...
  repeat {
    ...
    if (uzunluk > budget && satir_sayisi == 0L) {
      ...
      #
      # SABİT YÜK TEK BAŞINA BÜTÇEYİ AŞIYORSA KESME DE YETMEZ. Eski yorum
      # "gönderilen yük artık bütçeye sığar" diyordu; bu HER ZAMAN doğru
      # değildir: `pk_build_analysis_payload()` ifşa bloğu + not payı tüm
      # bütçeyi tükettiğinde `fit_butcesi` `0`a kelepçelenir, `kalan` negatif
      # olur ve `kisa_ozet` yalnızca kesme notundan ibaret kalır. Yük o zaman
      # `sabit_yuk + nchar(kesme_notu)` uzunluğundadır ve bütçeyi aşabilir.
      # `budget_exhausted` bu durumu AÇIKÇA bildirir; kullanıcıya/modele giden
      # bildirim (`pk_prompt_budget_note()`) her iki durumda da korunur.
      return(list(
        payload = yuk,
        preview_rows = 0L,
        ...
        chars = uzunluk,
        over_budget = TRUE,
        budget_exhausted = uzunluk > budget,
...
#'
#' HER İKİ MOTOR DA BÜTÇEYE SIĞDIRIR. v1 yolu artık `pk_prompt_fit_payload()`
#' çağırır (not payı düşülmüş bütçeyle) ve kırpma olduğunda
#' `pk_prompt_budget_note()` ifşasını yüke ekler; bütçenin altındaki bir yükte
#' çıktı v1'in eski davranışıyla BİREBİR aynıdır. v1'de olmayan tek şey
#' POLİTİKA İFŞALARI ve olgu/paket makinesidir; onlar v2'ye özgüdür.
pk_build_analysis_payload <- function(stat_summary, query, engine_is_v2 = FALSE, policy = NULL) {
  ...
  if (!isTRUE(engine_is_v2)) {
    ...
    # NOT PAYI v1'DE DE AYRILIR: not, modelin GÖRDÜĞÜ nihai yüke girer.
    #
    # v2 yolu `not_payi` kadar düşerek sığdırma yapıyor, v1 yolu tüm bütçeyi
    # geçirip notu SONRADAN ekliyordu; yük kırpıldığında ya da bütçeyi aştığında
    # `pk_prompt_budget_note()` boş olmadığı için nihai yük
    # `MERGEN_PK_PROMPT_CHAR_BUDGET` sınırını not uzunluğu kadar aşıyordu.
    toplam_v1 <- pk_prompt_char_budget(if (is.list(query)) query$meta else NULL)
    fit_v1 <- pk_prompt_fit_payload(
      stat_summary$summary_text,
      stat_summary$preview_data,
      kur,
      budget = max(0L, as.integer(toplam_v1) - .PK_PROMPT_NOTE_ALLOWANCE)
    )
  ...
  not_payi <- .PK_PROMPT_NOTE_ALLOWANCE
