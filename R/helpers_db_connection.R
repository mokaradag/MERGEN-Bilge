...
# DIŞ GEÇEN-SÜRE SINIRININ MUTLAK SON TARİHİ.
#
# R, kurulu `setTimeLimit()` sınırını GERİ OKUMA yolu sunmaz. Eski çıkış
# işleyicisi `elapsed = Inf` yazarak DIŞ çağıranın sınırını RESTORE etmek
# yerine SİLİYORDU: `db_pool_healthy(timeout_sec = 5)` -> `get_pool_info()` ->
# `get_connection()` -> bu yardımcı zincirinden sonra `DBI::dbGetQuery()`
# HİÇBİR geçen-süre sınırı olmadan çalışıyor ve asılı bir sonda sorgusu
# kesilemiyordu. Mutlak son tarih burada tutulur; iç bütçe dışı yalnızca
# DARALTIR ve çıkışta dışın KALAN süresi geri yüklenir.
.db_elapsed_state <- new.env(parent = emptyenv())
.db_elapsed_state$deadline_at <- NULL

# `honor_outer = FALSE`: TEMİZLİK KENDİ TABANINI KORUR. Dış mutlak son tarih
# TÜKENMİŞ olsa bile kapatma 0.05 sn'ye daralmaz; aksi hâlde `dbDisconnect()`
# kesilir, bağlantı kirli işaretlenir ve fiziksel oturum finalizer'a kadar
...
.db_with_elapsed_budget <- function(budget_sec, fn, honor_outer = TRUE) {
  ...
  on.exit({
    .db_elapsed_state$deadline_at <- dis_son
    kalan <- if (is.null(dis_son)) Inf else max(0.05, dis_son - as.numeric(Sys.time()))
    try(setTimeLimit(cpu = Inf, elapsed = kalan, transient = TRUE), silent = TRUE)
  }, add = TRUE)
...
db_pool_healthy <- function(timeout_sec = 5) {
  ...
  # AYNI muhasebeden geçer: iç yardımcı bu sınırı görebilsin ve çıkışta onu
  # SİLMEK yerine KALAN süresini geri yükleyebilsin.
  info <- tryCatch(
    .db_with_elapsed_budget(as.numeric(timeout_sec), get_pool_info),
...
get_connection <- function(target = "primary") {
  ...
  if (is.null(dsn_var) || !nzchar(dsn_var)) {
    stop(
      sprintf(
        "HATA: Tanımsız veritabanı hedefi: '%s'",
  ...
  if (identical(login_plan$mode, "refuse")) {
    stop("PK istek bütçesi login zaman aşımını temsil edemiyor; bağlantı açılmadı.",
         call. = FALSE)
  }
...
release_connection <- function(conn_info) {
  ...
  if (exists("log_warn", mode = "function", inherits = TRUE)) {
    try(log_warn(paste0(
      "[DB] Sınırlı kapatma başarısız; bağlantı KİRLİ sayıldı ve bütçesiz ",
...
worker_db_connect <- function(max_retries = 3, retry_delay = 1) {
  # SIFIR DENEME SESSİZ `NULL` ÜRETEMEZ.
  #
  # `seq_len(0L)` döngüyü atlar ve fonksiyon hatasız `NULL` dönerdi; sonraki DB
  # işlemi "null bağlantı" hatasıyla düşer ve gerçek neden ("hiç bağlantı
  # denemesine izin verilmedi") kaybolurdu.
  deneme <- suppressWarnings(as.integer(max_retries)[1])
  if (length(deneme) != 1L || is.na(deneme) || deneme < 1L) {
    stop("worker_db_connect: 'max_retries' en az 1 olmalıdır.", call. = FALSE)
  }
  max_retries <- deneme
