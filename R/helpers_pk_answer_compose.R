...
# TAM SAYI VEYA YEDEK: sifir uzunluk / NA / sayisal olmayan deger yedege duser
# ve sonuc en az 1 olur.
.pk_compose_int_or <- function(x, yedek) {
  v <- suppressWarnings(as.integer(x)[1])
  if (length(v) != 1L || is.na(v)) return(yedek)
  max(1L, v)
}

# Yapilandirma degerini cozup TAM SAYIYA indirger.
.pk_compose_int_cfg <- function(key, query_meta, yedek) {
  .pk_compose_int_or(.pk_compose_cfg(key, query_meta, yedek), yedek)
}
...
.pk_compose_has_root <- function(katlanmis, kok) {
  ...
  # HER EŞLEŞME KENDİ ÖNCEKİ KARAKTERİYLE DENETLENİR.
  #
  # `katlanmis` SKALER olduğu için `substr()` `konum` birden çok konum taşısa
  # da TEK bir değer döndürür; `ifelse()` onu tüm konumlara geri dönüştürüp
  # her eşleşmeyi İLK eşleşmenin önceki karakterine göre yargılıyordu.
  # "makexcel excel" (kök `excel`) örneğinde konumlar 4 ve 10, önceki karakter
  # ise ikisi için de 3. konumdaki `k` sanılıyor; bağımsız ikinci geçiş
  # tanınmıyor ve AÇIK bir dışa aktarım isteği yok sayılıyordu.
  onceki <- vapply(konum, function(k) {
    if (k <= 1L) "" else substr(katlanmis, k - 1L, k - 1L)
  }, character(1))
...
# OLUMSUZLAMA KÖKÜ SÖZCÜK BAŞINDA OLMALI; `-ma/-me` KÖKLERİ EK DENETİMİ İSTER.
#
# `gonderme` / `ekleme` / `olusturma` / `uretme` / `isteme` kökleri SIRADAN
# OLUMLU istek biçimlerinin İÇİNDE de geçer: "göndermeni istiyorum",
# "eklemeni istiyorum", "rapor oluşturmanı istiyorum". Çıplak alt dize
# eşleşmesi bunları olumsuzlama sayıyor, olumlu kapsam "ni istiyorum"a
# düşüyor, hiçbir ipucu eşleşmiyor ve KULLANICININ İSTEDİĞİ dışa aktarım
# sessizce düşüyordu. Bu kökler yalnızca sözcük ORADA BİTİYORSA ya da ardından
# GERÇEK bir olumsuz emir eki geliyorsa olumsuzlama sayılır.
.PK_COMPOSE_NEG_STEM_SUFFIX_REQUIRED <- c(
  "yollama", "gonderme", "gönderme", "ekleme", "cikarma", "çıkarma",
  "olusturma", "oluşturma", "uretme", "üretme", "isteme"
)

.PK_COMPOSE_NEG_IMPERATIVE_SUFFIX <-
  "^(yin|yın|yiniz|yınız|yiz|yız|sin|sın|sinler|sınlar)"

.PK_COMPOSE_WORD_CHAR <- "[a-z0-9çğıöşü]"

.pk_compose_negation_is_real <- function(cumle, bas, kok) {
  onceki <- if (bas == 1L) "" else substr(cumle, bas - 1L, bas - 1L)
  if (nzchar(onceki) && grepl(.PK_COMPOSE_WORD_CHAR, onceki)) return(FALSE)

  if (!(kok %in% .PK_COMPOSE_NEG_STEM_SUFFIX_REQUIRED)) return(TRUE)

  kalan <- substr(cumle, bas + nchar(kok), nchar(cumle))
  if (!nzchar(kalan) || !grepl(paste0("^", .PK_COMPOSE_WORD_CHAR), kalan)) return(TRUE)
  grepl(.PK_COMPOSE_NEG_IMPERATIVE_SUFFIX, kalan, perl = TRUE)
}
...
.pk_compose_positive_span <- function(cumle) {
  ...
  for (n in PK_COMPOSE_NEGATIONS) {
    ...
    for (bas in as.integer(konum)) {
      if (!.pk_compose_negation_is_real(cumle, bas, kok)) next
      sonlar <- c(sonlar, bas + nchar(kok))
    }
  }
...
pk_compose_decide <- function(rows, cols, user_prompt = "", query_meta = NULL) {
  ...
  # YAPILANDIRMA DEĞERİ TAM SAYIYA İNDİRGENİR. `.pk_compose_cfg()` yalnızca
  # çözücü YOKSA/HATA verirse yedeğe döner; çözülen ama sayısal olmayan bir
  # değer olduğu gibi geçerdi. Karakter değerde `rows > dt_satir` METİN
  # karşılaştırması yapar ("100" > "200" TRUE); `NA` değerde `if (NA)` tüm v2
  # kompozisyonunu düşürür.
  onizleme <- .pk_compose_int_cfg("MERGEN_PK_PREVIEW_ROWS", query_meta, 10L)
  dt_satir <- .pk_compose_int_cfg("MERGEN_PK_DT_MAX_ROWS", query_meta, 200L)
  dt_sutun <- .pk_compose_int_cfg("MERGEN_PK_INLINE_MAX_COLS_DT", query_meta, 12L)
...
pk_compose_markdown_table <- function(data, max_rows = 15L, max_cols = 12L, meta = list(),
  ...
  # SAYISAL OLMAYAN SINIR VARSAYILANA DÜŞER: `max(1L, NA_integer_)` `NA` kalır
  # ve `utils::head(data, NA)` TÜM kompozisyonu hataya düşürürdü.
  max_rows <- .pk_compose_int_or(max_rows, 15L)
  max_cols <- .pk_compose_int_or(max_cols, 12L)
...
#' Ek KULLANICIYA GERÇEKTEN SUNULDU MU?
#'
#' TEK SÖZLEŞME: `pk_compose_block()`, `pk_compose_attachment_card()` ve
#' `pk_build_analysis_result_v2()` aynı ölçütü kullanmak ZORUNDADIR. XLSX yolu
#' bozulup `pk_export_build()` `csv_fallback` döndürdüğünde dosyalar YİNE
#' sunulur ve indirme bağlantısı üretilir; yalnızca `status == "ok"` kontrol
#' eden bir çağıran, aynı mesajda bağlantısı verilen eki "yok" sayar ve
#' "kalan değerler gösterilmiyor" der.
#'
#' @param artifact `pk_export_build()` artefaktı.
#' @return Tek ögeli mantıksal.
pk_attachment_is_served <- function(artifact) {
  is.list(artifact) &&
...
pk_compose_block <- function(decision, data, artifact = NULL, meta = list(),
  ...
  if (identical(decision$mode, "inline_table")) {
    ...
    ek_var <- pk_attachment_is_served(artifact)
