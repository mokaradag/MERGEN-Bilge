...
pk_async_config_install_env <- function(snapshot) {
  ...
  # SENTINEL = "bu anahtar KALDIRILDI": işçide de temizlenir.
  kaldirilacak <- names(yeni)[vapply(yeni, function(v) identical(v, .PK_ASYNC_OPTION_UNSET),
                                     logical(1))]
  yeni <- yeni[setdiff(names(yeni), kaldirilacak)]
  for (anahtar in kaldirilacak) Sys.unsetenv(anahtar)
  ...
  # Yalnızca KALDIRMA taşındıysa da bu bir ARIZA değildir: geri yükleyici
  # dönmelidir (aksi hâlde çağıran kapalı başarısız olup isteği reddederdi).
  if (!length(yeni) && !length(kaldirilacak)) return(NULL)

  if (length(yeni)) do.call(Sys.setenv, yeni)
