...
# Departman dışında ek kapsam ima ETMEYEN roller. `USER`, SSO tarafında
# varsayılan roldür (`helpers_sso.R` içinde `row$Yetki %||% "USER"`), bu yüzden
# tanınan roller kümesinden çıkarılamaz.
PK_RLS_DEPT_ONLY_ROLES <- c("USER")

# Planın TANIDIĞI tüm roller. Kapsam adımlarının `applicable` koşulları rol
# adına BİREBİR eşitlikle bakar; listede olmayan bir rol hiçbir adımı
# uygulanabilir yapmaz ve predikatsız plan çerçevenin TAMAMINI döndürür.
# Bilinmeyen rol bu yüzden KAPALI BAŞARISIZ olmak zorundadır (PR #705, P1).
PK_RLS_KNOWN_ROLES <- c("ADMIN", PK_RLS_DEPT_ONLY_ROLES, PK_RLS_SCOPED_ROLES)
...
pk_rls_plan <- function(user_info, rls_cols, actual_columns) {
  ...
  # TANINMAYAN ROL KAPALI BAŞARISIZ OLUR (PR #705 incelemesi, P1).
  #
  # Aşağıdaki kapsam adımları `identical(yetki, "PY")` /
  # `yetki %in% c("KY-P", "DIR-P")` gibi BİREBİR karşılaştırmalar kullanır.
  # Tanınmayan bir yetki kodu -- yazım hatası, yeni eklenmiş ama bu katmana
  # bağlanmamış bir rol veya dizin/veri eşitleme artığı -- proje ve EPS
  # adımlarını `not_applicable` bırakır. Departman kapsamı da
  # `not_applicable` beyan edilmişse plan HİÇ predikat üretmez ve
  # `apply_rls_to_data()` çerçevenin TAMAMINI döndürür. Rol doğrulaması bu
  # sessiz açık-başarısız yolu kapatır: eksik yetki bilgisi hiçbir zaman
  # DAHA GENİŞ veri erişimi anlamına gelemez.
  if (!(yetki %in% PK_RLS_KNOWN_ROLES)) {
    return(bos_plan(
      ok = FALSE, abort = TRUE, reason = "unknown_role",
      detail = "Kullanici yetki tipi (Yetki) taninmiyor; RLS plani uretilemez.",
      states = list(role = yetki)
    ))
  }
