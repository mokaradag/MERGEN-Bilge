...
validate_uploaded_file <- function(path,
  ...
  # Windows ad bileşeninin SONUNDAKİ boşluk ve noktaları YOK SAYAR: `rapor.txt.`
  # ve `rapor.txt ` adları `rapor.txt` dosyasına çözülür. Doğrulama bunlara
  # "başarılı" dediğinde indeks kaydı diskte olmayan bir adı işaret ediyor ya da
  # aynı klasördeki mevcut dosyanın üzerine yazılıyordu.
  # `R/utils_safe_path.R` aynı kararı zaten veriyor; iki denetim hizalanır.
  if (grepl("[ .]$", filename, perl = TRUE)) {
    return(list(
      ok = FALSE,
      error = "Dosya adı boşluk veya nokta ile bitemez.",
      code = "bad_filename"
    ))
  }
