...
.pk_cache_canonical_encode <- function(rls_info, alanlar) {
  ...
  bolumler <- vapply(alanlar, function(ad) {
    ...
    if (ad %in% .PK_CACHE_SET_FIELDS && length(ogeler)) {
      # `NA` ATILMAZ (PR #705 incelemesi, P3).
      #
      # Eleman sayisi asagidaki uzunluk onekini besledigi icin `NA` elenince
      # `c("P1", NA)` ile `c("P1")` AYNI imzaya dusuyordu: iki FARKLI yetki
      # kapsami tek onbellek girisini paylasabilirdi. `parca()` `NA` degerini
      # zaten `<na>` olarak kodlar (kume disi alanlarla AYNI davranis), bu
      # yuzden `NA` yalnizca siralamada sona alinir ve KORUNUR.
      ogeler <- sort(unique(ogeler), method = "radix", na.last = TRUE)
    }
    paste0(
      parca(ad),
