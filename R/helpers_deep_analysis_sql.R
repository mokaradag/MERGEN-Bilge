...
pk_deep_execute_sql <- function(conn, sql_text, deadline_at = NULL,
  ...
  if (!isTRUE(plan$dispatch)) {
    # ALAN KÜMESİ DİĞER ÇIKIŞLARLA AYNI OLMALIDIR: eksik `cached` alanı
    # tüketicide `if (sonuc$cached)` ifadesini "argument is of length zero" ile
    # düşürüyor, telemetri de mekanizmayı boş kaydediyordu.
    return(list(
