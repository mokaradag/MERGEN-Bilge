...
claude_code_config <- list(
  ...
  # Maksimum istek süresi (saniye); geçersiz/NA/KESİRLİ değer varsayılana düşer.
  # `as.integer("1.9")` sessizce `1` üretiyordu: yazım hatası içeren bir
  # `.Renviron` değeri normal istekleri BİR SANİYE sonra sonlandırabiliyordu.
  # Bu yüzden `as.numeric()` ile okunur ve TAM POZİTİF tamsayı olmayan her değer
  # varsayılana düşer (sessiz kırpma yok).
  timeout_seconds = local({
    v <- suppressWarnings(as.numeric(Sys.getenv("CLAUDE_CODE_TIMEOUT", "14400")))
  ...
  # İzin verilen maksimum eş zamanlı işlem sayısı; geçersiz/NA/KESİRLİ değer
  # varsayılana düşer (bkz. `timeout_seconds`: `as.integer("1.9")` sessizce tek
  # işçiye indiriyordu).
  max_concurrent = local({
    v <- suppressWarnings(as.numeric(Sys.getenv("CLAUDE_CODE_MAX_CONCURRENT", "5")))
    if (length(v) != 1L || is.na(v) || !is.finite(v) || v <= 0 || v != trunc(v)) {
      5L
    } else {
      as.integer(v)
...
claude_code_runtime_limits <- list(
  ...
  # ALT SINIR retention süresidir: `.cc_limit_num()` yalnızca sonlu ve negatif
  # olmayan değerleri denetlediği için `0`/`1` gibi bir değer kabul ediliyordu.
  # Retention'dan KÜÇÜK bir yetim eşiği, retention penceresini aşan uzun bir CLI
  # çalıştırmasının AKTİF lease'ini yetim sayıp çalışma alanını sildiriyordu.
  runtime_lease_orphan_sec = max(
    .cc_limit_num("CLAUDE_CODE_RUNTIME_LEASE_ORPHAN_SEC", 86400),
    .cc_limit_num("CLAUDE_CODE_RUNTIME_RETENTION_SEC", 21600)
  )
)
