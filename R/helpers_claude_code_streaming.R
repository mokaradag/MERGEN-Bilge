...
run_claude_code_streaming <- function(prompt,
  ...
  tryCatch({
    ...
    # KORUNAN araç olayları da ayrıştırma girdisine girer: kırpma yalnızca metin
    # deltalarını düşürmeli, `tool_use`/`tool_result` kayıtlarını düşürmemeli.
    birikmis_cikti <- function() {
      satirlar <- cc_stream_all_lines(satir_env)
    ...
    tum_satirlar <- cc_stream_all_lines(satir_env)
    ...
    if (identical(cikis_kodu, 0L)) {
      ...
      # Tam çıktıyı ayrıştır. Tampon KIRPILDIYSA delta metni eksiktir; son
      # `result` kaydındaki tam metin yetkilidir (aksi hâlde yalnızca kuyruk
      # metni dönüyordu).
      ayristirma <- parse_claude_code_json_output(
        tum_cikti,
        prefer_result_text = cc_stream_buffer_trimmed(satir_env)
      )
