...
.pk_result_v1 <- function(filtered_data, secure_data, query, user_prompt,
  ...
  if (nrow(secure_data) > nrow(filtered_data)) {
    data_str <- paste0(
      data_str,
      # `secure_data` YETKİLENDİRME SONRASI çerçevedir; yalnızca kullanıcı
      # filtresi `filtered_data` ile eklenir. Eski metin modele bu sayının RLS
      # ÖNCESİ olduğunu söylüyordu; model yetkili alt kümeyi veritabanı geneli
      # payda gibi sunabiliyordu (aynı yükteki filtreleme uyarısıyla çelişir).
      sprintf(paste0("\n\n(Kullanici yetkisi uygulandiktan SONRA, kullanici ",
                     "filtresinden ONCE toplam %d satir vardi)"),
              nrow(secure_data))
    )
  }
...
.pk_result_v2 <- function(filtered_data, secure_data, query, user_prompt,
  ...
  if (is.list(paket_sonucu)) {
    if (!isTRUE(paket_sonucu$ok)) {
      # `pk_async_bounded_fs()` HER hata için `ok = FALSE` döner; hepsini son
      # tarih saymak GERÇEK bir paket kurulum hatasını (ör. bozuk sütun
      # metadata'sı) "süre doldu" diye raporluyor, hata metnini atıyor ve hata
      # yolunu HİÇ çalıştırmıyordu. Gerçek halt (iptal/son tarih) ile iç hata
      # AYRILIR.
      durum_paket <- halt_durumu()
      butce <- if (exists("pk_deep_bounded_deadline_reason", mode = "function", inherits = TRUE)) {
        pk_deep_bounded_deadline_reason(paket_sonucu,
                                        getOption("mergen.pk.async.deadline_at", NULL))
      } else if (identical(paket_sonucu$error, "budget_exhausted")) {
        "deadline"
      } else {
        NULL
      }
      if (!is.null(durum_paket) || !is.null(butce)) {
        return(iptal_sonucu(durum_paket %||% butce %||% "deadline"))
      }
      stop(as.character(paket_sonucu$error %||% "Analiz paketi kurulamadi.")[1], call. = FALSE)
    }
  ...
  # EK SUNULMUYORSA ÖZET METNİ EK VAAT ETMEZ. `karar` bu noktadan SONRA
  # hesaplandığı için mod burada yeniden çözülür; `inline_table` kipinde
  # `pk_compose_block()` hiçbir artefakt üretmez.
  ek_var <- !identical(
    pk_compose_decide(nrow(filtered_data), ncol(filtered_data), user_prompt, meta)$mode,
    "inline_table"
  )
  yedek_metin <- pk_compose_facts_summary(paket$facts, attachment_available = ek_var)
  ...
  list(
    ...
    # YEDEK METİN GERÇEKTEN SUNULAN EKE GÖRE ÜRETİLİR.
    #
    # `ek_var` yalnızca KOMPOZİSYON kipinden tahmin ediliyordu; `pk_export_build()`
    # daha SONRA çalışır ve `refused` / `failed` / `empty` dönebilir (parça
    # tavanını aşan geniş sonuç). O durumlarda yedek metin hâlâ "kalan değerler
    # ek dosyanın Ozet sayfasında" diyordu; köken doğrulaması model düzyazısını
    # reddettiğinde kullanıcı VAR OLMAYAN bir eke yönlendiriliyordu.
    # SUNULAN EK ÖLÇÜTÜ `pk_compose_block()` İLE AYNIDIR: `csv_fallback` de
    # kullanıcıya indirilebilir dosya sunar (bkz. `pk_attachment_is_served()`).
    pk_fallback_text = local({
      ek_sunuldu <- pk_attachment_is_served(artefakt)
      if (identical(ek_var, ek_sunuldu)) {
        yedek_metin
      } else {
        pk_compose_facts_summary(paket$facts, attachment_available = ek_sunuldu)
      }
    }),
