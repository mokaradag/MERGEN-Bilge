...
sendMessageInit <- function(
  ...
  send_message <- function(prompt_text, is_summarization_request = FALSE) {
    ...
    request_callbacks <- mergen_build_send_message_request_callbacks(
      session = session,
      values = values,
      reset_chat_state_fn = reset_chat_state_fn,
    ...
    cleanup_send_message <- request_callbacks$cleanup
    abort_send_message <- request_callbacks$abort

    runtime_model <- mergen_prepare_send_message_model_runtime(
      input = input,
      settings_data = settings_data,
      current_settings = current_settings,
      tool_family = tool_family,
    ...
    current_settings <- runtime_model$current_settings
    model_selected <- runtime_model$model_selected
    excel_deep_on <- runtime_model$excel_deep_on
    excel_deep_level <- runtime_model$excel_deep_level
    coding_deep_on <- runtime_model$coding_deep_on
    ...
    run_llm_request_stage <- function(messages_to_process, analysis_max_tokens = NULL) {
      ...
      api_key_plan <- NULL
      {
        ...
        # Plan DONDURULAMADIYSA canlı çözüme DÜŞÜLMEZ: aynı istek iki farklı
        # yapılandırma durumundan derlenirdi.
        if (identical(as.character(api_key_plan$source %||% "")[1], "snapshot_failed")) {
          abort_send_message(message = paste0(
            "API anahtarı gönderim anında çözümlenemedi; istek durduruldu."
          ), type = "error")
          return(invisible(NULL))
        }
    ...
    if (identical(tool_family, "sql_analysis")) {
      ...
      # BOŞ ANLIK GÖRÜNTÜ KABUL EDİLMEZ. `mergen_pk_send_snapshot()` hem
      # `reactiveValuesToList()` hem `as.list()` başarısız olduğunda `list()`
      # döner; `list()` `NULL` OLMADIĞI için `%||%` onu korur ve `settings_data`
      # isteğin geri kalanı boyunca BOŞ kalırdı: derin düşünme, detay seviyesi
      # ve TTS gibi gönderim-anı ayarları sessizce varsayılana düşerdi.
      if (is.list(pk_dispatch_snapshot$settings) &&
          length(pk_dispatch_snapshot$settings) > 0L) {
        settings_data <- pk_dispatch_snapshot$settings
      }
      ...
      if (!identical(pk_outcome$action, "continue")) {
        ...
        if (identical(pk_outcome$action, "answer")) {
          ...
          pk_message_id <- add_message_fn(pk_outcome$answer, "ai")  # KİMLİK ALANI AŞAĞIDA ÇIKARILIR: `add_message_fn()` bir mesaj NESNESİ döndürür (`list(id=, db_id=, ...)`) ve nesnenin kendisi hiçbir DOM düğümüne karşılık gelmez; çıkarılmadığında çipler HER doğrudan-yanıt PK sonucunda DÜŞÜYORDU (`R/server_handler_pk_async.R` aynı tüketici için zaten `id` çıkarır).
          if (exists("mergen_pk_emit_chips", mode = "function", inherits = TRUE)) {
            try(mergen_pk_emit_chips(list(session = session, req_id = req_id),
                                     pk_outcome$chips,
                                     message_id = as.character((if (is.list(pk_message_id)) pk_message_id$id else pk_message_id) %||% "")[1]), silent = TRUE)
          }
        }
        return(invisible(NULL))
      }
