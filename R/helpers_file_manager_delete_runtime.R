...
fm_delete_persisted_file_artifacts <- function(
  ...
  # `delete_failed` ile `deleted_physical` AYNI ANDA TRUE olabilir: bir aday
  # (ör. `persisted_path`) silinemezken başka bir aday (`datapath`) silinebilir.
  # `&& !durum$deleted_physical` bu KISMİ başarısızlığı başarıya çeviriyor,
  # indeks kaydı düşüyor ve silinemeyen kalıcı dosya diskte KAYITSIZ kalıyordu.
  korunsun <- isTRUE(durum$delete_failed) || isTRUE(durum$belirsiz)
  ...
  # AD OLMADAN fiziksel silme başarılıysa indeks kaydı TEMİZLENEMEZ. Bunu
  # `delete_failed = FALSE` ile bildirmek, çağıranın yerel durumu kaldırmasına
  # ve BAYAT indeks kaydının kalmasına yol açıyordu.
  if (isTRUE(durum$deleted_physical) && (!nzchar(file_name) || is.null(uid))) {
    korunsun <- TRUE
    fm_debug("delete_index_skipped", "dosya adı yok; indeks kaydı temizlenemedi")
  }
