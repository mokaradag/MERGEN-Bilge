...
pk_entity_resolve <- function(phrase, candidates, aliases = NULL,
  ...
  if ((isTRUE(puanlar$scan_truncated) || isTRUE(puanlar$phrase_truncated)) &&
    ...
    # KIRPILMIŞ KISA LİSTE HİÇ ADAY TAŞIMAYABİLİR; `.pk_entity_below_threshold_decision()` ile AYNI kurtarma kümesi kullanılır ve eşiği geçmedikleri için "Tümü" kapalıdır.
    ...
    #
    # Bu dalda küme KANITLANMIŞ biçimde EKSİKTİR: kırpılan adaylar tam da "Tümü"nün kapsadığını iddia ettiği kümededir; kullanıcı görmediği adayları dışlayan bir alt kümeyi "Tümü" sanabilirdi.
    ...
    # `suggestions_only` ÇİPLERİN GERÇEK KAYNAĞINI İZLER: `aday_kumesi` ya GERÇEK puanlanmış eşleşmeleri ya da kurtarma kayıtlarını (`tier = "suggestion"`) taşır. Sabit `TRUE` gerçek eşleşmeleri "filtrelenemez öneri" ilan ediyordu; özne rolünde ise bayrak HİÇ verilmediği için kurtarma kayıtları filtrelenebilir sanılıyordu.
    yalnizca_oneri <- !length(eslesmeler)
    if (identical(entity_role, "refinement")) {
      return(.pk_entity_decision(
        ...
        suggestions_only = yalnizca_oneri
      ))
    }
    return(.pk_entity_decision(
      "clarify", rule = 2L,
      ...
      scored = puanlar, clarification = netlestirme, thresholds = esikler,
      plural = cogul, margin = fark, suggestions_only = yalnizca_oneri
    ))
  }
...
.pk_entity_below_threshold_decision <- function(phrase, candidates, eslesmeler,
                                                ...
                                                cogul, fark, query_meta = NULL) {
  # Katman eşiğini geçen aday YOKSA `eslesmeler` boştur; katman DIŞI en yakın komşular ayrıca hesaplanır. Bunlar FİLTRELEMEYE UYGUN DEĞİLDİR, yalnızca kurtarma seçenekleridir. Bayrak çip KAYNAĞINDAN türetilir: `eslesmeler` doluysa çipler gerçek puanlı eşleşmelerdir, "yalnızca öneri" değildir.
  yalnizca_oneri <- !length(eslesmeler)
  en_yakin <- if (!yalnizca_oneri) {
    eslesmeler
  } else {
    # `query_meta` İLETİLİR: ana puanlama yolu sorgu bazlı tarama tavanını, kurtarma yolu küresel tavanı kullanıyor ve aynı istek için iki yol ayrışıyordu.
    pk_entity_nearest_candidates(phrase, candidates, esikler$max_candidates,
                                 query_meta = query_meta)
  }
  ...
  if (identical(entity_role, "refinement")) {
    return(.pk_entity_decision(
      ...
      scored = puanlar, clarification = netlestirme, thresholds = esikler,
      plural = cogul, margin = fark, disclose = TRUE, suggestions_only = yalnizca_oneri
    ))
  }
