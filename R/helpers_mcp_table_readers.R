...
helpers_mcp_tools$safe_read_table_generic <- function(path, sheet = 1, n_max = Inf) {
  ...
  # ÜST SINIR `.Machine$integer.max`tır. Doğrulama üst sınır uygulamadığında
  # `n_max = 3e9` sonlu/negatif olmayan/tam sayı olduğu için geçiyor; sonraki
  # `as.integer()` çağrısı `NA_integer_` üretiyor (bastırılmamış coercion
  # uyarısıyla), `is.finite(NA_integer_)` FALSE olduğu için `fread()` `nrows = Inf`
  # ve `read.csv()` `nrows = -1L` alıyordu. İkisi de SINIRSIZ okumadır: büyük bir
  # CSV tamamen belleğe alınıp worker belleğini tüketiyordu.
  ...
  if (!isTRUE(gecerli)) {
    stop(
      paste0(
        "safe_read_table_generic: 'n_max' 0 ile .Machine$integer.max ",
        "arasında tam sayı veya Inf olmalı."
      ),
      call. = FALSE
    )
  }
