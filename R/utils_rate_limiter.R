...
check_rate_limit <- function(user_id) {
  ...
  # `all.names = TRUE`: `ls()` NOKTA ile başlayan adları atlıyordu. `.abc` gibi
  # geçerli bir kullanıcı kimliği kapasite denetiminde ve temizlikte hiç
  # görünmüyor, çok sayıda nokta önekli kimlik `rate_limiter$requests` ortamını
  # `max_users_cache` sınırının ÖTESİNE büyütebiliyordu.
  if (length(ls(envir = rate_limiter$requests, all.names = TRUE)) >= rate_limiter$max_users_cache) {
    ...
    if (!yakin_zamanda) {
      ...
      en_eski <- NULL
      for (anahtar in ls(envir = rate_limiter$requests, all.names = TRUE)) {
