...
.pk_v2_additive_columns <- function(query, numeric_columns) {
  ...
  # KAPALI BAŞARISIZ: metadata kararını verecek erişimci YOKSA hiçbir sütun
  # toplanabilir SAYILMAZ. Eskiden TÜM sayısal sütunlar dönüyordu; yani
  # metadata sözleşmesi tam da uygulanamadığı anda oran/yüzde/ID sütunları da
  # toplanıyor ve uydurma bir toplam üretiliyordu.
...
pk_apply_smart_filters_v2 <- function(data, filter_instructions, query = NULL) {
  ...
  if (nrow(data) == 0L) {
    ...
    bos_agg <- .pk_v2_agg_token(aggregation)

    if (identical(bos_agg, "count")) {
    ...
    # TOPLULAŞTIRMA ANAHTARI BOŞ SONUÇTA DA DOĞRULANIR.
    #
    # Eskiden `count` dışındaki HER değer (geçerli `sum`/`group_by` ya da
    # tamamen tanınmayan bir anahtar) sıfır sütunlu bir `data.frame()` ile
    # dönüyordu: istek ne doğrulanıyor ne de beklenen biçimi taşıyordu, üstelik
    # toplulaştırmasız yol da ORİJİNAL sütun şemasını kaybediyordu. Artık
    # tanınmayan anahtar dolu sonuçtaki ile AYNI biçimde REDDEDİLİR ve
    # toplulaştırmasız boş sonuç sütun şemasını KORUR.
    if (nzchar(bos_agg) && !(bos_agg %in% PK_FILTER_V2_AGGREGATIONS)) {
      bos_red <- bos_karar
      bos_red$action <- "refuse"
      bos_red$refusal_message <- paste0(
      ...
      return(sonuc_ekle(data[0, , drop = FALSE], 0L, bos_red))
    }

    return(sonuc_ekle(data[0, , drop = FALSE], 0L, bos_karar))
  }
  ...
  # BOŞ DİZİ DÜŞÜRME KAYDI ÜRETMEZ.
  #
  # LLM `filter_expression: []` döndürdüğünde jsonlite bunu `list()` /
  # `character(0)` yapar; `as.character(...)[1]` `NA_character_` üretir ve
  # `nzchar(NA)` R'de TRUE'dur. Sonuç: HİÇ ifade verilmemişken kullanıcıya
  # "çalıştırılabilir ifade düşürüldü" diye bir bozulma bildiriliyordu.
  ifade_dusurmesi <- list()
  ifade <- tryCatch(as.character(filter_instructions$filter_expression),
                    error = function(e) character(0))
  ifade <- ifade[!is.na(ifade) & nzchar(trimws(ifade))]
  if (length(ifade)) {
