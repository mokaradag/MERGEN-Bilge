...
register_session_cleanup_on_end <- function(session, extra_cleanup = list()) {
  ...
  # Tekilleştirme AYNI ÇAĞRIDAKİ yinelemeleri de kapsar: `extra_cleanup`
  # listesi aynı fonksiyonu iki kez taşıdığında sadece `onceki_defter` ile
  # karşılaştırmak yetmiyordu; ilk kayıtta iki kopya da deftere giriyor ve
  # oturum kapanışında aynı temizlik iki kez çalışıyordu.
  yeni_defter <- onceki_defter
  for (fn in yeni_fnler) {
    if (!any(vapply(yeni_defter, identical, logical(1), fn))) {
      yeni_defter[[length(yeni_defter) + 1L]] <- fn
    }
  }

  session$userData$mergen_session_cleanup_extra <- yeni_defter
