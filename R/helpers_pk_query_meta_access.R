...
pk_meta_capability_check <- function(query, requirements = list()) {
    ...
    for (cap in deger) {
      if (!is.null(istekler[[cap]]) && !identical(istekler[[cap]], unname(alanlar[[alan]]))) {
        gecersiz <- c(gecersiz, sprintf(
          "capability '%s' birden fazla rol altinda istendi.", cap
        ))
      } else {
  ...
  if (!length(istekler) && !length(gecersiz)) {
    return(list(
      status = PK_META_STATUS_OK,
      missing = character(0),
      columns = list(),
      role_mismatches = list(),
      invalid_requirements = character(0)
    ))
  }
  ...
  durum <- if (length(eksik) || length(rol_uyusmazliklari) || length(gecersiz)) {
  ...
  list(
    status = durum,
    missing = unique(c(eksik, names(rol_uyusmazliklari))),
    columns = sutunlar,
...
pk_meta_validate_actual_columns <- function(query, actual_columns) {
  ...
  gercek <- unique(gercek[!is.na(gercek) & nzchar(trimws(gercek))])
