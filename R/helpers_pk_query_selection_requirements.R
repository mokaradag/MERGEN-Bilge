...
pk_select_normalize_requirements <- function(raw) {
  ...
  if (length(raw) == 0L) {
    # ONARIM MESAJI ALTI ALAN KURALIYLA AYNI OLMALIDIR: eski metin "en az bir
    # alan" diyordu, model onu izleyip tek alan gönderiyor ve TEK onarım hakkı
    # boşa harcanıp istek reddediliyordu.
    return(list(ok = FALSE, value = bos, error = sprintf(paste0(
      "Geçiş B 'requirements' nesnesi boş olamaz; ALTI alanın tümü ",
      "gönderilmelidir (%s) ya da alan hiç gönderilmemelidir."
    ), paste(.PK_SELECT_REQ_FIELDS, collapse = ", "))))
  }
  ...
  # PARÇALI NESNE DE SÖZLEŞME İHLALİDİR: yukarıdaki muhafız yalnızca `{}`yi
  # reddediyordu. `{"measures":[]}` gibi TEK alanlı bir parçanın uzunluğu 1'dir;
  # normalleştirme `ok = TRUE` dönüyor ve yetenek kaydı denetimi HİÇ
  # çalışmıyordu. İstem ALTI alanın da gönderilmesini ister. TÜMÜ BEYAN EDİLMİŞ
  # ama BOŞ nesne SÖZLEŞMEYE UYGUNDUR (karar politikası `not_asserted` kabul eder).
