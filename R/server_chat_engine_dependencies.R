...
# =============================================================================
.pk_codex_same_connection_list <- function(left, right) {
...
if (exists("pk_deep_analysis_process", mode = "function", inherits = TRUE) &&
    !exists(".pk_codex_deep_core", inherits = FALSE)) {

  .pk_codex_deep_core <- get(
    "pk_deep_analysis_process", mode = "function", inherits = TRUE
  )
  .pk_codex_deep_core_env <- environment(.pk_codex_deep_core)
  .pk_codex_real_get_connection <- get(
    "get_connection", mode = "function", envir = .pk_codex_deep_core_env, inherits = TRUE
  )
  .pk_codex_real_release_connection <- get(
    "release_connection", mode = "function", envir = .pk_codex_deep_core_env, inherits = TRUE
  )
  .pk_codex_real_get_user_rls_info <- get(
    "get_user_rls_info", mode = "function", envir = .pk_codex_deep_core_env, inherits = TRUE
  )
  .pk_codex_real_pk_analysis_observe <- get(
    "pk_analysis_observe", mode = "function", envir = .pk_codex_deep_core_env, inherits = TRUE
  )
  ...
  pk_deep_analysis_process <- function(user_prompt, chat_history, session,
    ...
    call_env$pk_analysis_observe <- function(session, conn, info) {
      ...
      if (!is.null(telemetry_list)) {
        on.exit(
          try(.pk_codex_real_release_connection(telemetry_list), silent = TRUE),
          add = TRUE
        )
      }
      .pk_codex_real_pk_analysis_observe(session, telemetry_conn, info)
    }

    core <- .pk_codex_deep_core
...
.pk_codex_room_footer_append <- function(text, footer) {
  footer <- .pk_codex_scalar_text(footer)
  if (!nzchar(footer)) return(text)

  base <- .pk_codex_scalar_text(text)
  if (grepl("**Analiz Kaynağı", base, fixed = TRUE)) return(text)
  paste0(base, footer)
}

.pk_codex_room_footer_publish <- function(footer) {
  footer <- .pk_codex_scalar_text(footer)
...
if (exists("oo_arac_sql_baglami_kur", mode = "function", inherits = TRUE) &&
    !exists(".pk_codex_room_sql_core", inherits = FALSE)) {

  .pk_codex_room_sql_core <- get(
    "oo_arac_sql_baglami_kur", mode = "function", inherits = TRUE
  )

  oo_arac_sql_baglami_kur <- function(soru, gecmis, oda_session, arac_meta) {
    result <- .pk_codex_room_sql_core(soru, gecmis, oda_session, arac_meta)
    ...
    if (is.list(result) && nzchar(footer)) {
      ...
      .pk_codex_room_footer_publish(footer)
    }
    result
  }
}
...
if (exists("ortakOturumYzBind", mode = "function", inherits = TRUE) &&
    !exists(".pk_codex_room_bind_core", inherits = FALSE)) {

  .pk_codex_room_bind_core <- get(
    "ortakOturumYzBind", mode = "function", inherits = TRUE
  )

  ortakOturumYzBind <- function(input, output, session, ctx, motor) {
    result <- .pk_codex_room_bind_core(input, output, session, ctx, motor)
    ...
    if (is.function(original_llm)) {
      motor$llm_uret <- function(oturum_id, soru_id, soran_id, istek_id,
        ...
        .pk_room_provenance_key <- .pk_codex_scalar_text(istek_id)
        original_llm(
          oturum_id = oturum_id,
          soru_id = soru_id,
          soran_id = soran_id,
    ...
    if (is.function(original_finish)) {
      motor$tamamla <- function(oturum_id, soru_id, istek_id,
                                ...
                                persona_id = NULL) {
        key <- .pk_codex_scalar_text(istek_id)
        ...
        if (!is.null(yanit_metni)) {
          yanit_metni <- .pk_codex_room_footer_append(yanit_metni, footer)
        }
        if (!is.null(hata_metni)) {
          hata_metni <- .pk_codex_room_footer_append(hata_metni, footer)
        }
