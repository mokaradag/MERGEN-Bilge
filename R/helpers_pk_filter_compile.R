...
pk_filter_normalize_leaf <- function(f) {
  ...
  # TANINMAYAN BİRLEŞTİRME BELİRTECİ SESSİZCE `and` OLMAZ.
  #
  # `.pk_filter_group_operator()` bilinmeyen belirteci `NA` sayıp grubu AÇIK bir
  # bildirimle reddeder; bu dal ise `PK_FILTER_OR_TOKENS` dışındaki HER şeyi
  # `and`e eşliyordu. Model `logic = "veyaa"` yazdığında aynı sütundaki iki
  # `contains` yaprağı KESİŞİM üretiyor (çoğu zaman sıfır satır), hiçbir yaprak
  # düşürülmediği için kullanıcıya HİÇBİR bildirim gitmiyordu.
  birlesim_ham <- birlesim
  bilinen_birlesim <- !nzchar(birlesim) ||
    birlesim %in% PK_FILTER_OR_TOKENS || birlesim %in% PK_FILTER_AND_TOKENS
  ...
  list(
    ...
    values = degerler,
    logic = birlesim,
    ok = nzchar(sutun) && length(degerler) > 0L && bilinen_islem && bilinen_birlesim,
    ...
    } else if (!bilinen_birlesim) {
      sprintf("desteklenmeyen filtre birlestirmesi: '%s'", birlesim_ham)
    } else {
      NA_character_
    }
  )
...
.pk_filter_mask_numeric <- function(col_vals, leaf) {
  ...
  if (inherits(col_vals, "integer64")) {
    # BİÇİM KAPISI DÖNÜŞÜMDEN ÖNCE (PR #705 incelemesi, P2).
    #
    # `bit64::as.integer64()` SAYISAL girdiyi sessizce KIRPAR (`1.5` -> `1`);
    # `anyNA()` bunu yakalayamaz ve yaprak DÜŞMEK yerine YANLIŞ satırları
    # seçerdi. `pk_filter_numeric_operand()` aynı riski `^[+-]?[0-9]+$`
    # kapısıyla zaten kapatıyor. Sayısal girdi bilimsel gösterime düşürülmez
    # (`format(1e15)` -> "1e+15" tam sayı kapısını geçemezdi).
    ham_deger <- if (is.character(leaf$values)) {
      trimws(leaf$values)
    } else {
      trimws(format(leaf$values, scientific = FALSE, trim = TRUE))
    }
    if (anyNA(ham_deger) || !all(nzchar(ham_deger)) ||
        !all(grepl("^[+-]?[0-9]+$", ham_deger))) {
      return(NULL)
    }
...
pk_filter_leaf_mask <- function(data, leaf, query = NULL) {
  ...
  if (!is.null(query) && exists("pk_meta_is_filterable", mode = "function", inherits = TRUE)) {
    # KAPALI BAŞARISIZ: metadata kararı DEĞERLENDİRİLEMİYORSA yaprak engellenir.
    # Eski `FALSE` yedeği, filtrelenebilirlik denetimi hata verdiğinde filtreyi
    # yine de uyguluyor ve metadata sözleşmesini tam da uygulanamadığı anda
    # atlıyordu.
    kapali <- tryCatch(.pk_filter_meta_blocks(query, leaf$column), error = function(e) TRUE)
    if (isTRUE(kapali)) {
      return(list(
        ok = FALSE, mask = NULL,
