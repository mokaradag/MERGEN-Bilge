...
.PK_ASYNC_DB_POOL_ENV <- c(
  ...
  "DB_DSN"  # DSN DE TASINIR VE PARMAK IZINE GIRER: `init_db_pool_once()` havuzu bu DSN ile kurar; tasinmazsa SICAK bir isci, operator ana surecte `DB_DSN` degistirdikten sonra bile ONCEKI veritabanina baglanan havuzu KORUR.
)
...
pk_async_worker_pool_fingerprint <- function(workers = NULL) {
  ...
  paste(c(
    ...
    oku("MERGEN_DB_POOL_IDLE_TIMEOUT"), oku("DB_DSN"),
...
pk_async_worker_pool_apply_share <- function(admission) {
  ...
  function() {
    ...
    # ISARET TAVANLA BIRLIKTE GERI ALINIR: aksi halde isaret "pay uygulandi" derken tavan TAM degere donuyor, `pk_db_pool_process_share()` yeniden bolusturme YAPMIYOR ve pay penceresi disinda kurulan bir havuz TOPLAM oturum tavanini asabiliyordu.
    if (is.na(eski_isaret)) Sys.unsetenv("MERGEN_DB_POOL_SHARE_APPLIED")
    else Sys.setenv(MERGEN_DB_POOL_SHARE_APPLIED = eski_isaret)
    invisible(TRUE)
  }
}
...
.pk_async_worker_db_pool_init <- function(hedef, workers = NULL) {
  ...
  sonuc <- tryCatch(inherits(baslat("primary"), "Pool"), error = function(e) {
