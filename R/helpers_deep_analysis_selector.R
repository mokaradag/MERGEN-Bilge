...
find_multiple_queries_with_ai <- function(user_prompt, library, session,
  ...
  # Günlüğe giden hata metni merkezi redaksiyondan geçer; yardımcı yoksa metin
  # AYNEN kullanılır (izole test bağlamı).
  .deep_guvenli_log <- function(x) {
    metin <- suppressWarnings(as.character(x)[1])
    if (length(metin) != 1L || is.na(metin)) metin <- ""
    if (exists("pk_safe_log_text", mode = "function", inherits = TRUE)) {
      return(pk_safe_log_text(metin))
    }
    metin
  }
  ...
  tryCatch({
    ...
    result <- tryCatch({
      ...
      # HATA METNİ REDAKSİYONDAN GEÇER: sürücü metni, uç nokta ya da bağlantı
      # dizesi taşıyabilir. Tekil seçici (`helpers_pk_analysis_ai_selector.R`)
      # aynı sınırda `pk_safe_log_text()` kullanır.
      cat(sprintf("[DEEP_ANALYSIS] AI çoklu seçim zaman aşımı/hata: %s\n",
                  .deep_guvenli_log(conditionMessage(e))))
      NULL
    }, finally = {
    ...
    for (m in parsed$matches) {
      ...
      #
      # KAYDIN KENDİSİ DE LİSTE OLMALIDIR. `simplifyVector = FALSE` ile
      # `{"matches": [1, 2]}` sayısal SKALER'lerden oluşan bir liste üretir;
      # `m$match_id` o zaman "$ operator is invalid for atomic vectors" ile
      # hata verir ve yine TÜM seçim kaybolurdu.
      if (!is.list(m)) next
      # `match_id` DE ATOMİK OLMALIDIR: iç içe liste `as.integer()` içinde
      # "(list) object cannot be coerced" hatası verirdi.
      if (!is.atomic(m$match_id)) next
      ...
      if (length(idx) == 1L && !is.na(idx) && idx > 0 && idx <= length(library)) {
        ...
        # LİSTE DEĞERLER ÖNCE ELENİR: `simplifyVector = FALSE` ile
        # `{"confidence": [90]}` bir liste döndürür ve `as.numeric()` iç içe
        # listede HATA fırlatarak GEÇERLİ tüm eşleşmeleri düşürürdü.
        if (!is.null(m$confidence) && !is.atomic(m$confidence)) next
        ...
        if (confidence >= 30) {
          ...
          # `reason` da SKALER metne indirgenir; liste kalırsa aşağı akıştaki
          # `sprintf()`/`paste()` tüketicileri liste alırdı.
          q$selection_reason <- skaler(m$reason, "")
          q$.matched_idx <- idx
