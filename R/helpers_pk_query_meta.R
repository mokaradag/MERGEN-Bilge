...
pk_meta_apply_alias_overlay <- function(meta, overlay) {
  hatalar <- character(0)
  # `meta` LİSTE OLMAK ZORUNDA: atomik bir değer geldiğinde `meta[[id]]`
  # "subscript out of bounds" fırlatır ve boot, bu fonksiyonun üretmesi gereken
  ...
  for (id in adlar) {
    ...
    mevcut_giris <- .pk_meta_named_entry(meta, id)  # `[[` KARAKTER ALT SİMGESİ BOŞ/ADSIZ listede `NULL` DÖNDÜRMEZ, "subscript out of bounds" FIRLATIR; birleşik metadata boşken boot, açıklayıcı bulgu yerine OPAK bir hatayla düşerdi.
    if (is.null(mevcut_giris)) {
      hatalar <- c(hatalar, sprintf(
...
.pk_meta_validate_capability_uniqueness <- function(query_id, meta) {
    ...
    if (!is.list(varyantlar)) {
      ...
      if (is.null(adlar) || length(adlar) != length(varyantlar) ||
        ...
        if (length(tekrar_varyant)) {
          ...
          # ADLAR CAPABILITY ANAHTARLARIYLA AYNI BİÇİMDE KIRPILIR.
          #
          # `eslesme` anahtarları `trimws(cmeta$capability)` ile üretilir;
          # varyant adları HAM bırakılınca `" labor.remaining_hours "` gibi
          # geçerli bir küratör beyanı hem "tanimsiz capability" hem de
          # "birden fazla sahip" bulgusu üretip açılışı DÜŞÜRÜYORDU.
          names(varyantlar) <- trimws(adlar)
        }
      }
    }
  } else {
  ...
  for (cap in names(eslesme)) {
    ...
    varyant <- .pk_meta_named_entry(varyantlar, cap)
...
pk_meta_validate_schema_dependent <- function(query_id, meta, schema, rls_columns = NULL) {
  ...
  # TEKRAR DENETİMİ DE KIRPILMIŞ ADLAR ÜZERİNDEDİR: ham hâlde farklı görünen
  # iki ad sonra TEK girdiye çöküyor, ikinci beyan HİÇ denetlenmiyordu.
  .sema_kirpik <- trimws(sema_ham[!is.na(sema_ham)])
  sema_tekrar <- unique(.sema_kirpik[duplicated(.sema_kirpik)])
...
.pk_meta_validate_schema_roles <- function(query_id, meta, schema_types, sema_sutunlari) {
  ...
  # LİSTE OLMAYAN `column_meta` SESSİZCE ATLANMAZ.
  #
  # Atomik bir `column_meta` geldiğinde `names()` NULL, `trimws(NULL)` ise
  # `character(0)` olur; ad ataması adları `NA` yapar ve aşağıdaki
  # `intersect()` HİÇBİR sütun döndürmez. Sonuç: `role='date'` / `role='measure'`
  # yapısal denetimleri O sorgu için HİÇ çalışmaz ama açılış şemayı
  # "doğrulandı" diye raporlardı. Liste değilse boş listeye indirgenir; şekil
...
pk_query_meta_attach <- function(query_library,
  ...
  for (i in seq_along(kutuphane_safe)) {
    ...
    id <- trimws(q_item$id)
    meta <- .pk_meta_named_entry(birlesik, id) %||% list()
