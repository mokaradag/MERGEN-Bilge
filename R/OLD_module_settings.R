			...
			div(
			  ...
			  fluidRow(
				...
				column(
				  ...
				  p("Excel dosyalarıyla gelişmiş analiz için MCP tabanlı araçları etkinleştirir.", class = "setting-description", style = "margin-top:4px;"),
				  div(
					class = "setting-item",
					div(
					  class = "checkbox-item",
  ...
  moduleServer(id, function(input, output, session) {
    ...
    observeEvent(input$loaded_settings, {
      ...
      if (!is.null(loaded$enable_mcp_tools)) {
        settings$enable_mcp_tools <- loaded$enable_mcp_tools
        updateCheckboxInput(session, "enable_mcp_tools", value = loaded$enable_mcp_tools)
      }
    ...
    observeEvent(input$enable_mcp_tools,        { settings$enable_mcp_tools        <- input$enable_mcp_tools })
