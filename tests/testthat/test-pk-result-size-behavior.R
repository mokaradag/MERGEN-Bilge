...
local({
  ...
  # SIZINTI TEMIZLENIR: `local = globalenv()` bu yardımcıları süit boyunca
  # KALICI olarak global ortama kurar; sonraki dosyalar
  # `exists(..., inherits = TRUE)` ile yoklayıp kendi fail-closed dallarını
  # ATLAYABILIR ve dosya tek başına geçerken paket koşumunda FARKLI davranır.
  # Kardeş dosyalar (test-pk-sql-execute-bounded-behavior.R,
  # test-pk-runtime-hardening-behavior.R) aynı korumayı uygular.
  #
  # ANLIK GÖRÜNTÜ `%||%` ATAMASINDAN ÖNCE ALINIR: sonra alınırsa operatör
  # `eklenen` kümesine GİRMEZ, `withr::defer()` onu hiç kaldırmaz ve süitin
  # geri kalanında `globalenv()` içinde kalır.
...
test_that("seçilen sorgu analiz son tarihini override edebilir", {
  ...
  # `MERGEN_PK_ANALYSIS_DEADLINE_SEC` verirse `pk_set_exec_context()` MUTLAK
  # son tarihi o değerle yeniden hesaplar ve varsayılan-yol iddiası üretim
  # doğruyken kırılır.
  eski_env <- Sys.getenv("MERGEN_PK_ANALYSIS_DEADLINE_SEC", unset = NA_character_)
  Sys.unsetenv("MERGEN_PK_ANALYSIS_DEADLINE_SEC")
  on.exit({
    if (!is.na(eski_env)) Sys.setenv(MERGEN_PK_ANALYSIS_DEADLINE_SEC = eski_env)
  }, add = TRUE)
