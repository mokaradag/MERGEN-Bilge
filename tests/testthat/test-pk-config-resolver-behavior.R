...
local({
  ...
  # SIZINTI TEMİZLENİR (PR #705 incelemesi, P3): `local = globalenv()` her
  # `pk_config_*` yardımcısını KALICI olarak kurar. `exists("pk_config_resolve",
  # inherits = TRUE)` bakan sonraki bir dosya kendi kapalı-başarısız dalını
  # ATLAR ve paket dosya SIRASINA bağlı davranır.
  # `tests/testthat/test-pk-cache-behavior.R` aynı deseni zaten uyguluyor.
...
test_that("bozuk eşik beyanı YOK değil GEÇERSİZ raporlanır", {
  ...
  #
  # ORTAM DEGISKENI ACIKCA KALDIRILIR. Test onu HIC denetlemiyordu: makinede
