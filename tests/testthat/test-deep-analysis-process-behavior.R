...
.deepProcessEnv <- function() {
  ...
  for (dosya in c("helpers_pk_config.R", "helpers_pk_async_cancel.R",
                  ...
                  # `pk_deep_analysis_process()` Durdur yolunda
                  # `pk_rls_halt_message()` çağırır; sahibi bu dosyadır ve
...
test_that("beklenmeyen istisna mesaji yardimci yuklu olsa da OLMASA da AYNIDIR", {
  ...
  mesaji_al <- function(yardimci_yuklu) {
    ...
    if (isTRUE(yardimci_yuklu)) {
      ...
      # YOKLUK KANITLANIR, VARSAYILMAZ (PR incelemesi).
      #
      # `.deepProcessEnv()` ortamı `parent = globalenv()` ile kurar ve üretim
      # kodu yardımcıları `exists(..., inherits = TRUE)` ile çözer. Aynı
      # oturumdaki BAŞKA bir test dosyası bu yardımcıları `globalenv()` içine
      # yüklemişse "yardımcısız" dal da onları GÖRÜR, iki çağrı AYNI yolu koşar
      # ve `expect_identical()` ÖNEMSİZ biçimde geçerdi. Ön koşul kanıtlanamazsa
      # test SESSİZCE geçmek yerine ATLANIR.
      # YALNIZCA DALLARI AYIRAN AD DENETLENİR. `redact_connection_identifiers`
      # `.deepProcessEnv()` içinde HER İKİ dalda da `env`'e yüklenir ve yerel
  ...
  # MESAJIN VARLIGI ONCE IDDIA EDILIR (PR #705 incelemesi, P3).
  #
  # `error_msg` tamamen KAYBOLURSA `mesaji_al()` `NULL` doner:
  # `expect_identical(NULL, NULL)` gecer ve asagidaki `c(yardimcisiz,
  # yardimcili)` BOS vektore indigi icin sizinti iddialarinin govdesi HIC
  # calismazdi. Sozlesme sessizce hicbir sey dogrulamiyordu.
  for (mesaj in list(yardimcisiz, yardimcili)) {
    expect_true(is.character(mesaj) && length(mesaj) == 1L && !is.na(mesaj))
  }
