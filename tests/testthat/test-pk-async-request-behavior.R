...
test_that("globals paketi KÜÇÜK kalır ve memoize edilir", {
  ...
  # GİRİŞ NOKTASI PAKETTE OLMALIDIR: `pk_async_worker_globals()` `NULL`
  # girdileri DÜŞÜRÜR, bu yüzden çözülemeyen bir giriş noktası hatasız kaybolur.
  # Temiz bir PSOCK işçisi o zaman eksik fonksiyonla düşer ve HER istek
  # senkron yola geri döner — bu test ise yeşil kalırdı.
  expect_true("pk_async_run_analysis" %in% names(ilk))
  expect_true(is.function(ilk[["pk_async_run_analysis"]]))
