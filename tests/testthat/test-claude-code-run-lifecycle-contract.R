...
test_that("normal streaming finalization request id ile korunur", {
  ...
  # GÖNDERİM ANINDA YAKALANAN kimlik kullanılır: `ctx$env$request_id` geri
  # çağrıda okunduğunda YENİ bir çalıştırma alanı değiştirebiliyor ve eski geri
  # çağrı yeni isteğin kimliğini test edip aktif-run denetimini geçiyordu.
  expect_true(
    grepl("gonderilen_request_id <- ctx\\$env\\$request_id", dispatch_txt, perl = TRUE),
    info = "Çıktı işleme gönderim anındaki request id'yi yakalamalıdır."
  )
  expect_true(
    grepl("cc_is_active_run\\(ctx\\$rv, gonderilen_request_id\\)", dispatch_txt, perl = TRUE),
  ...
  expect_false(
    ...
    info = "Geri çağrılar canlı `ctx$env$request_id` yerine yakalanan kimliği okumalıdır."
  )
})
