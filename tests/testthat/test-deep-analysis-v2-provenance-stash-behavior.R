...
.deep_v2_fact <- function(id, value = 60) {
  list(
    ...
    decimals = 1L,
    # TÜRKÇE ONDALIK AYIRICI: üretim gösterimi virgül kullanır. Nokta ile
    # kurulan fikstür, virgüllü değer eşleştirmesini HİÇ sınamıyordu.
    display = sprintf("%%%s", sub(".", ",", sprintf("%0.1f", value), fixed = TRUE)),
    label = "İlerleme"
  )
}
...
test_that("DOĞRU virgüllü değer alıntılandığında yanıt ENGELLENMEZ", {
  ...
  fact_id <- "progress.weighted.weighted_mean.overall.abc123"
  fact <- .deep_v2_fact(fact_id)
  expect_identical(fact$display, "%60,0")

  session <- list(userData = new.env(parent = emptyenv()))
  env$pk_provenance_clear(session, request_id = "req-accept")
  observers <- env$pk_deep_observation_helpers(
    session = session,
    conn = NULL,
    username = "sentetik",
  ...
  # Model metni KORUNUR; deterministik yedek metne DÜŞÜLMEZ.
  expect_false(grepl("Yanıt doğrulanamadı", decorated, fixed = TRUE))
  expect_true(grepl("Ağırlıklı ilerleme %60,0", decorated, fixed = TRUE))
  expect_false(grepl("**Hesaplanan değerler**", decorated, fixed = TRUE))
  expect_true(grepl("Analiz Kaynağı (Derin Analiz)", decorated, fixed = TRUE))
})
