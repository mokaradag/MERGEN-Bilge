...
local({
  ...
  # SÜREÇ-YEREL AYNA sahip dosyadan ÖNCE ve AYNI ortama yüklenir: işaret
  # yardımcıları `globalenv()` içinde tanımlanır ve `.pk_request_marker_store`
  # sembolünü oradan çözer.
  source(file.path(repo_root, "R", "helpers_pk_async_marker_store.R"),
         encoding = "UTF-8", local = globalenv())
