...
test_that("TEK ortak jeton `not_for` dışlaması için YETMEZ", {
  # OLUMSUZ İFADE BÜTÜN OLARAK EŞLEŞMELİDİR. `all()` -> `any()` gerilemesi bu
  # kohortta HİÇ yakalanmıyordu: `not_for = "planlanan butce"` ifadesi
  # "planlanan iscilik" isteğini de reddederdi ve suite yine yeşil kalırdı.
  lib <- .pk_ret_lib()
  lib[[2]]$meta$not_for <- "planlanan butce"

  # İstem yalnızca `planlanan` jetonunu paylaşır; `butce` YOKTUR.
  dislanan <- pk_retrieval_excluded_ids(lib, "planlanan iscilik listesi")
  expect_false("q002" %in% dislanan)

  # Her iki jeton da varsa dışlama UYGULANIR.
  expect_true("q002" %in% pk_retrieval_excluded_ids(lib, "planlanan butce ozeti"))

  # Tek sözcüklük ifade ESKİSİ GİBİ davranır.
  lib[[2]]$meta$not_for <- "butce"
  expect_true("q002" %in% pk_retrieval_excluded_ids(lib, "sentetik butce ozeti"))
})
