...
test_that("alias indeksi katlanmış anahtarı kanonik hedefe eşler", {
  # `pk_entity_alias_index()` ADLANDIRILMIŞ kayıtlar için `pk_tr_fold()` çağırır
  # ve o yardımcı `stringi` YOKSA BİLEREK hata verir (sessiz/yanlış katlama
  # yasak). Paket yoksa bu test HATA değil ATLAMA üretmelidir.
