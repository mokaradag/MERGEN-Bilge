...
.read_repo_text_deep_split <- function(rel_path) {
  ...
  # BOŞ/OKUNAMAYAN DOSYA VACUOUS GEÇİRİR: `""` döndüğünde bu dosyadaki tüm
  # `expect_false(grepl(...))` iddiaları ÖNEMSİZ biçimde geçer ve yapısal
  # bölme sözleşmesi korumayı bırakır. Kısmi checkout ya da bozuk birleştirme
  # bu yolla "sözleşme sağlanıyor" diye raporlanırdı.
  size <- suppressWarnings(file.info(abs_path)$size[1])
  if (is.na(size) || size <= 0) {
    stop(sprintf("Kaynak dosya BOŞ ya da okunamıyor: %s", rel_path), call. = FALSE)
  }
