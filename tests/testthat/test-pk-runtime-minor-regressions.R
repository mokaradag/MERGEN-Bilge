...
test_that("her red yolu maskeyi kapatır", {
  ...
  # BİRİNCİL VARLIK GERÇEKTEN BİLİNMEZ OLMALIDIR.
  #
  # TEK grup sütunuyla `pk_filter_primary_column(NULL, "ProjeAdi")` tek-sütun
  # yedeğinden `"ProjeAdi"` döner; red o zaman BİRİNCİL SIFIR EŞLEŞME dalından
  # gelir ve beyan edilen `is.null(birincil)` dalı HİÇ çalışmazdı. İkinci bir
  # grup sütunu eklenerek birincil GERÇEKTEN çözümsüz bırakılır.
  derlenmis <- list(
    ok = TRUE, all_dropped = FALSE, mask = c(TRUE, TRUE), dropped = list(),
    groups = list(
      list(column = "ProjeAdi", zero_match = TRUE,
           applied = list(list(values = "YOKPROJE"))),
      list(column = "Yil", zero_match = FALSE,
           applied = list(list(values = "2024")))
    )
  )
