...
test_that("dort capraz-motor madde v1'de de ETKINDIR", {
  ...
  for (dosya in c("R/helpers_pk_rls.R", "R/helpers_pk_sql_readonly.R", "R/helpers_pk_sql_local_temp_batch.R",
                  "R/helpers_pk_safe_errors.R")) {
    metin <- oku(dosya)
    # EKSİK DOSYA OLUMSUZ İDDİALARI KENDİLİĞİNDEN GEÇİRİR (boş metinde
    # `grepl()` her zaman FALSE); önce OKUNABİLİRLİK doğrulanır.
    expect_true(nzchar(metin), info = sprintf("%s okunamadı.", dosya))
...
test_that("motor kipi istek basina TEK KEZ cozulur (karisik hat yasagi)", {
  repo_root <- resolve_repo_root_for_tests()
  full <- file.path(repo_root, "R", "module_proje_kaynak_analizi.R")
  size <- suppressWarnings(file.info(full)$size[1])
  expect_false(is.na(size), info = "Modul dosyasi okunamadi.")
  con <- file(full, open = "rb")
  on.exit(close(con), add = TRUE)
  raw_data <- readBin(con, what = "raw", n = size)
  metin <- enc2utf8(suppressWarnings(
  ...
  # Kuresel `v1` + metadata `engine = "v2"` bir sorguyu v1 seciciyle secip v2
  # kapilari/filtreleri/telemetrisiyle calistiriyordu (karisik hat).
  expect_false(
    grepl("pk_engine_is_v2(selected_query$meta)", kod, fixed = TRUE, useBytes = TRUE),
    info = "Asagi akis motor kipini sorgu metadatasindan YENIDEN cozmemelidir."
  )

  # Kip SECIM sonucundan gelir; v1 yollari da damgalanir.
  expect_true(
    grepl("pk_engine_kip <- if (!is.null(selected_query$pk_engine_mode))", kod,
          fixed = TRUE, useBytes = TRUE),
    info = "Kip once secim damgasindan okunmalidir."
  )
  expect_true(
    grepl("pk_engine_v2 <- identical(pk_engine_kip, \"v2\")", kod,
          fixed = TRUE, useBytes = TRUE),
    info = "`pk_engine_v2` tek cozumlemeden turemelidir."
  )
  expect_equal(
    length(gregexpr("$pk_engine_mode <- \"v1\"", kod, fixed = TRUE)[[1]]),
    2L,
    info = "Her iki v1 secim yolu da kipi damgalamalidir."
  )

  # Korumasiz `pk_engine_mode()` cagrisi KALMAMALIDIR (izole/isci baglami).
  expect_false(
    grepl("engine = pk_engine_mode()", kod, fixed = TRUE, useBytes = TRUE),
    info = "`pk_engine_mode()` korumasiz cagrilmamalidir."
  )
})
...
test_that("Sayisal koken dogrulamasi olgu SAKLANMAMISSA hic calismaz (v1 yolu)", {
  ...
  # ÜRETİMLE AYNI: her istek başında `pk_provenance_clear()` aktif istek
  # kimliğini yazar. Kimliksiz kayıt artık YAZILMAZ (sahiplik denetimi
  # uygulanamayacağı için bayat/yanlış yanıt kaydı tüketebiliyordu).
...
test_that("Olgu saklandiginda isaretler silinir ve blok alt bilginin ONUNE gelir", {
  ...
  env$pk_provenance_stash(
    oturum,
    paste0("\n\n**Önizleme**\n\n| a |\n", "\n\n---\n**Analiz Kaynağı**\n- Sorgu: q\n"),
    facts = olgular, query_id = "q_sentetik", request_id = "r1"
  )
