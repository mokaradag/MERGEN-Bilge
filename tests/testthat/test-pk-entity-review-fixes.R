...
test_that("en yakin aday onerileri TARAMA TAVANINA uyar", {
  ...
  withr::with_envvar(list(MERGEN_PK_RESOLVE_MAX_SCAN_CANDIDATES = "50000"), {
    ...
    yakin <- env$pk_entity_nearest_candidates("SENTETIK PROJE 00042X", adaylar, 3L)
...
test_that("devralinan varlik baglami SOHBET SINIRINDA temizlenir", {
  ...
  # `%||%` TEST ÇERÇEVESİNDE TANIMLI DEĞİLDİR: bu dosya operatörü yalnızca
  # `env` içine kurar. Küresel sızıntıya bağlanmak paketi SIRA BAĞIMLI yapardı
  # (PR #705 incelemesi, P2).
  .nesil <- oturum$userData[["pk_unsaved_chat_epoch"]]
...
test_that("devralinan varlik baglami isciye DUZ VERI olarak tasinir", {
  ...
  env <- new.env(parent = globalenv())
  env$`%||%` <- function(a, b) if (is.null(a)) b else a
  for (f in c("helpers_pk_async_snapshot_validate.R", "helpers_pk_async_snapshot.R", "helpers_pk_async_secrets.R")) {
