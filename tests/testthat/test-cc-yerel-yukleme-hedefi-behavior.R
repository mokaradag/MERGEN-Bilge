...
testthat::test_that("yükleme gözlemcisi hedefi doğrulama yardımcısından alır", {
  ...
  testthat::expect_true(grepl("calisma_alani, goreceli,", metin, fixed = TRUE))
  testthat::expect_true(grepl("listeleme_onbellegi = listeleme_onbellegi", metin, fixed = TRUE))
  testthat::expect_false(grepl("hedef <- file.path(calisma_alani, goreceli)", metin, fixed = TRUE))
})

testthat::test_that("listeleme önbelleği aynı üst dizini tekrar listelemez", {
  kok <- withr::local_tempdir()
  dir.create(file.path(kok, "alt"), recursive = TRUE)

  sayac <- new.env(parent = emptyenv())
  sayac$n <- 0L
  onbellek <- new.env(parent = emptyenv())

  env <- .cc_yy_env()
  # Gerçek `list.files()` sayılır; önbellek ikinci çağrıda onu atlamalıdır.
  gercek <- base::list.files
  env$list.files <- function(...) {
    sayac$n <- sayac$n + 1L
    gercek(...)
  }

  for (ad in c("bir.txt", "iki.txt", "uc.txt")) {
    hedef <- env$cc_setup_yerel_yukleme_hedefi(
      kok, file.path("alt", ad), listeleme_onbellegi = onbellek
    )
    testthat::expect_false(is.null(hedef))
  }

  testthat::expect_identical(sayac$n, 1L)
})
