...
test_that("ortak oturum açık tema kontrastı ve modal yerleşimi korunur", {
  ...
  expect_true(grepl("color: #475569 !important", secici_css, fixed = TRUE))
  expect_true(grepl("-webkit-text-fill-color: #475569", secici_css, fixed = TRUE))
  expect_true(grepl("color: #334155 !important", secici_css, fixed = TRUE))
  expect_false(grepl("color: #0f172a !important", secici_css, fixed = TRUE))
  expect_false(grepl("color: #020617 !important", secici_css, fixed = TRUE))
