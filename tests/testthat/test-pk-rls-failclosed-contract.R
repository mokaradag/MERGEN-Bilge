...
test_that("taninmayan Yetki kodu KAPALI BASARISIZ olur (PR #705 P1)", {
  env <- .pk_rls_env()

  # DUZELTME ONCESI: bilinmeyen bir rol icin proje/EPS adimlari rol adina
  # BIREBIR bakildigindan `not_applicable` kaliyordu; departman kapsami da
  # acikca `not_applicable` beyan edilince plan HIC predikat uretmiyor ve
  # `apply_rls_to_data()` cerceveyi OLDUGU GIBI donduruyordu.
  for (yetki in c("XYZ", "PY-2", "py", "DIR", "ADMIN2")) {
    plan <- env$pk_rls_plan(
      list(Yetki = yetki, scope_state_depts = "not_applicable",
           scope_state_projects = "not_applicable", scope_state_eps = "not_applicable"),
      list(masraf_yeri_col = "MasrafYeri", proje_kodu_col = "ProjeKodu"),
      c("MasrafYeri", "ProjeKodu", "Deger")
    )
    expect_true(isTRUE(plan$abort), info = yetki)
    expect_identical(plan$reason, "unknown_role", info = yetki)
    expect_length(plan$predicates, 0L)
  }

  # TANINAN roller bu kapida DURMAZ.
  for (yetki in c("ADMIN", "USER", "PY", "KY-P", "DIR-P")) {
    plan <- env$pk_rls_plan(
      list(Yetki = yetki, allowed_depts = "A", scope_state_depts = "available",
           allowed_projects = "P1", scope_state_projects = "available",
           allowed_eps = "E1", scope_state_eps = "available"),
      list(masraf_yeri_col = "MasrafYeri", proje_kodu_col = "ProjeKodu",
           eps_kodu_col = "EpsKodu"),
      c("MasrafYeri", "ProjeKodu", "EpsKodu", "Deger")
    )
    expect_false(identical(plan$reason, "unknown_role"), info = yetki)
  }

  # `USER` rolu SSO varsayilanidir; departman kapsami uygulanmaya DEVAM eder.
  kullanici <- env$pk_rls_plan(
    list(Yetki = "USER", allowed_depts = c("10", "20"), scope_state_depts = "available"),
    list(masraf_yeri_col = "MasrafYeri"),
    c("MasrafYeri", "Deger")
  )
  expect_false(isTRUE(kullanici$abort))
  expect_length(kullanici$predicates, 1L)
  expect_identical(kullanici$predicates[[1]]$column, "MasrafYeri")
})
