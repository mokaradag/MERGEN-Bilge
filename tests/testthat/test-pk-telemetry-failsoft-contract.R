...
# `withr` OPSIYONELDIR; asagidaki sizinti temizligi onu kullanir.
testthat::skip_if_not_installed("withr")

local({
  repo_root <- resolve_repo_root_for_tests()

  # SIZINTI TEMIZLENIR: asagidaki tanimlar `globalenv()` icine yuklenir (uretim
  # cagri yolunu sinamak icin gereklidir) ama testthat tek oturumda calistigi
  # icin SONRAKI dosyalara sizarlar. Eklenen adlar kaydedilir ve dosya
  # bittiginde geri alinir.
  #
  # ANLIK GORUNTU `%||%` ATAMASINDAN ONCE ALINIR: sonra alinirsa operator
  # temel kumede kalir, `withr::defer()` onu HIC kaldirmaz ve sonraki dosyalar
  # bu zayif tanimi (yalniz `NULL` yedege duser) cozebilir.
  onceki_adlar <- ls(globalenv(), all.names = TRUE)

  # TELEMETRI BAYRAGI DOSYA SEVIYESINDE SABITLENIR.
  #
  # `pk_telemetry_log_analysis()` bayrak `false` cozuldugunde satir YAZMADAN
  # `FALSE` doner. Kosucu ya da gelistirici `.Renviron`'i `false` ayarlarsa
  # yazma yolu iddialari (satir yazildi / alt bilgi uretildi / alanlar dogru)
  # UYGULAMA dogru oldugu halde duserdi. Bayragi bilerek `false` yapan tek
  # test kendi kaydet/geri-yukle blogunu tasir.
  eski_telemetri <- Sys.getenv("MERGEN_PK_TELEMETRY", unset = NA_character_)
  withr::defer({
    if (is.na(eski_telemetri)) Sys.unsetenv("MERGEN_PK_TELEMETRY")
    else Sys.setenv(MERGEN_PK_TELEMETRY = eski_telemetri)
...
test_that("INSERT hatası kullanıcı isteğine sızmaz", {
  ...
  # Sondalama başarılı olacak ama INSERT patlayacak. Hazırlık probu artık TAM
  # şemayı doğruladığı için yalnızca `AnalizLogID` taşıyan bir tablo hazır
  # SAYILMAZ; INSERT hatasını izole etmek için tüm kolonlar vardır ve INSERT
  # listesinde YER ALMAYAN fazladan bir NOT NULL sütun eklenir.
  .pk_tel_create_table(conn)
  DBI::dbExecute(conn, "ALTER TABLE MB_Analiz_Log ADD COLUMN ZorunluEk TEXT NOT NULL DEFAULT ''")
  DBI::dbExecute(conn, "
    CREATE TRIGGER pk_tel_insert_reddet BEFORE INSERT ON MB_Analiz_Log
    BEGIN SELECT RAISE(ABORT, 'no such column: ZorunluEk'); END
  ")
...
test_that("hedef VERİLMEDİĞİNDE etkin yürütme bağlamından çözülür", {
  ...
  # LIFO GERI ALMA ZORUNLUDUR. `on.exit(..., add = TRUE)` varsayilan
  # `after = TRUE` ile KAYIT SIRASINDA (FIFO) calisir; baglamlar 1->2->3
  # sirasiyla KURULDUGU icin FIFO geri alma ara bir durumu geri yukler ve
  # yurutme baglami test bittikten SONRA da KURULU kalirdi (sonraki dosyalar
  # yanlis `db_target` gorur). `after = FALSE` cozmeyi 3->2->1 yapar.
  geri_al <- pk_set_exec_context(query = list(id = "q1", db_target = "secondary"))
  on.exit(geri_al(), add = TRUE, after = FALSE)
  ...
  on.exit(geri_al2(), add = TRUE, after = FALSE)
  ...
  on.exit(geri_al3(), add = TRUE, after = FALSE)
  expect_false(pk_telemetry_table_ready(yok_conn))
  expect_equal(length(cagrilar), 2L)
})

# --- Hazırlık probu TAM ŞEMAYI doğrular (PR incelemesi) --------------------
# Eski davranış yalnızca `AnalizLogID` projeksiyonu yapıyordu; INSERT'in
# kullandığı bir sütunu EKSİK olan eski bir tablo "hazır" önbellekleniyor ve
# her analiz aynı geçersiz INSERT'i sessizce yineliyordu.

test_that("hazırlık kolon listesi kayıt alanlarıyla birebir aynıdır", {
  kayit <- pk_telemetry_build_record(.pk_tel_sample_info())
  expect_identical(
    pk_telemetry_required_columns(),
    c("AnalizLogID", names(kayit))
  )
})

test_that("INSERT sütunu eksik eski tablo HAZIR sayılmaz", {
  pk_telemetry_reset_state()
  on.exit(pk_telemetry_reset_state(), add = TRUE)
  ...
  # `SatirFiltreSonrasi` BİLEREK yok: yeni bir sürümde eklenmiş sütunu
  # taşımayan eski bir dağıtımı temsil eder.
  DBI::dbExecute(conn, "
    CREATE TABLE MB_Analiz_Log (
      AnalizLogID        INTEGER PRIMARY KEY AUTOINCREMENT,
      IstekID            TEXT,
  ...
  # Hiçbir satır yazılmamıştır; eksik şema sessiz veri kaybı üretmez.
  expect_equal(
    DBI::dbGetQuery(conn, "SELECT COUNT(*) AS n FROM MB_Analiz_Log")$n, 0L
  )
})

test_that("KALICI yazım hatası hazırlık önbelleğini geçersiz kılar", {
  pk_telemetry_reset_state()
  on.exit(pk_telemetry_reset_state(), add = TRUE)
  ...
  # Tablo çalışma sırasında kaybolursa (DBA müdahalesi) INSERT KALICI olarak
  # başarısız olur; önbellekteki TRUE temizlenmeli, yoksa aynı geçersiz INSERT
  # süreç ömrü boyunca her istekte yeniden denenirdi.
  DBI::dbExecute(conn, "DROP TABLE MB_Analiz_Log")
