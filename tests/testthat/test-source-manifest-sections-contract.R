...
.expected_manifest_section_anchors <- list(
  ...
  # (R/config_file_store_bucket_clear.R) üç durumlu dizin varlık kararı ve
  # sahiplik kaybı uzlaştırmasıyla büyüdüğü için AYRI dosyaya alındı;
  # config_file_store.R 25 fonksiyon tavanında kalmaya devam eder. İndeks
  # kilidinden SONRA yüklenir (fiziksel silme + indeks tek kilit altında).
  config_app_core = list(first = "R/config_sso.R", last = "R/config_version_history.R", n = 9L),
  ...
  # files_preview_pipeline n = 11L -> 13L BİLİNÇLİ GÜNCELLEME (PR #717 inceleme
  # düzeltmeleri): ratchet bütçesi nedeniyle terfi katmanı İKİ odaklı dosyaya
  # bölündü — `R/helpers_files_promote_probe.R` (saf görünürlük yoklaması) ve
  # `R/helpers_files_staging_copy.R` (aşamalı kopya). Sıra bağımlılık sırasıdır:
  # yol -> yoklama -> terfi -> aşamalı kopya -> orkestrasyon.
  # files_preview_pipeline n = 10L -> 11L bilinçli güncelleme: sahiplik kanıtlı
  ...
  # İnceleme takibi: `helpers_pk_select_timeout.R` v1 ve v2 seçicilerinin
  # ORTAK zaman aşımı tabanıdır (v1'de taban zaman aşımı YALNIZCA async son
  # tarihi varken kuruluyordu ve sevk edilen varsayılanlarda hiç
  # kurulmuyordu). Karar tek dosyada toplandı. 91 -> 92.
  # `helpers_pk_entity_apply_leaf.R` (yaprak düzeyi varlık kararı) bakım
  # ratchet'i altında kalmak için `helpers_pk_entity_apply.R`den ayrıldı.
  # Ratchet bölünmesidir, yeni davranış değildir. 92 -> 93.
  # 95 -> 96: `helpers_pk_async_secrets.R` (işçiye taşınan sırlar)
  # `helpers_pk_async_snapshot.R` bakım ratchet'i sınırında olduğu için AYRI
  # dosyaya çıkarıldı.
  # BİLİNÇLİ GÜNCELLEME (PR #705 inceleme takibi): 96 -> 97.
...
test_that("bölümlenmiş manifest tekrar içermez ve tüm dosyalar repoda mevcuttur", {
  ...
  # 472 -> 473: `helpers_pk_select_timeout.R` v1 ve v2 seçicilerinin ORTAK
  # zaman aşımı tabanıdır; iki hattın kararı bir daha ayrışamasın diye tek
  # dosyada toplanmıştır. Yine ratchet bölünmesidir, yeni davranış yoktur.
  # 474 -> 475: `helpers_pk_async_bootstrap_fs.R` işçi bootstrap'ının SINIRLI
  # yol-varlık denetimini taşır. `helpers_pk_async_bootstrap.R` ve
  # `helpers_pk_async_worker_env.R` 24-fonksiyon KÜRESEL tavanında olduğu için
  # yardımcı oraya eklenemezdi. Ratchet bölünmesidir; davranış eklenmez.
  # 475 -> 476: `helpers_pk_sql_local_temp_batch.R` salt-okunur kapının YEREL
  ...
  # 489 -> 492: aynı PR'ın son inceleme turu, ÜÇ ratchet bölünmesi (davranış
  # sözleşmesi korunur, yeni özellik eklenmez):
  #   * R/utils_path_reservation.R — hedef yolu sahiplik jetonuyla rezerve eden
  #     ortak primitifler; üç ayrı terfi yolu (atomik yazma, yükleme kopyası,
  #     ortak çalışma alanı) ayrı ayrı yeniden yazmak yerine bunu kullanır.
  #   * R/helpers_files_promote_target.R — sahiplik kanıtlı kalıcı-ada terfi;
  #     `helpers_files_copy_promote.R` 130 satır tavanındaydı.
