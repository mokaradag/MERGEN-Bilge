...
test_that("ATOMİK `column_meta` erisimcilerde KAPALI BASARISIZ yedege duser", {
  # `[[` LİSTEDE `NULL` döner ama ATOMİK vektörde "subscript out of bounds"
  # FIRLATIR. Bozuk bir metadata katmanı erişimcileri çökertiyordu; belgelenen
  # yedekler ("none" / FALSE) dönmelidir.
  bozuk <- list(meta = list(column_meta = c("ProjeAdi", "ProjeKodu")))

  expect_identical(pk_meta_aggregate_for(bozuk, "ProjeAdi"), "none")
  expect_identical(pk_meta_match_mode(bozuk, "ProjeAdi"), "none")
  expect_false(pk_meta_is_filterable(bozuk, "ProjeAdi"))

  # BOŞ/adsız `column_meta` de aynı yedeklere düşer.
  bos <- list(meta = list(column_meta = list()))
  expect_identical(pk_meta_aggregate_for(bos, "X"), "none")
  expect_identical(pk_meta_match_mode(bos, "X"), "none")
  expect_false(pk_meta_is_filterable(bos, "X"))
})

test_that("ATOMİK `column_meta` opak hata yerine metadata bulgusu üretir", {
  # `[[` ATOMİK bir vektörde "subscript out of bounds" fırlatır (LİSTE ve
  # `NULL` için `NULL` döner). Bozuk bir metadata katmanı `column_meta` alanını
  # karakter vektörü olarak yazdığında boot, bu fonksiyonun üretmesi gereken
  # AÇIKLAYICI bulgu yerine opak bir hatayla düşerdi.
  bozuk <- list(q001 = .pk_meta_test_query())
  bozuk$q001$column_meta <- c("ProjeAdi", "ProjeKodu")

  sonuc <- pk_meta_apply_alias_overlay(bozuk, list(q001 = list(ProjeAdi = c("a" = "B"))))

  expect_true(any(grepl("beyan edilmemis", sonuc$errors, fixed = TRUE)))
})
...
test_that("SQL loader metadata sözleşmesini çağırır ve strict bayrağından bağımsızdır", {
  ...
  # toplardı.
  # Sahip ortam BİR KEZ çözülür (`.sql_loader_env <- environment()`) ve hem
  # okuma hem YAZMA aynı ortamda yapılır. Eskiden guard ÜST ortamdaki kopyayı
  # kabul ediyor, atamalar ise YEREL bir kopya oluşturuyordu: sahneleme
  # ortamında bayat bir envanter doğrulanıp "doğrulandı" diye raporlanabilirdi.
  expect_true(
    grepl(".sql_loader_env <- environment()", metin, fixed = TRUE, useBytes = TRUE),
    info = "Yukleme ortami bir kez cozulmelidir (.sql_loader_env)."
  )
  cagri_metni <- "pk_query_meta_attach(query_library, envir = .sql_loader_env)"
