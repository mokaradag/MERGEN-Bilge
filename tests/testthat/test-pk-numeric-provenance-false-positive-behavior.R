...
test_that("BAĞLAM PENCERESİ önceki iddianın toplulaştırmasını taşımaz", {
  env <- .prov_fp_env()
  ort <- .prov_fp_fact("f_ort", 5, aggregation = "mean")
  top <- .prov_fp_fact("f_top", 100, aggregation = "sum")

  # İkinci iddianın KENDİ toplulaştırma sözcüğü yoktur. Pencere iddia sınırını
  # aşarsa en sağdaki terim `Ortalama` bulunur ve DOĞRU `sum` olgusu
  # `aggregation_mismatch` bildirilirdi.
  metin <- "Ortalama sure 5 [fact:f_ort]. Butce 100 [fact:f_top]."
  sonuc <- env$pk_numeric_provenance_validate(metin, list(ort, top))
  expect_false("aggregation_mismatch" %in% .prov_fp_reasons(sonuc))

  # GERÇEK uyuşmazlık HÂLÂ yakalanır: iddia "Ortalama" der, olgu `sum`'dur.
  gercek <- env$pk_numeric_provenance_validate(
    "Ortalama butce 100 [fact:f_top].", list(top)
  )
  expect_true("aggregation_mismatch" %in% .prov_fp_reasons(gercek))
})

test_that("ONDALIK NOKTA cümle sonu sayılmaz", {
  env <- .prov_fp_env()
  olgu <- .prov_fp_fact("f_bin", 18420)
  sonuc <- env$pk_numeric_provenance_validate(
...
test_that("RAKAM İÇEREN BİRİM alıntısız taramada da tam eşleşir", {
  env <- .prov_fp_env()
  olgu <- .prov_fp_fact("f_alan", 1234, unit = "m2")
  # `1.234 m2 [fact:...]` doğru alıntılanmıştır: artakalan `2` nöbetçi
  # denetimini bloke edip `missing_fact_marker` üretmemelidir.
  sonuc <- env$pk_numeric_provenance_validate(
    "Toplam alan 1.234 m2 [fact:f_alan].", list(olgu)
  )
  # TAM EŞLEŞME İDDİA EDİLİR (PR #705 incelemesi, P3): yalnızca
  # `missing_fact_marker` yokluğuna bakmak, aynı sayının `no_number` ya da
  # `value_mismatch` diye sınıflandırıldığı bir regresyonu GEÇİRİRDİ.
  expect_identical(sonuc$checked, 1L)
  expect_length(sonuc$mismatches %||% list(), 0L)
  expect_false("missing_fact_marker" %in% .prov_fp_reasons(sonuc))
})

test_that("TANINAN BİRİM taşıyan yıl-benzeri sayı köksüz kalamaz", {
  env <- .prov_fp_env()
  olgu <- .prov_fp_fact("f_saat", 2024, unit = "saat")

  # "2024 saat" 1900-2100 aralığındadır ama YIL DEĞİL, ölçülen bir değerdir.
  koksuz <- env$pk_numeric_provenance_validate("Toplam 2024 saat.", list(olgu))
  expect_true("missing_fact_marker" %in% .prov_fp_reasons(koksuz))

  # GERÇEK yıl ifadesi HÂLÂ muaf kalır (davranış değişmedi).
  yil <- env$pk_numeric_provenance_validate("2024 yilinda tamamlandi.", list(olgu))
  expect_false("missing_fact_marker" %in% .prov_fp_reasons(yil))
})
...
test_that("sayi ile isaret arasindaki isim obegi CIFT uyusmazlik uretmez (PR #705 P2)", {
  ...
  # `no_number`, koken-siz tarayici ise nobetciyi bulamayip
  # `missing_fact_marker` kaydediyordu. TEK bir dogru alinti icin
  # `checked = 2, mismatches = 2` cikiyor ve `block` kipinde DOGRU yanit
  # yedekle degistiriliyordu.
  for (metin in c(
    "Toplam 15.574 geciken aktivite [fact:f].",
    "Toplam 15.574 geciken toplam acik aktivite [fact:f].",
    "Toplam 15.574 aktivite [fact:f]."
  )) {
    sonuc <- env$pk_numeric_provenance_validate(metin, olgular)
  ...
  # BITISIK bicim ve vurgu davranisi DEGISMEZ.
  bitisik <- env$pk_numeric_provenance_validate("Toplam 15.574 [fact:f].", olgular)
  expect_length(bitisik$mismatches %||% list(), 0L)

  # KORUMA GEVSEMEZ: araya baska bir SAYI girerse iddia yine reddedilir.
  arada_sayi <- env$pk_numeric_provenance_validate(
    "Toplam 15.574 ve 42 aktivite [fact:f].", olgular
  )
  ...
  yanlis <- env$pk_numeric_provenance_validate(
    "Toplam 11.111 geciken aktivite [fact:f].", olgular
  )
  expect_true("value_mismatch" %in% .prov_fp_reasons(yanlis))
