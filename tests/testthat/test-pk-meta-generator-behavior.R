...
# `withr` DOSYA DUZEYINDE ZORUNLUDUR (PR #705 incelemesi, P3): bu dosya
# `withr::with_envvar()`, `withr::local_tempdir()` ve `withr::local_envvar()`
# cagirir. Paket kurulu degilse her test "there is no package called 'withr'"
# hatasiyla DUSERDI; bu kohortta `withr` istege bagli bir test bagimliligidir.
...
.GEN_ENV_KEYS <- c(
  ...
  # `pkgh_read_state()` bunu OKUR: listede eksikken operatör/CI kabuğu
  # `MERGEN_PK_META_RESUME_MAX_AGE_SEC=0` verdiğinde tazelik sözleşmesi
  # (bayat durum kabul EDİLMEZ) kod kusuru olmadan kırılıyordu.
...
test_that("devam onbellegi yazilip geri okunur ve kip degisince YOK SAYILIR", {
  ...
  # VARSAYILAN TTL YOLU SINANIR: dagitim/CI kabugu
  # `MERGEN_PK_META_RESUME_MAX_AGE_SEC` verirse tazelik dali degisir.
...
test_that("kosu kimligi ayni saniyede bile CARPISMAZ", {
  # ZAMAN DILIMI SABITLENIR (PR #705 incelemesi, P2): `pkg_meta_run_id()`
  # damgayi `format()` ile OTURUM saat diliminde bicimlendirir; UTC bir fikstur
  # Turkce VM'de (Europe/Istanbul, UTC+3) "20260817-150000-" uretir ve onek
  # iddiasi duserdi.
...
test_that("BAYAT devam durumu yeniden kullanilmaz (DB kanidi eskir)", {
  ...
  # VARSAYILAN TTL YOLU SINANIR: ortamda tanimli bir siniri okumak bu
  # sozlesmeyi kod kusuru olmadan kirardi (asagida sinir ACIKCA kapatilir).
  withr::local_envvar(c(MERGEN_PK_META_RESUME_MAX_AGE_SEC = NA_character_))
