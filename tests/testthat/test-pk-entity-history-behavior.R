...
test_that("D11 hâlâ geçerlidir: v1 yolu chat_history'yi OKUMAZ", {
  ...
  yorumu_at <- function(satir) {
    ...
    for (i in seq_along(karakterler)) {
      ...
      if (nzchar(tirnak)) {
        # ARDIŞIK TERS BÖLÜ SAYILIR, tek önceki karaktere BAKILMAZ: `"\\\\"`
        # gibi bir dizede kapanış tırnağından önceki karakter `\\` olduğu için
        # tırnak kapanmıyor, satırın geri kalanındaki her `#` dize içeriği
        # sanılıyordu. Tek sayıda ters bölü kaçış, çift sayıda kaçış DEĞİL.
        kacis_sayisi <- 0L
        j <- i - 1L
        while (j >= 1L && identical(karakterler[j], "\\")) {
          kacis_sayisi <- kacis_sayisi + 1L
          j <- j - 1L
        }
        if (identical(k, tirnak) && kacis_sayisi %% 2L == 0L) tirnak <- ""
      } else if (k %in% c("\"", "'")) {
        tirnak <- k
      } else if (identical(k, "#")) {
  ...
  # İÇ BOŞLUK NORMALLEŞTİRİLİR: sözleşme `chat_history`'nin nerede GÖRÜNDÜĞÜ
  # hakkındadır, kaynağın nasıl GİRİNTİLENDİĞİ hakkında değil. Argüman listesi
  # yeniden sarılırsa (davranış değişmeden) bu test süiti düşürüyordu.
  # Kardeş sözleşme (test-pk-query-selection-contract.R) aynı çağrı yerini
  # zaten biçim toleranslı karşılaştırıyor.
  bosluk_normalle <- function(x) trimws(gsub("[ \t]+", " ", x))

  kod <- vapply(kod, bosluk_normalle, character(1), USE.NAMES = FALSE)
  gecisler <- kod[grepl("chat_history", kod, fixed = TRUE)]
  ...
  izinli <- vapply(izinli, bosluk_normalle, character(1), USE.NAMES = FALSE)
