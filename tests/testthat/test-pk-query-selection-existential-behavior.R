...
test_that("`unsupported` SERBEST METİNDİR ve kanonikleştirilmez", {
  ...
  donusum <- pk_select_canonicalize_requirements(
    .pk_exist_query(),
    # DEĞER BİLEREK VARLIK KİPİ TAŞIR (PR #705 incelemesi, P3): eski fikstür
    # (`"gecikme nedeni kirilimi"`) İKİ ayrı nedenle `changed = FALSE` üretiyordu
    # ve `unsupported` alanı kanonikleştirme listesine EKLENSE bile blok yeşil
    # kalırdı. Bu değer kanonikleştirilseydi `activity.delayed_count` olurdu.
    list(measures = character(0), unsupported = "activity.delayed_exists"),
    capability_ids = .pk_exist_ids(), registry = .pk_exist_registry()
  )
  expect_false(isTRUE(donusum$changed))
  expect_setequal(donusum$requirements$unsupported, "activity.delayed_exists")
  expect_length(donusum$requirements$measures, 0L)
})
