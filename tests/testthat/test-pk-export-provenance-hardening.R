...
test_that("P1: tek satirlik tabakadan BASKA bir satir secilmez", {
  env <- .pk_export_hardening_env()
  # "Z" ORTADADIR. Sonda dururken `pk_packet_examples()` onu SINIR SATIRI
  # dalindan (ilk + son satir) zaten ekliyor ve satir `kalan` havuzundan
  # dusuruluyordu; tabakali orneklem tek satirlik "Z" tabakasini HIC gormuyor,
  # asagidaki iki iddia yalnizca sinir dalindan sagalaniyordu. Yani `sample(idx, 1)`
  # kusuru geri gelse bile test GECERDI (vacuous regresyon kilidi).
  veri <- data.frame(
    Bolum = c(rep("A", 30), "Z", rep("A", 30)),
    Saat = c(rep(1, 30), 2, rep(1, 30)),
    stringsAsFactors = FALSE
  )
  ...
  # davranir ve 31. satir yerine 1..31 arasindan rastgele bir satir secerdi.
  # Tek satirlik "Z" tabakasi YALNIZCA 31. satirla temsil edilebilir ve o satir
  # sinir dalindan GELMEZ; iddia bu yuzden tabakali ornekleyiciyi gercekten
  # sinar.
  expect_true(31L %in% ornek$indices)
...
test_that("P2: `-ma/-me` kokleri OLUMLU istek bicimlerinde olumsuzlama SAYILMAZ", {
  env <- .pk_export_hardening_env()

  # GERİLEME: `gonderme` / `ekleme` / `olusturma` kökleri sıradan OLUMLU istek
  # biçimlerinin İÇİNDE de geçiyordu ("göndermeni istiyorum"). Çıplak alt dize
  # eşleşmesi bunları olumsuzlama sayıyor, olumlu kapsam "ni istiyorum"a
  # düşüyor ve KULLANICININ İSTEDİĞİ dışa aktarım sessizce dusuyordu.
  niyet <- env$pk_compose_export_intent("Excel dosyasını göndermeni istiyorum")
  expect_true(isTRUE(niyet$wants))
  expect_identical(niyet$format, "xlsx")

  for (soru in c("listeyi eklemeni istiyorum", "rapor oluşturmanı istiyorum",
                 "excel dökümünü üretmeni istiyorum")) {
    expect_true(isTRUE(env$pk_compose_export_intent(soru)$wants), info = soru)
  }

  # GERÇEK olumsuz emir hala olumsuzlamadır: sözcük orada BİTER ya da gerçek
  # bir olumsuz emir eki gelir.
  for (soru in c("Excel gönderme", "Excel dosyası göndermeyin",
                 "rapor oluşturma", "listeye ekleme")) {
    expect_false(env$pk_compose_wants_export(soru), info = soru)
  }

  # Kök SÖZCÜK ORTASINDA geçerse olumsuzlama değildir.
  expect_true(env$pk_compose_wants_export("excel dosyasini hazirla ve yolla"))
})
