...
test_that("açık tema disiplini: tombstone, tekrar bütçesi ve tema satır bütçesi korunur", {
  ...
  #    zincirinin ~%46'sı böyleydi. Taban: 26 (tamamı, canlı kuralların grup
  #    üyesi kalıntılarıdır; tek başına ölü kural kalmadı); tema alan
  #    dosyaları için taban: 7. Yeni hayalet seçici eklemek bütçeyi aşar —
  #    seçiciyi gerçek DOM sınıfına bağlayın ya da dinamik üretim önekini
  #    rapora bilinçli ekleyin (frontend_orphan_constructed_prefixes).
  dead_rows <- attr(report, "dead_selector_rows", exact = TRUE)
  theme_dead_rows <- attr(report, "theme_dead_selector_rows", exact = TRUE)
  max_dead <- .as_int_env_frontend("MERGEN_TEST_MAX_FRONTEND_DEAD_SELECTORS", 26L)
