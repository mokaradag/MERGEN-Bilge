...
test_that("işçi durum metni bilinmeyen durumda genel hata verir", {
  ...
  # BAŞLIĞIN GERÇEKTEN İDDİA ETTİĞİ DURUM: eşlenmemiş bir değer. Yukarıdaki iki
  # durum BİLİNEN durumlardır; yardımcı eşlenmemiş bir durumda `NULL`/boş dize
  # döndürseydi bu test yine geçer, sevk katmanı ise sohbete BOŞ bir mesaj
  # eklerdi.
  for (bilinmeyen in list("kaynak_siniri", "", NA_character_, NULL)) {
    metin <- env$mergen_pk_worker_outcome_text(bilinmeyen)
    expect_type(metin, "character")
