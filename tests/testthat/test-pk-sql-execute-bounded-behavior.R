...
local({
  ...
  #
  # SIZINTI TEMİZLENİR: bu tanımlar `globalenv()` içine yüklenir (çağrı yerleri
  # niteliksizdir), ama dosya bittiğinde ORADA KALMAMALIDIR. Tam suite koşumunda
  # sonraki dosyalar `exists(..., inherits = TRUE)` ile yoklayıp kendi
  # fail-closed dallarını ATLIYOR; dosya tek başına geçerken paket koşumunda
  # FARKLI davranıyordu. Yalnızca BU dosyanın eklediği adlar kaldırılır.
...
test_that("boş sonuç sütun sözleşmesini korur", {
  ...
  # SÜTUN SÖZLEŞMESİ GERÇEKTEN DENETLENİR: yalnızca satır sayısı ve çerçeve
  # türüne bakmak, SIFIR SÜTUNLU bir çerçeveyi de geçirir; testin adı ise
  # sütun sözleşmesinin korunduğunu iddia ediyordu.
  expect_equal(names(sonuc$data), c("id", "ad", "tutar"))
  expect_gte(.pk_clear_calls$n, 1L)
})
