...
testthat::test_that("sonraki AI yanıtı varsa açıklama olarak o kullanılır", {
  ...
  testthat::local_mocked_bindings(
    dbGetQuery = function(conn, statement, ...) {
      if (grepl("INNER JOIN MB_Chats", statement, fixed = TRUE)) {
        return(main_rows)
      }
      data.frame(MessageContent = "Bu bir kedi fotoğrafıdır", stringsAsFactors = FALSE)
    },
    .package = "DBI"
  )
...
testthat::test_that("sonraki AI yanıtı yoksa satır içi açıklamaya düşer", {
  ...
  testthat::local_mocked_bindings(
    dbGetQuery = function(conn, statement, ...) {
      ...
      data.frame(MessageContent = character(0), stringsAsFactors = FALSE)  # sonraki yanıt yok
    },
    .package = "DBI"
  )
...
testthat::test_that("görsel mesajı yoksa boş liste döner", {
  ...
  testthat::local_mocked_bindings(
    dbGetQuery = function(conn, statement, ...) {
      data.frame(MessageContent = character(0), ChatID = integer(0), MessageOrder = integer(0))
    },
    .package = "DBI"
  )
