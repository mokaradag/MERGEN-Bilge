...
.pk_observation_test_env <- function() {
  ...
  for (rel in c(
    ...
    # `integer64` KESİNLİK yardımcısı: v1 sayısal filtre dalı `bigint` operandı
    # bunun üzerinden üretir; manifestte filtre dosyalarından ÖNCE gelir.
    "R/helpers_pk_precision.R",
...
test_that("toplulaştırma kökeninde eşleşen kayıt sayısı korunur", {
  ...
  # ÇAĞIRAN BAĞLAMI `local({...})` İÇİNDE KURULUR -- BU KASITLIDIR.
  #
  # `.pk_filter_observation_context()` bağlamı `sys.frames()` ile, yani ÇAĞRI
  # YIĞINI üzerinden toplar; LEKSİK kapsam kullanmaz. `local()` bloğu yığında
  # gerçek bir çerçevedir, dolayısıyla `session`/`selected_query` üretim koduna
  # ULAŞIR. (Bunları `env` içine yazmak çalışmaz: `env` bir çağrı çerçevesi
  # değildir; öyle bir değişiklik bu testleri düşürür.)
  result <- local({
    session <- list()
...
test_that("gözlem yalnız uygulanan filtreleri ve BİLDİRİLEN motoru kaydeder", {
  ...
  # ÇAĞIRANIN AÇIKÇA BİLDİRDİĞİ MOTOR KORUNUR.
  #
  # Bu iddia eskiden `"v2"` bekliyordu: `"v1"` bildirimi de çözüm dalına
  # sokuluyor ve sorgu metadata'sındaki `engine = "v2"` onu EZİYORDU. Yanıtı v1
  # hattı ürettiği hâlde `MB_Analiz_Log` `v2` raporluyordu; rollout metrikleri
  # bu yüzden yanlış atfediliyordu.
  expect_identical(captured$info$engine, "v1")
  ...
  # BİLDİRİM YOKSA yapılandırmadan çözülür (davranış değişmedi).
  captured$info <- NULL
  env$pk_analysis_observe(list(), NULL, list(
    request_id = "req-observe-2",
    question = "iki üstünü say",
    username = "kullanici",
    query_id = "q-engine",
    query_name = "Motor Sorgusu",
