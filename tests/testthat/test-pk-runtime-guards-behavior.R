...
test_that("v2 Derin Dusunme kanonik paket ve olgulari kullanir", {
  ...
  # STUB'LAR `...` KABUL EDER: uretim bu yardimcilari ek adlandirilmis
  # argumanlarla cagirabilir; sabit imzali bir stub o durumda "unused argument"
  # ile duser ve test, urun davranisi dogruyken KIRMIZI olurdu.
...
test_that("kalan başlatma bütçesi GEÇEN SÜRE kadar KÜÇÜLÜR", {
  ...
  # PR #705 inceleme bulgusu: TÜKENMİŞ bütçe artık 1 saniyelik YENİ bir
  # başlatma izni üretmez. `max(kalan, 1)` süresi dolmuş bir isteğin
  # `makeClusterPSOCK()` çağırmasına ve kaynak tüketmesine izin veriyordu;
  # çağıran 0 değerini görüp tipli `deadline` sonucu döndürür.
  expect_identical(
