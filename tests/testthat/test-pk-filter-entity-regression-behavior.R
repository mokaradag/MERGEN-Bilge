...
test_that("varlik cozumleyicisi grup cocuklarini gezer", {
  ...
  # dosyalarını FARKLI bir dala sokar ve iddia üretimden sapabilir.
  env$`%||%` <- function(a, b) if (is.null(a)) b else a
  for (.pk_kaynak_dosya in c("helpers_pk_entity_tree.R", "helpers_pk_entity_apply_leaf.R", "helpers_pk_entity_apply.R")) source(file.path(repo_root, "R", .pk_kaynak_dosya),
...
.pk_entity_test_env <- function() {
  ...
  for (dosya in c("helpers_pk_text_turkish.R", "helpers_pk_config.R",
                  ...
                  "helpers_pk_entity_apply_leaf.R", "helpers_pk_entity_apply.R")) {
    source(file.path(repo_root, "R", dosya), encoding = "UTF-8", local = env)
  }
  env
}
