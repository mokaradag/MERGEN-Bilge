...
**STATUS: MET.** Built as `tools/pk/generate_query_meta.R` plus five helpers under
`tools/pk/` (config, schema/evidence, render, health, run); DB access is injected so
the whole decision path is offline-testable. Covered by
`tests/testthat/test-pk-meta-generator-behavior.R` and
