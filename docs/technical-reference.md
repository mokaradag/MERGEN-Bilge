...
2. **Belirleyici göreli adaylar — TARAMA YOK** (`.preview_direct_rel_candidates()`, `.preview_probe_direct()`): TÜM model tabanlarında sırasıyla düz ad (`A&&B&&C.pdf`), iç içe yol (`A/B/C.pdf`) ve ipucu PDF ise aynı gövdeli `.docx` > `.docm` > `.doc` eşdeğerleri (her uzantı için önce düz, sonra iç içe) denenir. Tam PDF her zaman Word eşdeğerinden önce gelir; Word eşdeğeri yalnızca PDF atfına uygulanır (tür genişletilmez). Bu, üretimde "PDF atıflı kaynak diskte Word" durumunu da taramasız çözer.
3. **Önbellekteki indeks — TARAMA YOK**: her tabanın önbellekteki (taze, bayat ya da kısmi) indeksinde tam ipucu aranır. Daha önce bulunmuş bir kaynak böylece kısmi indeksle bile yeniden taranmadan açılır.
4. **Taban başına EN FAZLA BİR sınırlı tarama**: yalnızca indeks yetkili değilse (yok/bayat ve geri çekilmede değil) `.build_basename_index()` tek kez tarar; tam ipucu, aynı gövdeli Word eşdeğeri, parça-içerme ve dosya adıyla zayıf arama AYNI indeks girdisini paylaşır (`search_file_in_folder(..., idx = )`). Tek bir tıklama bir tabanı ikinci kez tarayamaz.

**Güvenlik sınırları.** İpucu parçaları `.preview_hint_parts_safe()` ile süzülür: `..`/`.`, yol ayırıcı, mutlak yol, sürücü harfi ve HER KONUMDA `:` (NTFS alternatif veri akışı) ile kontrol karakterleri reddedilir. Hangi katmandan gelirse gelsin HER aday açılmadan önce `path_existing_variant()` ile var olan bir DOSYA (klasör değil) olarak ve `.preview_path_inside()` ile model tabanının İÇİNDE doğrulanır; kanonik denetim `normalize_mcp_path()` üzerinden yapılır (UNC biçimi korunur, POSIX'te sembolik bağlantı hedefi çözülür). Kök dışına çözülen indeks isabeti de reddedilir.

**İndeks ve geri çekilme.** İndeks taraması (`.file_index_scan_bounded()`) yalnızca AD listelemesi yapar: dizin başına tek listeleme (UNC'de boş dönen `list.files()` için paylaşılan `cc_scan_list_dir_bounded()` yedeği dahil), dosya başına `normalizePath()`/boyut okuması YAPILMAZ; bağlantı/reparse point dizinleri izlenmez, dosya bağlantıları indekse alınmaz, bağımlılık/önbellek ağaçları atlanır. Tam tarama `MCP_INDEX_TTL_MIN` (varsayılan 10 dk) boyunca yetkilidir; kısmi ya da başarısız tarama TTL ile önbelleğe alınmaz, önceki harita korunur ve `FILE_INDEX_SCAN_FAIL_BACKOFF_SEC` (30 sn) geri çekilmesi taramanın **BİTİŞ** anından ölçülür (başlangıç anından ölçülen damga, 40-70 sn süren UNC taramasının ardından aynı tıklamada taramayı yeniden başlatıyordu). Süre bütçesi işbirlikçidir: işletim sisteminde bloklanan tek bir dizin çağrısı bütçeyi aşabilir; bu yüzden tarama yalnızca 1-3. adımlar başarısız olduğunda çalışan istisnai yoldur. Başlangıçta `config_api.R` model tabanlarının indeksini önceden ısıtır.
