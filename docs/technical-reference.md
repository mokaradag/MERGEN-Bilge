# MERGEN Bilge Teknik Referans

Bu belge, MERGEN Bilge'nin ürün kapsamı, sayfa davranışları, mimari notları, kurulum ayrıntıları, test/doğrulama profilleri ve bakım sınırları için derin teknik referanstır. Hızlı başlangıç için [`../README.md`](../README.md), dokümantasyon merkezi için [`README.md`](README.md), sıkı ajan/bakım kuralları için [`../CLAUDE.md`](../CLAUDE.md), operasyon için [`../RUNBOOK.md`](../RUNBOOK.md) ve mimari yön bulma için [`architecture-map.md`](architecture-map.md) okunmalıdır.

> Bu dosya ayrıntılı başvuru kaynağıdır; pratik çalışma sırasında `CLAUDE.md`, `RUNBOOK.md`, `dependency-locking.md`, `architecture-map.md` ve ilgili testler birlikte değerlendirilmelidir.
...
- `R/helpers_markdown_safety.R` ayrıca oluşturulan görsel kartı için tek kanonik güvenli HTML üreticisini barındırır: `mergen_generated_image_card_html()` (görsel + filigran + indir/kopyala/yazdır butonları + isteğe bağlı açıklama). `message_id` ve `description` kaçışlanır; `img_src` güvenli kabul edilir. Hem canlı görsel oluşturma yolu (`R/module_image_generation.R`) hem de kaydedilmiş söyleşi yeniden yükleme yolu (`R/helpers_chat_message_formatting.R::db_message_render_image_html()`) kart işaretlemesini bu yardımcıya delege eder; daha önce üç yerde tekrarlanan işaretleme tek kaynağa indirildi. Sözleşme `tests/testthat/test-generated-image-card-html-contract.R` ile izlenir.
...
- `decode_stream_delta_payload()` artık her girişte tek elemanlı karakter döndürmeyi garanti eder: bozuk/yarım yazılmış satırlardan gelen boş liste (`[]`) veya NA alan şekilleri `if()` içinde NA/`logical(0)` üretmez ve `""` olarak zararsızlaşır. Üretim yazıcısı (`create_stream_line_appender()`) boş metni zaten hiç yazmaz; bu skaler garanti yalnızca okuyucu tarafını bozuk satır uçlarına karşı sağlamlaştırır. Geçerli base64/düz metin davranışı (Türkçe karakterler dahil) değişmemiştir; `tests/testthat/test-llm-stream-io-contract.R` içindeki skaler dönüş regresyon testleriyle korunur.
- True streaming yoklama (poll) döngüsü kararları da saf yardımcı sınırına taşınmıştır: `R/helpers_streaming_poll_lifecycle.R` içindeki `mergen_stream_classify_poll_lines()` akış JSONL satırlarını delta / akıl yürütme / debug olarak sınıflandırır (bozuk/yarım satırlar sessizce atlanır, satır sırası korunur); `mergen_stream_reasoning_recovery_plan()` worker dönüşünde gelen reasoning metni için none / replace_full / append_suffix kararını üretir ve reasoning'in yalnızca final chunk'ta geldiği uçlarda `MB_Messages.ReasoningContent`'in NULL kalmasını engelleyen yolun saf çekirdeğidir; `mergen_stream_poll_interval_ms()` ve `mergen_stream_persist_delay()` yoklama aralığı ile ertelenen sohbet kalıcılaştırma gecikmesini çözer. `R/server_handler_true_streaming.R` bu kararları inline taşımaz; sınır `tests/testthat/test-streaming-poll-lifecycle-contract.R` ile, davranış ise Türkçe/emoji base64 round-trip dahil `tests/testthat/test-streaming-poll-lifecycle-behavior.R` ile Shiny/DB/LLM başlatmadan korunur.
...
- AI Uzman sunucu işleyicilerinin saf karar mantığı `R/helpers_ai_expert_handlers_support.R` içindedir: `ai_expert_page_name_tr()` (sayfa kimliği → tek kaynak Türkçe sayfa adı), `ai_expert_first_idle_delay_ms()` / `ai_expert_idle_interval_ms()` (sıklık → ms) ve `build_ai_expert_idle_user_context()` (boşta konuşma bağlam metni; `Sys.time()` yerine dışarıdan `now_text` alır, deterministiktir). Bu dosya Shiny/DB/ağ/worker yan etkisi içermez; `R/server_ai_expert_handlers.R` bunu çağırarak orkestrasyona odaklı kalır (726/23 → 652/22). Saf karar mantığı handler'a geri inline edilmemeli; üç AI Uzman LLM `tracked_future_promise(call_ai_expert_llm)` bloğu ise VM-only async yoldur ve cloud'da yeniden yapılandırılmamalıdır.
...
- Görsel anlama destekli modeller `.Renviron` içinde `FILTER_MODEL` satırından sonra kısa bir başlıkla `MERGEN_VISION_MODELS=model-1;model-2` biçiminde tanımlanmalıdır.
- Derin Düşünme modelleri `.Renviron` üzerinden tanımlandığında, bilinmeyen model kimlikleri başlangıçta güvenli biçimde `local_model_capabilities` ve `local_model_endpoint_map` kapsamına alınır; endpoint eşlemesi eksikse varsayılan `primary` kullanılır. Bu kayıt mekanizması artık tek sahiptedir: `R/helpers_deep_thinking_model_capabilities.R` içindeki saf `apply_deep_thinking_model_capabilities()` yardımcısı, `config_api.R` içinde önceden iki ayrı source-time blok olarak yaşayan davranışı birebir bileşimiyle birleştirir (var olan açık tanımların değerleri kazanır, yalnızca eksik alanlar tamamlanır) ve `config_api.R` tarafından guard'lı çağrılır. Bu koruma, eksik isimli vektör erişiminden kaynaklanabilecek “altindis sınırlar dışında” başlangıç çökmesini engeller; `tests/testthat/test-api-model-config-refactor-contract.R`, `tests/testthat/test-deep-thinking-model-capabilities-behavior.R` (eski iki-blok mekanizmasına karşı denklik testi dahil) ve `tests/testthat/test-config-api-split-contract.R` ile korunur. Kullanıcı API anahtarı şifreli saklama katmanı da `config_api.R`'den `R/helpers_api_key_crypto.R` dosyasına taşınmıştır; davranış, NUL-tuz regresyon guard'ı ve atomik yazım yolu değişmemiştir.

#### Araç Bazlı Model Çözümleme ve Canlı Düşünce Akışı

Araç seçimi, Derin Düşünme durumu, düşük/yüksek seviye ve çalışma modu tek noktadan çözümlenir; çözülen model hem gerçek LLM isteğinde hem Düşünce Akışı rozetinde hem de final-boundary istek loglarında kullanılır. Excel Analizi için MCP araç geçişi korunur: ilk geçiş araç çağrılarını streaming olmadan tamamlar, araç sonuçları alındıktan sonraki ikinci sentez geçişinde düşünme destekli modeller için canlı Düşünce Akışı SSE ile akar. Bu ikinci geçişte normalize edilmiş mesajlar kullanılır, sistem mesajları öne alınır, reasoning deltaları canlı panele yazılır ve SSE başarısız olursa veya yalnızca reasoning-benzeri içerik dönerse güvenli non-streaming ikinci geçişe düşülür; reasoning metni nihai yanıt gövdesi olarak kullanılmaz ve görünür Düşünce Akışı paneli yinelenmez.

Proje ve Kaynak Analizi (`sql_analysis`) düşünmeli modelde artık benzer şekilde canlı Düşünce Akışını gerçek SSE ile akıtır: senkron sorgu/RLS/filtre ön adımından sonra nihai sentez genel true-SSE yolundan (`handle_true_streaming_mode`) akar. Karar saf `mergen_sql_analysis_stream_plan()` yardımcısıyla verilir; canlı SSE yalnızca akış açık, TTS kapalı ve MCP kapalıyken seçilir, aksi halde eski güvenli non-streaming korunur. Güvenlik ağı işçi (`call_local_llm_sse_worker`) içindedir ve yalnızca `allow_non_streaming_fallback` açıkken devreye girer: akış boş veya yalnızca reasoning-benzeri içerik döndürürse işçi tek seferlik `stream=FALSE` yeniden denemesiyle yalnızca nihai yanıt gövdesini düzeltir, her hata güvenle yutulur ve reasoning trace değiştirilmez.

Model adları sabit kabul edilmemelidir. Düşük/yüksek Derin Düşünme modelleri `.Renviron` veya ortam değişkenlerindeki `EXCEL_DEEP_LOW_MODEL`, `EXCEL_DEEP_HIGH_MODEL`, `CODING_DEEP_LOW_MODEL` ve `CODING_DEEP_HIGH_MODEL` anahtarlarından gelir; bilinmeyen veya ortam değişkeninden gelen modeller için kalıcı dokümantasyonda gerçek model ismi yerine yapılandırma anahtarı kullanılmalıdır. Bir model Düşünce Akışı gösterecekse `api_config$local_model_capabilities` içinde `thinking`, `stream_reasoning`, `omit_temperature`, `allow_reasoning_fallback` ve gerekiyorsa `request_overrides` ile açıkça tanımlanmalıdır. Modelin hangi endpoint’e gideceği `api_config$local_model_endpoint_map` üzerinden yönetilmelidir.

Sorun giderirken Düşünce Akışı rozetinde beklenen model görünmüyorsa önce runtime model çözümleme ve gerçek payload modelini gösteren final-boundary logları karşılaştırılmalıdır. Canlı Düşünce Akışı gelmiyorsa model capability kaydı ve endpoint map kontrol edilmelidir. SSE reasoning yalnızca düşünce üretip görünür yanıt üretmezse sistem reasoning panelini korur ve güvenli non-streaming ikinci geçişle nihai yanıtı alır.

Bakım sınırında yeni mantık ratchet eşiklerini büyütmemek için küçük helper dosyalarına ayrılmıştır: `R/helpers_api_model_tool_runtime.R` araç ailesi ve Derin Düşünme model çözümleme; `R/helpers_send_message_model_runtime.R` mesaj gönderme runtime hazırlığı ve MCP reasoning stream etkinleştirme; `R/helpers_llm_worker_second_pass.R` MCP ikinci sentez geçişi, SSE reasoning stream, reasoning-benzeri içerik tespiti ve non-streaming fallback sorumluluklarını taşır. Yeni runtime veya LLM ikinci geçiş davranışı eklenirse mevcut büyük dosyalara gömmek yerine ilgili helper genişletilmeli veya yeni küçük helper eklenmelidir; bu sınır `tests/testthat/test-send-message-model-runtime-contract.R`, `tests/testthat/test-llm-worker-second-pass-contract.R`, `tests/testthat/test-llm-worker-payload-refactor-contract.R`, `tests/testthat/test-send-message-request-lifecycle-contract.R`, `tests/testthat/test-api-model-config-refactor-contract.R` ve `tests/testthat/test-maintainability-ratchet.R` ile korunur.
...
- Dosya Yönetimi yükleme ve tablo yolları kullanıcıya görünen dosya adını aynı saf sınırdan çözer: `R/helpers_file_manager_table.R::fm_normalize_uploaded_file_info()`. Bu yardımcı storage-prefix temizleme, `display` metadata önceliği ve UTF-8/Türkçe karakter onarımını `normalize_file_display_name()` kanonik deposuna delege eder; `fm_clean_file_display_name()` uyumluluk wrapper'ı olarak aynı yardımcıyı çağırır. Böylece upload state, tablo satırı ve kalıcı yeniden yükleme yolları ayrı ayrı dosya adı temizleme mantığı taşımadan aynı davranışı korur.
...
- Açık temada sayfa hero başlıkları kurumsal mavi şerit olarak standardize edilmiştir; karşılama ekranı greeting/ikon/neural fonu, hızlı işlem kartları, Kişiselleştirme deneyim modu ve karakter aksanları ile Sistem Durumu kart kontrastı ilgili alan dosyalarında korunur.
- Açık tema başlık hizalaması rozetlere 50px `line-height` miras bırakmayacak şekilde sınırlandırılır; AJAN, Bağlı ve ADMIN rozetleri kompakt kalır.
- Bilge Yolaç AJAN rozeti ile Yönetici Paneli alt sayfalarındaki ADMIN rozetleri açık temada aynı kırmızı pill rozet diliyle gösterilir; davranış ağırlıklı olarak `www/css/chat_header.css`, `www/css/claude_code.css` ve `www/css/admin_analytics.css` içinde korunur.
...
- API anahtarı başlangıç/onboarding akışı, kurum anahtarı mevcutsa kullanıcıya “Kişisel API Key” veya “Varsayılan Kurum Anahtarı” seçeneklerini sunar; kurum anahtarı yoksa yalnızca kişisel anahtar yolu gösterilir. Bu ekran Yapılandırma sayfasından tekrar açılabilir, kişisel veya varsayılan anahtar değeri tarayıcıya gönderilmez ve yalnızca gizli olmayan bastırma tercihi kullanıcı etiketine özgü ayrı `localStorage` anahtarında (`api_key_onboarding_suppressed.<etiket>`) saklanır; farklı kullanıcıların sekmeleri ortak bir nesneyi okuyup yazarak birbirinin kaydını ezmez. Etiket, kanonik kullanıcı adının kısaltılmış SHA-256 özetidir (`api_key_pref_user_tag()`); sunucu tercihi kimlik hazır olunca bu etiketle ister, böylece aynı tarayıcı profilini kullanan başka kullanıcı önceki kullanıcının tercihini devralmaz. Etiket yoksa bastırma uygulanmaz. Modaldaki "Bu ekranı bir daha gösterme" kutusu seçim yapılana dek bekler: yalnızca işaretliyken "Kurum Anahtarı ile Devam Et" seçimi tercihi o kullanıcı için yazar, "Daha Sonra Karar Ver" hiçbir şey yazmaz. Tarayıcı bayrağı etiketiyle bildirir; sunucu yalnız geçerli kullanıcının etiketini taşıyan yanıtı kabul eder ve aynı oturum başka kullanıcıya geçerse onboarding kararını yeni kullanıcı için yeniden verir. Sunucu tercih bayrağını en fazla 6 saniye bekler ve yanıt gelene kadar saniyede bir yeniden ister (Zengin Deneyim açılışında tarayıcı meşgulken bayrak geç gelebiliyordu); bekleme penceresi kimlik düşünce ya da sahip değişince sıfırlanır. Kurum seçimi hatırlandığında bayrak `"default"` değeriyle saklanır ve kayıtlı kişisel anahtarın önüne geçer; kişisel anahtar kaydı bunu `"1"`e (yalnız bastırma) indirir; işaretsiz kurum seçimi (`"clear"`) tercihi siler. Kurum anahtarı varken kayıtlı kişisel anahtar tercih gelene (ya da tolerans dolana) dek aday olarak tutulur ve oturuma açılmaz. Modal kutusunun bekleyen değeri modal jetonu ve kullanıcı etiketiyle bildirilir; tarayıcı yazım onayları (`{ok, tag, nonce}`) yalnız bekleyen yazımın jetonu ve geçerli kullanıcının etiketiyle kabul edilir. Gözlemci kimliği yoklamaz: `mergen_session_identity_signal()` reaktif oturum kimlik sinyaline bağlıdır; aynı oturumda kullanıcı değişince (`mergen_session_owner_transition()`) kullanıcıya bağlı oturum depoları ve kişisel anahtar temizlenir.
...
- Sidebar alt kullanıcı paneli; canlı kimlikten gelen ad/avatar, Departman bilgisi, tema düğmesi, sürüm bilgisi ve SSO çıkış kısayolunu tek satırda görünür tutar; ilk render gecikmeleri statik iskelet görünümüyle karşılanır. Saf görünüm/karar yardımcıları (baş harf üretimi, avatar URL çözümü, Departman → departman → department seçim sırası, tema anahtarı, kontrol satırı ve kullanıcı rozeti HTML'i) `R/helpers_sidebar_user_display.R` içindedir ve modülden önce yüklenir; `R/module_sidebar_user_panel.R` UI kabuğu, logout olayı ve server render orkestrasyonuna odaklanır. Bu sınır `tests/testthat/test-sidebar-user-display-split-contract.R` ile korunur.
...
- Derin uzay açılış ekranı UI/sunucu olarak ayrıldı: `R/module_startup_screen_ui.R` (`createStartupScreenUI()` + saf `.startup_*()` yapıcıları; üç deneyim-modu kartı tek veri-odaklı `.startup_mode_card()` ile üretilir, üretilen tag ağacı birebir korunur) `R/module_startup_screen.R` (gözlemciler + `apply_experience_mode`) öncesinde yüklenir. `ui.R` içindeki `createStartupScreenUI()` çağrısı UI dosyasının manifestte önce yüklenmesine dayanır. Bütçeler: UI 430/16, sunucu 380/7 (`test-maintainability-ratchet.R`); sözleşme `test-startup-screen-ui-refactor-contract.R`. Modül 740 → 358 satıra indi.
- Görsel oluşturma UI/HTML render katmanı IO/üretim runtime'ından ayrıldı: `R/module_image_generation.R` (runtime IO/üretim/çeviri + `generate_image` worker-export globali + `IMAGE_SIZE_OPTIONS`/`get_image_web_url`) `R/module_image_generation_ui.R` (saf UI/HTML render: `imageSettingsUI`, `imageChatControlsUI`, `render_*`) öncesinde yüklenir — UI yapıcıları runtime sembollerini çağrı anında çözdüğü için bağımlılık-önce sıra. UI render yapıcıları async görsel worker'ında koşmaz; `generate_image` iç çağrıları runtime dosyasında kaldığı için worker globals çözümü değişmedi. Modül 730/22 → 545/17; UI dosyası 194/5. Sözleşme `test-image-generation-ui-refactor-contract.R`.
...
- Frontend bölge katmanı VERİ + DOĞRULAYICI olarak ayrıdır: `R/config_ui_asset_zones.R` yalnızca veri (zone + manifest dışı sahiplik haritaları), bölge çözümleme ve bölümleme (partition) doğrulama API'si (saf fonksiyonlar) `R/config_ui_asset_zone_validators.R` içindedir ve manifestte veriden hemen sonra yüklenir. Bu, `config_source_manifest.R` (veri) + `bootstrap_source_manifest.R` (doğrulayıcı) deseniyle aynıdır; ayrım en büyük runtime dosyasını (777 satır) iki odaklı dosyaya indirir ve `tests/testthat/test-ui-asset-zone-validators-split-contract.R` ile dondurulur.
...
- Varlık manifesti de aynı VERİ + DOĞRULAYICI desenini izler, ayrıca bir RENDER katmanı ekler: `R/config_ui_assets.R` SADECE VERİ (CSS/JS grupları, ertelenmiş grup listesi, render planı, JS/CSS sıra kuralları, `ui_asset_flatten_groups`) ve yükleme sırasının TEK sahibidir; saf çözümleyici/doğrulayıcı API'si (`ui_asset_all_css/js`, `ui_asset_validate*`, `ui_asset_render_plan_*`, `ui_asset_deferred_js_paths`, `ui_asset_public_root`) `R/config_ui_asset_validators.R` içindedir; htmltools etiket render katmanı (`ui_asset_tags`, `ui_asset_css_tags`, `ui_asset_js_tags`, `ui_asset_css_tag`, `ui_asset_script_tag`) `R/config_ui_asset_tags.R` içindedir. Manifest sırası VERİ → DOĞRULAYICI → RENDER'dır ve doğrulayıcı/etiket fonksiyonları veriyi çağrı anında çözer. Bu ayrım en büyük runtime dosyasını 690 satırdan VERİ-odaklı 425 satıra indirdi (küresel `MERGEN_TEST_MAX_FILE_LINES` 690 → 687), çıktıyı byte-birebir korudu (golden + HEAD karşılaştırması) ve `tests/testthat/test-ui-asset-config-split-contract.R` ile dondurulur. Doğrulayıcı/etiket fonksiyonunu çalıştıran izole testler üç dosyayı da kaynak almalıdır.

### Derin Uzay Giriş Sahnesi Notları
- Deep Space giriş sahnesi render ve lifecycle sorumlulukları bakım sınırlarını korumak için dört dosyaya ayrılmıştır: `www/js/deep_space_intro.js` (sahne orkestrasyonu), `www/js/deep_space_intro_earth_shader.js` (Dünya shader ayarları), `www/js/deep_space_intro_solar.js` (Güneş/flare/halo/bloom) ve `www/js/deep_space_intro_lifecycle.js` (timeout/rAF/auto-init cleanup sınırı).
- `R/config_ui_assets.R` içindeki yükleme sırası korunmalıdır: önce `js/deep_space_intro_earth_shader.js`, sonra `js/deep_space_intro_solar.js`, sonra `js/deep_space_intro_lifecycle.js`, en son `js/deep_space_intro.js`.
...
- Çalışma zamanı CSS/JS varlıkları `R/config_ui_assets.R` üzerinden açık ve sıralı biçimde yüklenir; CDN, bundle/minify veya gizli kaynak yükleme kullanılmaz. Uzun `page` CSS ve `deferred` JS listeleri named alt bölümlere ayrılmıştır, ancak bu alt bölümler `ui_asset_flatten_groups()` ile adsız vektöre indirildiği için dış sözleşme, tag üretimi, `defer` planı ve asset yükleme sırası değişmez.
...
- Bu sınır `tests/testthat/test-ui-asset-manifest-contract.R`, `tests/testthat/test-frontend-selector-contract.R`, `tests/testthat/test-frontend-maintainability-ratchet.R` ve `tests/testthat/test-maintainability-ratchet.R` ile korunur. Asset manifest bölümleme refactor'ında `test-ui-asset-manifest-contract.R`, `ui_asset_css_groups$page` ve `ui_asset_js_groups$deferred` vektörlerini birebir dondurarak sectioning'in runtime sıra değişikliği üretmemesini doğrular.
- Açık tema alan dosyalarının sırası korunmalıdır: `theme_light_core.css` sonrasında `welcome -> chat -> modals -> bilge_yolac -> personalization -> pages` zinciri yüklenir. Aynı seçici birden fazla tema dosyasında tanımlanamaz (tek-tanım sözleşmesi); kaldırılan eski override katman adları tombstone'ludur ve geri getirilemez (`tests/testthat/test-theme-light-modular-contract.R`).
...
- Manifestteki her CSS/JS varlığı `R/config_ui_asset_zones.R` içinde tam olarak bir frontend bölgesine atanır; manifest dışı bilinçli varlıklar (`app_loading*`, `admin_analytics.css`, smoke dosyaları) `ui_asset_unmanifested_ownership` kaydında gerekçesiyle sahiplenilir. Sahipsiz frontend dosyası `tests/testthat/test-ui-asset-zones-contract.R` testini düşürür. AI Uzman frontendinde `js/ai_expert_manager.js` yalnız durum makinesi/API export sınırıdır; `js/ai_expert_handlers.js` Shiny özel mesaj handler'larını ve stop-button binding'ini taşır ve manifestte manager'dan hemen sonra yüklenmelidir. Deep Space frontendinde `js/deep_space_intro_lifecycle.js`, stale timeout/rAF ve DOMContentLoaded auto-init sahipliğini taşır; `js/deep_space_intro.js` bu helper üzerinden cleanup yapmalıdır. Modern welcome frontendinde `js/modern_welcome_handler.js`, `initModernWelcome` Shiny mesajı, retry timer temizliği, görünür DOM/dependency readiness kontrolü ve welcome video/neural/greeting init lifecycle sahipliğini taşır; `js/shiny_message_handlers.js` genel toast/scroll/follow-up/search/storage/quick-action köprüsü olarak kalır.
...
- Tarayıcı smoke zorlaması: `MERGEN_BROWSER_BIN` açıkça ayarlanmış ortamlarda `tests/scripts/ai_browser_ux_smoke.R` require modunu otomatik açar; sessiz SKIP devre dışıdır ve kullanılamayan binary yolu erken hata verir. `MERGEN_REQUIRE_BROWSER_UX_SMOKE=true` da aynı şekilde browser UX smoke'u bloklayıcı yapar ve gerçek `UX_SMOKE_DONE:PASS` kanıtı gerektirir. `MERGEN_BROWSER_UX_BASE_URL` set edildiğinde runner kendi geçici Shiny child process'ini başlatmaz; o base URL'de zaten çalışan app'in `/smoke/ux-smoke.html` rotasını test eder. Kilitli Windows/VDI VM'lerinde mandatory proof için önerilen akış: app'i ayrı PowerShell penceresinde `http://127.0.0.1:28081` üzerinde açık tutmak, ikinci pencerede `MERGEN_BROWSER_UX_BASE_URL=http://127.0.0.1:28081` ve `MERGEN_REQUIRE_BROWSER_UX_SMOKE=true` ile `tests/scripts/run_vm_evidence_gate.R` çalıştırmaktır.
...
### VM evidence gate ve isolated testthat milestone'u

`tests/scripts/run_vm_evidence_gate.R`, VM kanıtını `artifacts/vm-evidence/<timestamp>/evidence.json` altında toplayan tek tekrarlanabilir evidence gate'tir. 2026-06-12 Windows VM milestone koşumunda `Toplam: 13 passed, 0 failed, 0 skipped` sonucu alındı; `full_testthat`, `browser_ux_smoke`, `vm_preflight_real` ve `db_encoding_preflight` adımları `PASSED` oldu. Örnek artifact: `artifacts/vm-evidence/20260612-211836/evidence.json`. Bu sonuç; tam izole testthat, VM preflight, DB encoding preflight ve mandatory external-app browser UX smoke proof'un aynı koşumda geçtiğini gösterir. Sadece `evidence.json` içinde `passed` görünen adımlar için kanıt sayılır; uzun süreli üretim yükü veya manuel kırılgan-akış QA'sı yerine geçmez.
...
- **Opsiyonel konsol tanı probe’u:** `www/js/console_error_probe.js` isteğe bağlı bir tanılama yardımcısıdır; `R/config_ui_assets.R` içinde diagnostics JS grubunda, normal app JS’lerinden önce yüklenir. Varsayılan davranışta sessizdir. Yalnızca `localStorage.setItem("MERGEN_DEBUG_CONSOLE_ERRORS", "1")` açıldığında log üretir; kapatmak için `localStorage.removeItem("MERGEN_DEBUG_CONSOLE_ERRORS")` kullanılır. Probe, resource yükleme hatalarını `MERGEN_DEBUG_RESOURCE_ERROR`, runtime/JS hatalarını `MERGEN_DEBUG_ERROR` veya `MERGEN_DEBUG_PROMISE` olarak görünür kılar. Hata gizlemek için değil, sadece teşhis içindir.
- **Güncel tarayıcı konsol durumu:** Söyleşi Geçmişi tarih aralığı artık Shiny `dateRangeInput` / `bootstrap-datepicker` yerine tarayıcı yerel tarih alanları ve `www/js/history_date_range.js` köprüsüyle çalışır. Bu değişiklik bootstrap-datepicker deprecation uyarılarını, ilk köprü sürümündeki `Shiny.setInputValue` uyumsuzluk hatasını ve önceki DevTools gruplu `<other>` / “1 error” artığı notunu ortadan kaldırmıştır. Köprü, Shiny input güncellemesini `Shiny.setInputValue` varsa onu tercih edip eski istemciler için `Shiny.onInputChange` fallback’iyle güvenli biçimde sürdürür. Güncel kabul ölçütü: uygulama açılışında Console’da canlı hata/uyarı olmamalı, Network’te failed request olmamalı ve smoke probe resource/runtime hatası üretmemelidir; tekrar üretilebilir kaynak URL’i, stack trace, failed request veya bozuk DOM davranışı görülürse gerçek regresyon olarak ele alınmalıdır.
...
Geri Bildirim Analizi sayfasında public UI shell, sekme UI üretimi ve etiket sayımı `R/helpers_admin_geri_bildirim.R` içinde tutulur. SQL sorgu paketi ve enjekte edilebilir veri çekim yardımcısı `R/helpers_admin_geri_bildirim_queries.R` dosyasına ayrılmıştır. Inline highcharter/DT render fonksiyonları (13 output) `R/module_admin_geri_bildirim_outputs.R` içindeki `admin_gb_outputs(output, gb_data, etiket_sayilari)` fonksiyonuna BİREBİR çıkarılmıştır (davranış değişmedi); kullanıcı ve detay DT tablolarının görünüm verisi ise `R/helpers_admin_geri_bildirim_output_tables.R` içindeki saf hazırlama yardımcılarında tutulur. `R/module_admin_geri_bildirim.R` artık yalnızca Shiny server orkestrasyonu, refresh/reactive veri akışı, sekme yönlendirici ve `admin_gb_outputs(...)` çağrısına odaklanır (760 → 55 satır). Aynı desen Yanıt Geri Bildirimi Analizi için de uygulanır: `R/module_admin_yanit_analizi_outputs.R` (`admin_yanit_outputs(output, ya_data, etiket_sayilari, refresh)`, 753 → 106 satır). Bu ayrımlar at-budget modülleri 800 satır eşiğinin çok altına indirir ve `test-admin-geri-bildirim-refactor-contract.R`, `test-admin-geri-bildirim-query-contract.R`, `test-admin-geri-bildirim-outputs-behavior.R` (golden grafik sözleşmesi), `test-admin-geri-bildirim-output-tables-behavior.R` (saf tablo veri hazırlama), `test-admin-yanit-analizi-refactor-contract.R`, `test-admin-yanit-analizi-outputs-behavior.R`, `test-source-manifest-contract.R` ve `test-maintainability-ratchet.R` ile korunur.
...
Son server runtime sözleşme refactor’ında düşük seviyeli runtime doğrulama ve auth-ready callback hata sarmalama yardımcıları R/helpers_server_runtime_contracts.R dosyasına ayrılmıştır. R/server_runtime_context.R runtime context orkestrasyonuna, context attach/require yardımcılarına ve modül kayıt yardımcılarına (serverRuntimeAttachModule/GetModule/ExposeSessionData) odaklanır. SSO sonrası (post-auth) + yenilenebilir modül wiring katmanı — serverRuntimeOnSsoAuthReady(), serverRuntimeRefreshModuleOnSsoAuthReady() ve serverRuntimeAttachRefreshableModule() — odaklı bir dosyaya, R/server_runtime_auth_ready.R, ayrılmıştır (manifestte context'ten SONRA, function_slot'tan ÖNCE; davranış byte-birebir korundu, context 687/17 -> 503/13, auth_ready 242/4). serverRuntimeOnSsoAuthReady() içinde SSO zaten hazır olduğunda çalışan immediate yol ile observer üzerinden çalışan yol aynı callback sarmalayıcısını kullanır; böylece File Manager, Görsel Galerisi ve benzeri refreshable modüllerde auth-ready davranışının iki farklı kolda sessizce ayrışması engellenir. Bu kaynak sırası ve davranış sözleşmesi test-server-runtime-context.R, test-server-runtime-auth-ready-split-contract.R, test-source-manifest-contract.R ve test-maintainability-ratchet.R ile korunur.
...
Chat/mesaj OKUMA tarafında sohbet-okuma SQL'i `R/helpers_db_chat_read_queries.R` saf üretici dosyasına ayrılmıştır. Bu dosya `db_chat_preview_query_sql()`, `db_chat_list_summary_query_sql()`, `db_chat_list_full_query_sql(with_reasoning)`, `db_chat_messages_query_sql(with_reasoning, scoped)`, `db_chat_messages_batch_query_sql(placeholder, with_reasoning, scoped)` ve `db_history_rows_query_sql(placeholder, scoped)` üreticilerini içerir; bağlantı açmaz, normalize/encode etmez, reaktif durum tutmaz. `ReasoningContent` (with_reasoning) ve kullanıcı-kapsamlı (`AND c.UserID = ?`) varyantlar koşullu parçalarla DRY hale getirildi ancak çıktı eski satır içi SQL ile **byte-birebir aynıdır** (extraction öncesi 14 SQL string'i stub'lı bağlantı ile yakalanıp golden olarak doğrulandı; tüm string'ler `identical()`). Kullanıcı izolasyonu (`c.UserID = ?`) ve soft-delete (`c.IsDeleted = 0`) güvenlik filtreleri bu üreticilerde korunur. `R/helpers_db_chat_readers.R` artık bağlantı edinme, üretici çağrısı, sorgu çalıştırma ve `normalize_db_read_visible_frame()`/formatlama orkestrasyonuna odaklanır ve **680/13 → 522/13** satıra inmiştir; SQL string'leri ASCII'dir ve encoding sınırına dokunmaz. Bu ayrım `test-db-chat-read-queries-contract.R` (yapısal + saf üretici davranışı), güncellenen `test-db-user-scope-contract.R` (güvenlik filtresi artık üretici dosyasında doğrulanır) ve dosya bazlı ratchet bütçeleriyle (`helpers_db_chat_readers.R` 560/14, `helpers_db_chat_read_queries.R` 180/8) korunur.
...
Proje/Kaynak Analizi sayısal köken mimarisi ANLAMSAL OLGU REFERANSI modeline geçirilmiştir. Model artık analitik sayıyı kendisi yazmaz: pakette her sayının yanında o sayının yuva jetonu (`{{fact:<olgu_kimligi>}}`) basılır, model sayının geçmesi gereken yere yalnızca jetonu koyar ve kanonik değeri (binlik/ondalık ayracı, yüzde, para birimi ve birim dâhil) R yerleştirir. Böylece üretimde ölçülen asıl arıza — doğru sayı yazıldığı hâlde gizli `[fact:...]` alıntı yerleşimi kuralına uyulmaması, yani `missing_fact_marker` baskınlığı — yapısal olarak ortadan kalkar; düzyazıdaki serbest sayıları olgulara GERİYE DOĞRU eşleştiren işaret gruplama, sözcük mesafesi, tolerans ve birim/toplulaştırma çıkarımı katmanları tamamen KALDIRILMIŞTIR. Dosya ayrımı: `R/helpers_pk_fact_reference.R` (yuva söz diziminin tek sahibi, olgu indeksi, tek geçişli değer basımı, iddia düzeyinde nötrleme ve `block` kipi cümle ayıklaması), `R/helpers_pk_fact_reference_scan.R` (beklenmeyen model sayısı için bilinçli olarak DAR yapısal tarama ve güvenilir istek girdisi anahtarları) ve `R/helpers_pk_numeric_provenance.R` (kip çözümleme, uygulama katmanı ve telemetri). Kullanıcının kendi kriteri (`"30 günden az"`) veritabanı olgusundan AYRI tutulur; `pk_packet_request_facts()` bu değerleri `request_input` türünde açıkça taşır. Telemetri protokol/yapı bulgularını (`malformed_reference`, `legacy_reference`, `model_numeric_literal`, `duplicate_numeric_literal`, `render_degraded`) esasa ilişkin güven bulgularından (`unknown_fact`, `unavailable_fact`, `ambiguous_fact`, `unit_conflict`) AYRI raporlar; çözülmüş bir yuvanın yanındaki yinelenen sayı ya da eski işaret KURTARILABİLİR sayılır ve `warn` notuna girmez. Olgu taşıyan bekleyen kayıt görünür akışı HER kipte erteler (ham delta yuva jetonu taşır); kapalı başarısız reddetme yalnızca `block` kipine aittir. Çözülemeyen bir referansın asla değer basmaması, yetkisiz kimliğin asla çözülmemesi ve kullanılamaz bir olgu için sayı uydurulmaması HER kipte (`off`/`log`/`warn`/`block`) geçerlidir; kip yalnızca kurtarılabilir bir sorunun kullanıcıya ne kadar gösterileceğini belirler ve `log` işletimsel güvenlik ağı olarak yanıtı değiştirmeden teslim eder. Bu sözleşme `test-pk-fact-reference-behavior.R`, `test-pk-fact-reference-scan-behavior.R`, `test-pk-fact-reference-hardening-behavior.R` ve `test-pk-numeric-provenance-contract.R` ile korunur.

Fonksiyon slot yardımcısı da 25+ fonksiyonlu dosya sayısını artırmamak için ayrı dosyada tutulmuştur. Dosya bazlı kazanımlar ayrıca korunur; örneğin Admin Geri Bildirim Analizi ayrımı (inline renderer'lar `*_outputs()` dosyasına çıkarıldıktan sonra) artık `MERGEN_TEST_MAX_ADMIN_GERI_BILDIRIM_LINES = 90` ve `MERGEN_TEST_MAX_ADMIN_GERI_BILDIRIM_FUNCTIONS = 3` eşikleriyle korunur; yeni SQL helper dosyası için `R/helpers_admin_geri_bildirim_queries.R` bütçesi 260 satır ve 3 fonksiyon, çıkarılan render dosyası `R/module_admin_geri_bildirim_outputs.R` için 670 satır / 5 fonksiyon, tablo hazırlama helper dosyası `R/helpers_admin_geri_bildirim_output_tables.R` için 145 satır / 5 fonksiyon olarak izlenir. Admin Yanıt Analizi ayrımı ise `MERGEN_TEST_MAX_ADMIN_YANIT_ANALIZI_LINES = 140` ile, çıkarılan `R/module_admin_yanit_analizi_outputs.R` ise 710 satır / 5 fonksiyon bütçesiyle korunur. Son Bilge Yolaç setup ve çalışma yaşam döngüsü extraction kazanımı ayrıca dosya bazlı ratchet ile korunur: `R/module_claude_code.R` için varsayılan üst sınır `MERGEN_TEST_MAX_CLAUDE_CODE_LINES = 799` olarak belirlenmiştir. Ek olarak mevcut büyük ve fonksiyon yoğun dosyalar için dosya bazlı bütçeler eklenmiştir; böylece genel skor değişmese bile `config_api.R`, `helpers_llm_worker.R`, `helpers_claude_code.R`, `helpers_db_chat_mutations.R`, azaltılmış `helpers_database.R` bütçesi, `helpers_chartlab.R` ve benzeri dosyaların sessizce büyümesi engellenir. Bu değer, mevcut 954 satırlık durumu korurken küçük güvenli düzeltmelere sınırlı hareket alanı bırakır. Bu sınır ancak yeni maintainability raporu daha iyi bir taban çizgisi gösterdiğinde sıkılaştırılmalıdır. LLM worker payload ayrımı da dosya bazlı ratchet ile korunur: `R/helpers_llm_worker.R` için varsayılan üst sınır `MERGEN_TEST_MAX_LLM_WORKER_LINES = 860`, `R/helpers_llm_worker_payload.R` için ise `MERGEN_TEST_MAX_LLM_WORKER_PAYLOAD_LINES = 320` ve `MERGEN_TEST_MAX_LLM_WORKER_PAYLOAD_FUNCTIONS = 12` olarak belirlenmiştir.
...
`send_message()` üç terminal LLM dalını artık simetrik ctx tabanlı handler'lara yönlendirir: gerçek SSE (`handle_true_streaming_mode`, `R/server_handler_true_streaming.R`), non-streaming (`generate_non_streaming_stoppable_fn`) ve TTS açık streaming. Önceden yalnızca TTS açık streaming dalı `send_message` gövdesinde satır içi promise zinciri olarak kalıyordu; bu dal `R/server_handler_streaming_tts.R` dosyasındaki `handle_streaming_tts_mode(ctx)` işleyicisine birebir taşındı (davranış değişmedi). Sonuç: `R/server_send_message.R` 694/14 → 590/9'a indi, yeni handler 172/6 oldu ve repo genelinde en büyük dosya satırı 694 → 690'a düştü; küresel `MERGEN_TEST_MAX_FILE_LINES` tabanı da 694 → 690 olarak sıkılaştırıldı. Handler'ın stale-istek/durdurma kararları `mergen_send_message_request_state(active_request_id, req_id, stop_generation)` üzerinden istek kimliği kapsamlı kalır: başarı yolu `simulate_streaming_stoppable_fn(...)`, başarısız/reddedilmiş yol `abort_send_message(...)`, durdurulmuş yol `cleanup_send_message()` çağırır. `R/config_source_manifest.R` handler'ı `R/server_handler_true_streaming.R`'den sonra ve `R/server_send_message.R`'den önce yükler. Bu kazanım yapısal ayrım ve deterministik promise zinciri davranışı (başarı/başarısız/durdurulmuş/reddedilmiş yollar) açısından `test-server-handler-streaming-tts-contract.R` ile korunur.
...
- `mergen_langflow_parse_prose_sources()` sondaki "Kaynak(lar/ça):" başlığını (BÜYÜK/küçük harf duyarsız; `KAYNAK:`/`KAYNAKÇA:` dahil) ve altındaki `(1) ...`, `1) ...`, `1. ...` biçimli numaralı girişleri ayrıştırır; düzyazıyı bölümden ayırır. Girişler arasındaki boş satırlar listeyi KESMEZ (bazı akışlar numaralı girişler arasına boşluk bırakır); liste yalnızca gerçek (boş olmayan) numarasız bir satırla sonlanır ve o satırdan sonraki metin ("Not:" gibi bir ibare) sessizce düşürülmez, düzyazıya geri eklenir. Bir giriş "Grup&&dosya.pdf (Sayfa 3)" veya "dosya.pdf, s. 3" gibi belge uzantısından SONRA gelen bir sayfa açıklaması taşıyorsa, `.langflow_strip_page_suffix()` bu eki uzantı doğrulamasından önce ayırır ve `page` alanını doldurur (ek uzantıdan önce gelen benzer metne yanlışlıkla uygulanmaz). URL/mutlak yol/sürücü/`..` gezinme girişleri ve belge uzantısı taşımayanlar reddedilir (kaynak uydurulmaz).
...
- `<sup>(n)</sup>` atıfları `[n]` biçimine çevrilir; öznitelikli açılış etiketi de yakalanır (`<sup class="citation">(1)</sup>`), sayılar yalnızca etiket İÇERİĞİNDEN alınır (öznitelik değerindeki rakamlar sayılmaz). Dönüşüm YALNIZCA (a) gerçek bir Kaynakça bloğu üretildiğinde VE (b) etiket içeriği "citation-şekilli" (tamamen parantezli rakam grupları, örn. `(1)`, `(1)(2)`, `(1, 2)`) olduğunda uygulanır; parantezsiz bare rakam (`m<sup>2</sup>` gibi sıradan üs/dipnot gösterimi) DOKUNULMADAN kalır. Markdown ham HTML'i kaçırdığından üstsimge etiketi erken indirgenir; render'da `citation_handler.js` `[n]` metnini `.citation-ref` üstsimge rozetine dönüştürür (tıklama, aynı mesajdaki `.kaynakca-entry[data-entry=n]` girişine yumuşak kaydırır ve kısa süre vurgular). Kaynak listesi 1..n sırasıyla numaralanmadıysa (örn. 2 atlanmışsa) atıflar pozisyona yeniden eşlenir; eşleşmeyen/aralık dışı atıflar etkisiz kılınır (düşürülür), yoğun yeniden numaralanan Kaynakça'da yanlış belgeye atıf yapılması önlenir.

Bu belgelerdeki dosya adları çoğunlukla `A&&B&&dosya.pdf` gibi DÜZ (dizin değil) `&&` ayraçlı gerçek adlardır; disk basename'i `&&` içerir. Kaynakça render'ında (`mergen_kaynakca_marker_html()`) yalnızca son parça (görünen dosya adı) tıklanabilir `.source-link` olur; önceki kategori parçaları `.kaynakca-breadcrumb` sınıfıyla soluk, `" - "` ile birleşmiş kırıntı yolu olarak gösterilir (uzun önek bağlantının altını çizmeden okunur kalır). `&&` içermeyen (yapısal) yollarda mevcut davranış korunur: başlık tıklanabilir. `data-filename` her durumda tam `&&` adını taşır; `handle_source_file_click()` bu adı ÖNCE belirleyici göreli adaylarla, yalnızca gerektiğinde indeksle çözer (sözleşme aşağıda). İndeks aşamasında (`search_file_in_folder()`) sıra ÖNCE tam basename eşleşmesi, SONRA `&&` ipucu-skorlu arama, SONRA aynı gövdeli Word eşdeğeri, SON ÇARE tüm `&&` parçalarını içeren en iyi aday (`.search_all_parts_contained()`); tam eşleşme önce denenir çünkü ipucu-skorlu arama yalnızca ipucunun SON parçasını basename sayar ve taban klasörde aynı son-parça adına sahip alakasız bir dosya varsa onu yanlışlıkla döndürebilir. Son çare parça-içerme araması yalnızca UZANTISIZ son parça için etkindir (opt-in); son parçanın bir uzantısı varsa (yaygın durum) ve atıflanan dosya diskte hiç yoksa, yalnızca alt-dize içerme skoruna güvenip alakasız benzer adlı bir dosyayı yanlışlıkla açmak yerine "bulunamadı" döner. Dosya indeksi (`.build_basename_index()`) `docx`, `docm`, `doc`, `pdf`, `pptx`, `ppt`, `xlsx`, `xls` uzantılarını kapsar.

Regresyon kapsamı: `tests/testthat/test-langflow-inline-sources-behavior.R` (üstsimge dönüşümü, citation-şekil kısıtı, düzyazı ayrıştırma, boş satır toleransı, imzalı işaretleyici yükseltme, kırıntı yolu render'ı, XSS kaçışı ve alt klasör/`&&` dosya çözümlemesi + tam-eşleşme-önce sırası).

#### Kaynak tıklama çözümleme sözleşmesi (başarım ve güvenlik)

Süreç Yönetimi / Uygulama Uzmanı kaynağına tıklamak, yüzlerce ilgisiz belge içeren bir model klasöründe bile SABİT sayıda dosya sistemi denetimiyle açılmalıdır. Kaynak kimliği zaten bilinir: imzalı Kaynakça işaretleyicisinin `yol=` alanı Langflow'un göreli dosya yoludur ve tarayıcıya yalnızca bu göreli ipucu (`data-filename`, `&&` ayraçlı) gider; mutlak sunucu yolu istemciye hiç ulaşmaz. Tıklamada sunucu ipucunu yeniden doğrular ve çözer (`R/helpers_preview.R`, `R/utils_file_index.R`):

1. **Kişisel kova** (yalnızca `personal` kapsamda): `resolve_uploaded_file(..., user_id = <tıklayan kullanıcı>)`. `model_bases` kapsamında (Ortak Oturum) bu adım HİÇ çalışmaz; küresel/çapraz kullanıcı indeksi hiçbir kapsamda kullanılmaz.
2. **Belirleyici göreli adaylar — TARAMA YOK** (`.preview_direct_rel_candidates()`, `.preview_probe_direct()`): TÜM model tabanlarında düz ad (`A&&B&&C.pdf`) ve iç içe yol (`A/B/C.pdf`) denenir.
3. **Önbellekteki indeks, tam ad — TARAMA YOK** (`.preview_index_lookup(..., "tam")`): her tabanın önbellekteki (taze, bayat ya da kısmi) indeksinde tam ad ve `&&` ipucu-skorlu arama. Daha önce bulunmuş bir kaynak böylece kısmi indeksle bile yeniden taranmadan açılır.
4. **Word eşdeğeri — TARAMA YOK**: ipucu PDF ise TÜM tabanlarda aynı gövdeli `.docx` > `.docm` > `.doc` belirleyici adayları, ardından önbellekteki indekste Word eşdeğeri ve parça-içerme. Tam PDF (herhangi bir tabanda) her zaman Word eşdeğerinden önce gelir; Word eşdeğeri yalnızca PDF atfına uygulanır (tür genişletilmez). Bu, üretimde "PDF atıflı kaynak diskte Word" durumunu da taramasız çözer.
5. **Taban başına EN FAZLA BİR sınırlı tarama**: yalnızca indeks yetkili değilse (yok/bayat ve geri çekilmede değil) `.build_basename_index()` tek kez tarar ve tam ad aranır; ardından TÜM tabanlarda Word eşdeğeri ve en son yalnızca dosya adıyla zayıf eşleşme AYNI indeks girdileriyle aranır. Tek bir tıklama bir tabanı ikinci kez tarayamaz; zayıf (yalnızca dosya adı) eşleşme, taranmamış başka bir tabandaki TAM dosyanın önüne geçemez.

**Güvenlik sınırları.** İpucu parçaları `.preview_hint_parts_safe()` ile süzülür: `..`/`.`, yol ayırıcı, mutlak yol, sürücü harfi ve HER KONUMDA `:` (NTFS alternatif veri akışı) ile kontrol karakterleri reddedilir. Hangi katmandan gelirse gelsin HER aday açılmadan önce `path_existing_variant()` ile var olan bir DOSYA (klasör değil) olarak ve `.preview_path_inside()` ile model tabanının İÇİNDE doğrulanır: önce sözcüksel olarak (`normalize_mcp_path()`, UNC biçimi korunur), sonra aday ve taban `normalizePath(mustWork = TRUE)` ile KANONİK biçime (bağlantı/reparse point çözülmüş) getirilerek; kanonik yol kök dışındaysa aday reddedilir ve açılan yol kanonik yoldur. Kök dışına çözülen indeks isabeti de reddedilir.

**İndeks ve geri çekilme.** İndeks taraması (`.file_index_scan_bounded()`) yalnızca AD listelemesi yapar: dizin başına tek listeleme (UNC'de boş dönen `list.files()` için paylaşılan `cc_scan_list_dir_bounded()` yedeği dahil), dosya başına `normalizePath()`/boyut okuması YAPILMAZ; bağlantı/reparse point dizinleri izlenmez, dosya bağlantıları indekse alınmaz, bağımlılık/önbellek ağaçları atlanır. Tam tarama `MCP_INDEX_TTL_MIN` (varsayılan 10 dk) boyunca yetkilidir; kısmi ya da başarısız tarama TTL ile önbelleğe alınmaz, önceki harita korunur ve `FILE_INDEX_SCAN_FAIL_BACKOFF_SEC` (30 sn) geri çekilmesi taramanın **BİTİŞ** anından ölçülür (başlangıç anından ölçülen damga, 40-70 sn süren UNC taramasının ardından aynı tıklamada taramayı yeniden başlatıyordu). Süre bütçesi işbirlikçidir: işletim sisteminde bloklanan tek bir dizin çağrısı bütçeyi aşabilir; bu yüzden tarama yalnızca 1-4. adımlar başarısız olduğunda çalışan istisnai yoldur. Başlangıçta `config_api.R` model tabanlarının indeksini önceden ısıtır.

Regresyon kapsamı: `tests/testthat/test-source-click-resolution-behavior.R` (taramasız belirleyici yol ve PDF -> Word, taban başına tek tarama, aşamaların indeks paylaşımı, bitiş anından ölçülen geri çekilme, kısmi indeksin taramasız yeniden kullanımı, yol geçişi/mutlak yol/ADS, kök dışı indeks isabeti, sembolik bağlantı, kişisel kova ve `model_bases` kapsamı, Türkçe adlar, UNC), `tests/testthat/test-preview-source-click-path-guard-behavior.R`, `tests/testthat/test-ortak-oturum-langflow-sources-behavior.R`.
...
Persona sistemi güncel kimlikler üzerinden yönetilir; önerilen kullanım `emre`, `selin`, `deniz`, `can` ve `ipek` persona kimlikleridir.
...
Özellikle worker tabanlı araç akışlarında (`Görsel Oluşturma`, `Excel Analizi`'nin MCP geçişi ve `Proje ve Kaynak Analizi`'nin SSE/non-streaming sentezi gibi), worker tarafında kullanılan yardımcı fonksiyonlar doğrudan görünmeyebilir. Bu nedenle yeni bir asenkron akış eklenirken ham `future_promise(...)` yerine `tracked_future_promise(...)` kullanılmalıdır. (Not: `Proje ve Kaynak Analizi` düşünmeli modelde artık canlı SSE Düşünce Akışı + işçi içi non-streaming güvenlik ağı kullanır; bk. `mergen_sql_analysis_stream_plan()` ve `call_local_llm_sse_worker`'daki `allow_non_streaming_fallback`.)
...
VOXCPM2_PREFIX_DEADLINE_MS=4000
