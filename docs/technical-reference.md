...
- API anahtarı başlangıç/onboarding akışı, kurum anahtarı mevcutsa kullanıcıya “Kişisel API Key” veya “Varsayılan Kurum Anahtarı” seçeneklerini sunar; kurum anahtarı yoksa yalnızca kişisel anahtar yolu gösterilir. Bu ekran Yapılandırma sayfasından tekrar açılabilir, kişisel veya varsayılan anahtar değeri tarayıcıya gönderilmez ve yalnızca gizli olmayan bastırma tercihi `mergen_settings.api_key_onboarding_suppressed_by_user` altında kullanıcı etiketiyle saklanır. Etiket, kanonik kullanıcı adının kısaltılmış SHA-256 özetidir (`api_key_pref_user_tag()`); sunucu tercihi kimlik hazır olunca bu etiketle ister, böylece aynı tarayıcı profilini kullanan başka kullanıcı önceki kullanıcının tercihini devralmaz. Etiket yoksa bastırma uygulanmaz. "Kurum Anahtarı ile Devam Et" seçimi bu tercihi o kullanıcı için otomatik yazar; kullanıcıya sonraki girişlerde tekrar sorulmaz. Sunucu tercih bayrağını en fazla 6 saniye bekler ve yanıt gelene kadar saniyede bir yeniden ister (Zengin Deneyim açılışında tarayıcı meşgulken bayrak geç gelebiliyordu).
