...
## Production-safe MB_* performans indeks envanteri (Temmuz 2026)

Bu bölüm, başarılı Wave 1-4 yayılımından sonra üretim ortamında güvenli olduğu gözlemlenen indeks setini kayıt altına alır. Daha önce yapılan tüm indeksleri tek seferde oluşturma girişimi geçersiz kılınmıştır ve önerilen rehber olarak değerlendirilmemelidir. Söz konusu girişim, çok sayıda indeksin eşzamanlı olarak oluşturulmasını ve yeni oluşturulan benzersiz bir oturum açma yolu indeks adayını içeriyordu. Oturum açma kesintisinin muhtemel nedeni; güvenli olmayan tek seferde yayılım, şema kilitlemesi ve/veya aşırı agresif benzersiz oturum açma yolu indeks denemesidir. Kök neden kesin olarak kanıtlanmamıştır.

| Tablo                     | İndeks                                   | Tür / benzersizlik / durum           | Anahtar kolonlar                                                     | INCLUDE kolonları                      | Amaç                                                                                                                                                                                                                                                                                                                                            |
| ------------------------- | ---------------------------------------- | ------------------------------------ | -------------------------------------------------------------------- | -------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `MB_Chats`                | `IX_MB_Chats_User_Active_Recent`         | NONCLUSTERED, benzersiz değil, etkin | `UserID` ASC, `IsDeleted` ASC, `CreateTimestamp` DESC, `ChatID` DESC | `ChatTitle`                            | Geçerli kullanıcının aktif/silinmemiş sohbet listesini ve en yeniye göre sıralamasını hızlandırır.                                                                                                                                                                                                                                              |
| `MB_Messages`             | `IX_MB_Messages_Chat_Order`              | NONCLUSTERED, benzersiz değil, etkin | `ChatID` ASC, `MessageOrder` ASC, `MessageID` ASC                    | `MessageType`, `MessageTimestamp`      | Mesajların yüklenmesini, sohbet bazında sıralanmasını, sohbet geçmişinin açılmasını ve `MAX(MessageOrder)+1` işlemini destekler.                                                                                                                                                                                                                |
| `MB_Messages`             | `IX_MB_Messages_Chat_Timestamp`          | NONCLUSTERED, benzersiz değil, etkin | `ChatID` ASC, `MessageTimestamp` DESC                                | —                                      | Son mesaj ve son etkinlik hesaplamalarını hızlandırır.                                                                                                                                                                                                                                                                                          |
| `MB_Feedback`             | `IX_MB_Feedback_User_Message`            | NONCLUSTERED, benzersiz değil, etkin | `UserID` ASC, `MessageID` ASC                                        | `FeedbackType`                         | Geri bildirim yükleme/kaydetme/silme arama yolunu destekler; güvenli yayılımda özellikle **benzersiz yapılmamıştır**.                                                                                                                                                                                                                           |
| `MB_Users`                | `IX_MB_Users_KullaniciAdi_Lookup`        | NONCLUSTERED, benzersiz değil, etkin | `KullaniciAdi` ASC                                                   | `UserID`, `KaynakAdi`, `LastLoginDate` | Oturum açma ve kullanıcı profili aramalarını hızlandırır. Bu açık performans indeksi özellikle **benzersiz değildir** ve `KullaniciAdi` üzerinde daha önceden var olan, adı `UQ__MB_Users__5BAE6A75C24F52F9` benzeri olan benzersiz nonclustered constraint/indeks ile birlikte bulunur; bu UQ nesnesi kaldırılmamalı veya değiştirilmemelidir. |
| `MB_Destek_Geri_Bildirim` | `IX_MB_Destek_Geri_Bildirim_User_Recent` | NONCLUSTERED, benzersiz değil, etkin | `UserID` ASC, `OlusturmaTarihi` DESC                                 | —                                      | Destek geri bildirimi kullanıcı geçmişini ve yönetici/kullanıcı tarafında son kayıt erişimini hızlandırır.                                                                                                                                                                                                                                      |
| `MB_Destek_Hata_Bildir`   | `IX_MB_Destek_Hata_Bildir_User_Recent`   | NONCLUSTERED, benzersiz değil, etkin | `UserID` ASC, `OlusturmaTarihi` DESC                                 | —                                      | Kullanıcıya özel hata bildirim geçmişini hızlandırır.                                                                                                                                                                                                                                                                                           |
| `MB_Destek_Hata_Bildir`   | `IX_MB_Destek_Hata_Bildir_Status_Recent` | NONCLUSTERED, benzersiz değil, etkin | `Durum` ASC, `OlusturmaTarihi` DESC                                  | `Oncelik`, `UserID`                    | Yönetici tarafında hata bildirimi durum/son kayıt pano erişimini hızlandırır.                                                                                                                                                                                                                                                                   |
| `MB_ClaudeCode_Sessions`  | `IX_MB_ClaudeCode_Sessions_User_Recent`  | NONCLUSTERED, benzersiz değil, etkin | `UserID` ASC, `IsDeleted` ASC, `LastRunAt` DESC, `CreatedAt` DESC    | —                                      | Bilge Yolaç oturum listesi ve son etkinlik erişimini hızlandırır.                                                                                                                                                                                                                                                                               |
| `MB_ClaudeCode_Runs`      | `IX_MB_ClaudeCode_Runs_Session_Order`    | NONCLUSTERED, benzersiz değil, etkin | `ClaudeSessionRecordID` ASC, `RunOrder` ASC                          | —                                      | Bilge Yolaç çalıştırma zaman çizelgesinin yüklenmesini hızlandırır.                                                                                                                                                                                                                                                                             |

İlk sekiz indeks için güvenli Wave 1-4 betiği [`sql/2026-07-safe-mb-performance-indexes.sql`](sql/2026-07-safe-mb-performance-indexes.sql) dosyasıdır. İki Bilge Yolaç indeksi ise idempotent Bilge Yolaç kurulum betiği olan [`sql/2026-07-bilge-yolac-sessions.sql`](sql/2026-07-bilge-yolac-sessions.sql) dosyasında kalmaya devam eder.

Güvenli Wave 1-4 yayılımına özellikle eklenmeyen önceki adaylar şunlardır: `IX_MB_Usage_Log_User_Model`, `IX_MB_Usage_Log_Chat_Message`, `IX_MB_Destek_Geri_Bildirim_Recent`, `IX_MB_Destek_Hata_Bildir_Recent`, `IX_MB_Destek_Hata_Bildir_Status_Priority` ve `IX_MB_Users_KullaniciAdi` adlı yeni oluşturulacak herhangi bir `UNIQUE` indeks. Query Store kanıtı, gerçek execution plan’lar ve DBA onayı olmadan bunlar eklenmemelidir.