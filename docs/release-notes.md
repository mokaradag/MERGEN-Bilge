...
- **Kalıcı silme:** Arşivlenmiş kartlara ikinci onay adımı isteyen "Kalıcı Sil" eylemi eklendi. `cc_db_hard_delete_session()` oturumu ve tüm çalıştırmalarını tek işlemde, sahiplik doğrulaması + `UserID` sınırıyla siler (geri alınamaz). Silme/geri yükleme normal arşiv akışıyla çakışmaz.
