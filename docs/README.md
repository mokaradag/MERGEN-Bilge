...
| [`pk-phase3b-operator-runbook.md`](pk-phase3b-operator-runbook.md) | Faz 3b metadata üreticisinin Windows VM operasyon kılavuzu: `describe`/`sample`, sağlık raporu, resume/kilit davranışı, güvenli `SET NOCOUNT ON`, scalar `DECLARE`, yerel `#temp` staging, yerel-temp indeksleri, final CTE ve `DROP TABLE IF EXISTS` uyumluluğu. |
| [`pk-sql-readonly-gate.md`](pk-sql-readonly-gate.md) | Proje ve Kaynak Analizi SQL salt-okunur kapısının nihai sözleşmesi: fail-closed kurallar, tam `SET NOCOUNT ON`, güvenli scalar `DECLARE`, doğrulanmış yerel `#temp` staging ve indeksler, metadata-only CTE dönüşümü ve korunmuş ratchet'ler. |
