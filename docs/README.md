...
| [`pk-sql-readonly-gate.md`](pk-sql-readonly-gate.md) | Proje ve Kaynak Analizi'nin ortak fail-closed SQL kapısı: kabul/red örnekleri, tam `SET NOCOUNT ON;` + tek salt-okunur sorgu istisnası, D23 güvenlik sınırları ve test sözleşmesi. |
| [`pk-phase3b-operator-runbook.md`](pk-phase3b-operator-runbook.md) | Proje ve Kaynak Analizi metadata üreticisinin Windows VM iş akışı, `health.txt` yorumlama, güvenli `sample` kürasyonu ve salt-okunur SQL kapısı. SQL Server uyumluluğu için yalnızca tam `SET NOCOUNT ON;` + tek salt-okunur `SELECT`/`CTE+SELECT` batch'i kabul edilir; sorgu metni değiştirilmez. |
