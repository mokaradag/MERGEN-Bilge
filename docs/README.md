...
| [`pk-phase3b-operator-runbook.md`](pk-phase3b-operator-runbook.md) | Faz 3b metadata üreticisinin Windows VM operasyon kılavuzu: `describe`/`sample`, sağlık raporu, resume/kilit davranışı, güvenli `SET NOCOUNT ON` ve yerel `#temp` batch uyumluluğu. |
| [`pk-sql-readonly-gate.md`](pk-sql-readonly-gate.md) | Proje ve Kaynak Analizi SQL salt-okunur kapısının nihai sözleşmesi: fail-closed kurallar, tam `SET NOCOUNT ON` istisnası, doğrulanmış yerel `#temp` staging, metadata-only CTE dönüşümü ve korunmuş ratchet'ler. |
...
- Faz 3b SQL uyumluluğu **genel bir çok-ifade izni değildir**. Yalnızca tam `SET NOCOUNT ON;` öneki ve [`pk-sql-readonly-gate.md`](pk-sql-readonly-gate.md) içinde kanıtlanan yerel `#temp` analitik batch biçimi kabul edilir; yasaklı SQL ve metadata-generator DB-yazma ratchet'leri gevşetilmez.
