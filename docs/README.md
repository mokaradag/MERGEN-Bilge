...
| Faz 3b metadata üreticisini çalıştırıyorum | [`pk-phase3b-operator-runbook.md`](pk-phase3b-operator-runbook.md) | [`pk-sql-readonly-gate.md`](pk-sql-readonly-gate.md), [`proje-kaynak-analizi-master-plan.md`](proje-kaynak-analizi-master-plan.md) |
...
| [`pk-phase3b-operator-runbook.md`](pk-phase3b-operator-runbook.md) | Faz 3b metadata üreticisinin Windows VM operasyon kılavuzu: `describe`/`sample`, sağlık raporu, resume/kilit davranışı, güvenli `SET NOCOUNT ON`, yerel `#temp` staging, yerel-temp indeksleri, final CTE ve `DROP TABLE IF EXISTS` uyumluluğu. |
| [`pk-sql-readonly-gate.md`](pk-sql-readonly-gate.md) | Proje ve Kaynak Analizi SQL salt-okunur kapısının nihai sözleşmesi: fail-closed kurallar, tam `SET NOCOUNT ON` istisnası, doğrulanmış yerel `#temp` staging ve indeksler, metadata-only CTE dönüşümü ve korunmuş ratchet'ler. |
...
| [`vm-evidence-status.md`](vm-evidence-status.md) | En son başarılı Windows VM evidence gate koşumu; 18 Ağustos 2026 tarihli 13/13 geçiş, artifact yolu, browser attach ayarı ve kanıt kapsamı. |
...
- Faz 3b SQL uyumluluğu **genel bir çok-ifade izni değildir**. Yalnızca tam `SET NOCOUNT ON;` öneki ve [`pk-sql-readonly-gate.md`](pk-sql-readonly-gate.md) içinde kanıtlanan yerel `#temp` analitik batch biçimi kabul edilir. Aynı batch içindeki yerel-temp indeksleri yalnızca daha önce oluşturulmuş `#temp` tabloları hedefleyebilir; kalıcı DDL/DML, `##global`, `EXEC`, `GO` ve metadata-generator DB-yazma ratchet'leri gevşetilmez.
