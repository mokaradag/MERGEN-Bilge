...
| `test-pk-v1-compatibility-contract.R` (+13 new) | **D21** v1 still returns the 20-row pre-filter frame while v2 returns the 10-row post-filter frame. `prompt_context`/`user_context`/`query_name`/`max_tokens` — the fields `server_send_message.R` and the Ortak Oturum bridge read — are intact on **both** engines. **D20** the `markdown tablo` instruction is still present in the v1 prompt (both modes) and gone from v2, which instead carries all five epistemic labels and forbids invented benchmarks. Facts/answer block/attachment exist only on v2; the v1 payload keeps its `ISTATISTIKSEL OZET` + `ORNEK SATIRLAR (JSON)` shape and contains no fact-reference slot. The six pure Phase-2 files are statically flag-free. Numeric provenance does not run at all when no facts were stashed (the v1 path), and when they are stashed the markers are stripped and the R block lands **before** the footer. The footer shows the attachment (and an explicit refusal) while never printing the pre-RLS count. |
...
> **SUPERSEDED — numeric provenance (§5.11).** The rows above describe the original
> *reverse-matching* validator: the model typed the number, placed an adjacent
> `[fact:...]` citation, and a scanner guessed which fact each prose number belonged to
> before checking value/unit/aggregation. Production measurement retired that design —
> `log`-mode telemetry over 40+ questions was dominated by `missing_fact_marker`, i.e.
> *correct* numbers that merely missed a hidden citation convention. The model now writes
> a `{{fact:<id>}}` **slot** and R substitutes the canonical display, so value/unit/
> tolerance/aggregation reverse-matching no longer exists and its files
> (`helpers_pk_numeric_provenance_binding.R`, `..._claims.R`) and tests
> (`test-pk-numeric-provenance-{false-positive,marker-group,token-scan}-*.R`) are
> deleted. The live contract is `test-pk-fact-reference-behavior.R`,
> `test-pk-fact-reference-scan-behavior.R` and the rewritten
> `test-pk-numeric-provenance-contract.R`; see master plan §5.11 for the current design.
...
| internal fact-reference syntax left in the displayed text | 5 |
