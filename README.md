...
MERGEN Bilge'nin on-prem Windows VM doğrulaması **16 Eylül 2026** tarihinde yeniden başarıyla tamamlandı: `tests/scripts/run_vm_evidence_gate.R` tam koşumu VM üzerinde uçtan uca geçti ve `Toplam: 13 passed, 0 failed, 0 skipped` sonucu verdi. `full_testthat`, `browser_ux_smoke`, `vm_preflight_real`, `db_encoding_preflight`, `maintainability_report`, `frontend_ratchet`, `seam_doctor`, source/UI asset manifest sözleşmeleri ve diğer gate adımlarının tümü `PASSED` oldu. Son kanıt artifact'ı: `artifacts/vm-evidence/20260916-143508/evidence.json`. Ayrıntılı son koşum kaydı: [`docs/vm-evidence-status.md`](docs/vm-evidence-status.md).

Bu kanıt kapısı `artifacts/vm-evidence/<timestamp>/evidence.json` altında secret-safe makine-okunur sonuç üretir. 16 Eylül 2026 koşumunda browser proof, zaten çalışan uygulamaya external-app modunda `http://127.0.0.1:28081` üzerinden bağlanarak ve `MERGEN_BROWSER_UX_BASE_URL=http://127.0.0.1:28081` ile `MERGEN_REQUIRE_BROWSER_UX_SMOKE=true` ayarlanarak alındı. Profil ayrıca zorlanmadı; gate etkin profili `vm` olarak çözdü. Bu sonuç, aynı VM koşumunda tam dosya-izole testthat suite'inin ve Windows çocuk `Rscript.exe` zincirinin, VM preflight'ın, DB encoding preflight'ın ve gerçek browser UX smoke'un geçtiğini kanıtlar; uzun süreli saha yükü, manuel kırılgan-akış QA'sı veya eski legacy DB satırlarının temizliği gibi kapsamları otomatik olarak kanıtlamaz. Operasyonel komutlar ve kanıt kapsamı için [`RUNBOOK.md`](RUNBOOK.md) izlenmelidir.
