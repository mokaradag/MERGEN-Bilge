...
AI validation yürütüm artifact’i olan `artifacts/ai-validation/<timestamp>/summary.json`, doktor artifact’inden ayrı değerlendirilmelidir. Bu dosya `profile_requested`, `profile_effective`, `validation_execution_status`, git branch/SHA/dirty çalışma ağacı özeti, `app_source_smoke_status`, `shiny_boot_smoke_status`, `browser_smoke_status`, `browser_required`, VM/SSO/DB ve SQL Server Türkçe kodlama preflight durumları, manuel fragile-flow durumu, `failed_step_labels`, `skipped_step_labels` ve `proof_boundary_notes` alanlarını taşır. Bu alanlar ham DSN, endpoint, token, key, secret, password, cookie veya auth header değeri yazmamalıdır.
