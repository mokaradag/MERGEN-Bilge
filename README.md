...
Son kaynak manifesti bakım güncellemesinde `global.R` içindeki mevcut `safe_source()` yükleme sırası değiştirilmeden korunmuş; ancak manifest doğrulama yardımcıları ve kritik sıra sözleşmeleri küçük, özel bir bootstrap dosyası olan `R/bootstrap_source_manifest.R` içine ayrılmıştır. `app.R` bu bootstrap katmanını `global.R` öncesinde yükler; `global.R` doğrudan source edilirse aynı bootstrap’i güvenli biçimde tamamlar. Böylece `global.R` çalışma zamanı kaynak manifesti olarak açık ve okunabilir kalırken eksik, yinelenen, parse edilemeyen veya kritik sırayı bozan kaynak dosyaları ilk uygulama kaynak zinciri çalışmadan önce Türkçe hata mesajlarıyla yakalanır. UTF-8 güvenli okuma, `safe_source()` yedek kodlama davranışı, Windows VM/SSO uyumluluğu ve `MERGEN_RUN_APP=false` / `MERGEN_DISABLE_FUTURES=true` test boot güvenliği korunur. Bu sözleşme `tests/testthat/test-source-manifest-contract.R`, `tests/testthat/test-e2e-boot-welcome-regression.R`, `tests/testthat/test-production-contracts.R` ve `tests/testthat/test-maintainability-ratchet.R` ile korunur.
