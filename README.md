...
Son ön yüz seçici bakımında Ana Söyleşi giriş alanının tarayıcı tarafı sözleşmesi de netleştirilmiştir. `www/js/input_handlers.js` artık eski `message_input` aramasına dayanmaz; gerçek UI sözleşmesi olan `#user_input`, `.chat-input` ve `textarea[name="user_input"]` seçicilerini tek yardımcı üzerinden kullanır. Böylece karakter sayacı, otomatik yükseklik ayarı, Enter ile gönderme, Shift+Enter ile yeni satır, Escape ile temizleme, gönder düğmesi ve stop-mode davranışı aynı DOM sözleşmesine bağlı kalır. Bu sınır `tests/testthat/test-frontend-selector-contract.R` ile korunur; test artık tüm `www/js/*.js` dosyalarında eski `message_input` izlerini tarar, `#_content_container` fallback’inin geri gelmesini engeller, CodeMirror observer cleanup sözleşmesini, Dosya Yönetimi getter kullanımını ve attach-state handler merkezileştirmesini de doğrular.
