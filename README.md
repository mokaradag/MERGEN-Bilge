...
Güncel VM kabul koşumunda `bash tools/ai_validate.sh quick`, `bash tools/ai_validate.sh full --boot-smoke` ve `MERGEN_REQUIRE_BROWSER_UX_SMOKE=true bash tools/ai_validate.sh full --boot-smoke` başarıyla tamamlanmış; gerçek tarayıcı smoke koşumu `UX_SMOKE_DONE:PASS` işaretiyle doğrulanmıştır.
