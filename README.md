...
Son davranış odaklı test güncellemesiyle statik sözleşme testlerinin yanında gerçek regresyon riski yüksek akışlar için ek birim/entegrasyon kontrolleri güçlendirilmiştir. Bu kapsamda hızlı eylem kimliği → araç ailesi → model seçimi eşlemesi, LLM yanıt ayrıştırma varyantları, request-id kapsamlı `send_message()` cleanup davranışı, File Manager kullanıcı klasörü/persist güvenliği, Bilge Yolaç doküman özetleme bağlam sıfırlaması ve SSO runtime bootstrap izolasyonu odak testlerle korunur. Bu testler gerçek DB, gerçek LLM endpoint’i, gerçek SSO sunucusu veya internet gerektirmez.
