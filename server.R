...
if (isTRUE(current_settings$enable_streaming) && !isTRUE(current_settings$enable_mcp_tools)) {
  ...
  # Ana ekrandaki 'Son Söyleşiler' veya global çağrılar için sohbet yükleme işleyicisi
  observeEvent(input$load_chat_id, {
    chat_id <- input$load_chat_id
    req(chat_id)
    
    # Devam eden bir yükleme varsa dur
    if (load_chat_in_progress()) return()
    load_chat_in_progress(TRUE)

    chat_to_load <- values$saved_chats[[chat_id]]
    
    # Mesajlar eksikse DB'den tamamını çek
    if (!is.null(chat_to_load) && (is.null(chat_to_load$messages) || length(chat_to_load$messages) == 0)) {
      detail <- tryCatch(
        load_chat_messages_from_db(chat_id),
        error = function(e) { NULL }
    ...
    if (!is.null(chat_to_load)) {
      ...
      # KRİTİK DÜZELTME: Mevcut sohbet ID'sini güncelle ki yeni mesajlar buraya kaydedilsin
      values$current_chat_id <- chat_id
      values$show_welcome <- FALSE
      ...
      if (length(values$messages) > 0) {
        for (i in seq_along(values$messages)) {
          msg <- values$messages[[i]]
          is_last <- (i == length(values$messages))
          
          selected_char_id <- isolate(settings_data$selected_character) %||% "mergen"
          chars_data <- get_characters_data()
          character_data <- if (!is.null(chars_data)) Find(function(x) x$id == selected_char_id, chars_data$styles) else NULL
          
          ui_to_insert <- render_message_bubble_ui(
            msg, settings_data, is_last_user_message = is_last,
            character_data = character_data,
            liked_ids = values$liked_messages, disliked_ids = values$disliked_messages
          )
          ...
          if(isTRUE(msg$has_code)) {
             wrapper_id <- paste0("message_wrapper_", msg$id)
             shinyjs::runjs(sprintf("setTimeout(() => { if(window.initializeCodeMirrorInElement) window.initializeCodeMirrorInElement('%s'); }, 200);", wrapper_id))
          }
        }
        # Grafikleri yeniden bağla
    ...
    shinyjs::delay(1000, { load_chat_in_progress(FALSE) })
  })
