...
Assert-Contains $runBatText 'AppendAllText\s*\(\s*\$env:MERGEN_CONSOLE_LOG[^)]*\$utf8NoBom' "run_mergen_prod.bat appends R console output to the daily mergen_YYYYMMDD.log file as explicit UTF-8"
Assert-Contains $runBatText 'New-Object\s+System\.Text\.UTF8Encoding\(\$false\)' "run_mergen_prod.bat uses BOM-less UTF-8 for appended R console output"
Assert-NotContains $runBatText 'Tee-Object\s+-FilePath\s+\$env:MERGEN_CONSOLE_LOG\s+-Append' "run_mergen_prod.bat avoids Windows PowerShell 5.1 Tee-Object append encoding semantics"
Assert-Contains $runBatText '&\s+\$env:RSCRIPT_EXE\s+\$env:MERGEN_RUN_SCRIPT' "run_mergen_prod.bat invokes Rscript through environment variables so paths with spaces stay quoted"
Assert-Contains $runBatText 'set\s+"MERGEN_CONSOLE_LOG=%MERGEN_LOG_DIR%\\mergen_%MERGEN_LOG_DATE%\.log"' "run_mergen_prod.bat names the daily console log as mergen_YYYYMMDD.log"
Assert-Contains $runBatText 'run_mergen_prod\.R' "run_mergen_prod.bat starts run_mergen_prod.R rather than directly calling shiny::runApp"
