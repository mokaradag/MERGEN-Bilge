...
pkg_default_connect_fn <- function(target = "primary", timeout_sec = NULL) {
  ...
  # `interruptible` YALNIZCA odbc >= 1.5.0 ile GECERLIDIR.
  #
  # Daha eski odbc surumlerinde `DBI::dbConnect()` taninmayan argumani SURUCU
  # ANAHTAR KELIMESI olarak iletir; surucu bunu reddedebilir ve gecerli bir DSN
  # ile bile baglanti kurulamaz. Surum yeterli degilse argument ATLANIR
  # (davranis eski surumde de calisir, yalnizca kesilebilirlik yoktur).
  baglanti_args <- list(odbc::odbc(), dsn = dsn)
  odbc_yeterli <- isTRUE(tryCatch(
    utils::packageVersion("odbc") >= "1.5.0",
    error = function(e) FALSE
  ))
  if (odbc_yeterli) baglanti_args$interruptible <- TRUE
...
pkg_default_sample_fn <- function(conn, sql, sample_rows = 500L, timeout_sec = NULL,
  ...
  while (toplam_satir < n) {
    ...
    parca <- if (is.finite(tavan_bayt) && is.finite(satir_bayt)) {
      ...
      # `min()` SAYISAL alanda uygulanir, `as.integer()` SONRA gelir.
      # Buyuk bir `MERGEN_PK_META_MAX_RESULT_MB` ve kucuk satir boyutunda
      # bolum `.Machine$integer.max` degerini asiyor, `as.integer()` `NA`
      # dondurup `DBI::dbFetch(n = NA)` cagrisini dusuruyordu: sonuc tavana
      # SIGDIGI halde sorgu ornekleme hatasi olarak raporlaniyordu.
      max(1L, as.integer(min(200, floor(kalan_bayt / satir_bayt))))
    } else {
      200L
    }
  }
