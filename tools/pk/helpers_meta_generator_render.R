...
pkgr_assert_writable_target <- function(path, forbidden = PKG_META_FORBIDDEN_TARGETS,
  ...
  # TUM YASAKLI HEDEFLER DENETLENIR.
  #
  # `.pkgr_normalize_rel()` `as.character(path)[1]` uygular, yani VEKTOR alan
  # bu cagri yalnizca ILK yasakli girdiyi goruyordu. Kalan girdiler yalnizca
  # asagidaki genel "beklenmeyen hedef" kapisina takiliyor ve operator, ozel
  # "operator alias dosyasina ASLA dokunmaz" uyarisi yerine genel bir mesaj
  # aliyordu. Liste artik ogeler halinde normalize edilir.
  yasak_temel <- basename(vapply(as.character(forbidden %||% character(0)),
                                 .pkgr_normalize_rel, character(1),
                                 USE.NAMES = FALSE))
...
pkgr_stage_local_meta_file <- function(text, path, forbidden = PKG_META_FORBIDDEN_TARGETS,
  ...
  #
  # SIRA ONEMLIDIR: `on.exit(..., add = TRUE)` varsayilan `after = TRUE` ile
  # KAYIT SIRASINDA calisir, yani yukarida kayitli `unlink()` once kosardi ve
  # Windows'ta acik tutamacli dosya SILINEMEZDI (kapatma amacini bosa cikarir).
  # `after = FALSE` kapatmayi silmeden ONCE calistirir.
  acik <- TRUE
  on.exit(if (acik) tryCatch(close(con), error = function(e) NULL),
