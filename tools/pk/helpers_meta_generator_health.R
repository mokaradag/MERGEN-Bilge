...
.pkgh_replace_with_backup <- function(staged, path, attempts = 3L) {
  ...
  # ':' YALNIZCA URETILEN SONEKTEN TEMIZLENIR, TAM YOLDAN DEGIL.
  #
  # Windows'ta `path` mutlak ve surucu harfiyle baslar (`C:/.../health.json`).
  # ':' tum dizeden silindiginde yedek yolu `C/.../health.json.bak-...` olur:
  # bu, ust dizini var olmayan GORECELI bir yoldur, `file.rename()` basarisiz
  # olur ve belgelenen yedekli takas kurtarmasi Windows'ta HIC calismazdi.
...
.pkgh_atomic_write_bytes <- function(bytes, path) {
  ...
  acik <- TRUE
  # SIRA ONEMLIDIR (`after = FALSE`): kapatma SILMEDEN ONCE kosmalidir.
  # `writeBin()` hata verdiginde (dolu disk / GC hatasi) varsayilan sirayla
  # once `unlink()` kosar, Windows acik tutamacli dosyayi SILMEZ ve
  # `.tmp-<pid>` artefakti kosu dizininde kalir.
  # `helpers_meta_generator_render.R` ayni tuzagi belgeleyip ayni sirayi
  # kullanir.
