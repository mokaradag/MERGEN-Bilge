...
`register_session_cleanup_on_end()` DEDUPLICATES the ledger: the same cleanup function registered twice (module rebind / Ctrl+Enter / `testServer`) was appended again while the early return skipped re-registration, so session shutdown ran that cleanup multiple times. New entries are filtered against the existing ledger with `identical()` before the ledger is updated.

`ai_expert_chunk_pipeline_baslat()` finalizes each chunk EXACTLY ONCE (`durum$sonuclandi`). When `queue_fn()` threw from `teslim_et()`, the `onFulfilled` error handler called `parca_sonuclandi()` a second time for the same chunk, `durum$aktif` was decremented twice and `sentez_surdur()` could exceed `policy$eszamanli_sinir`.
...
Milestone note: the latest Windows VM full evidence-gate rerun completed on 2026-07-26 with `Toplam: 13 passed, 0 failed, 0 skipped`; `full_testthat`, `browser_ux_smoke`, `vm_preflight_real`, and `db_encoding_preflight` all passed. Latest rerun artifact: `artifacts/vm-evidence/20260726-065220/evidence.json`. This is strong on-prem VM readiness evidence only for the steps reported as `passed`; it does not replace long-running production load observation or manual fragile-flow QA.
...
- The Çalışma Alanı welcome screen is a DECORATIVE retro 8-bit scene (`www/js/bilge_yolac_karsilama.js` + rewritten `www/css/bilge_yolac_welcome.css`): pixel persona sprites from the `claude_code_pixel_chars.js` data, CLI-style typed tips, and the `[OYNA]` launch line (`data-bs-ac`). It never captures input for gameplay, never plays audio, never sends Shiny inputs, stops when `cc-welcome-active` is removed, and honors `prefers-reduced-motion`.
- PERSONA SPRITES: `www/js/claude_code_pixel_chars.js` is the single 16x16 persona sprite source (five canonical ids, FIVE frames each — idle/breathe/think-a/think-b/wave — EXTENDED palette: 1-3 attire, 4-5 skin, 6 hair, 7 detail, 8 accessory). Colors align with the canonical accents; no mythological ids/art. The shared renderer `www/js/pixel_sprite_render.js` (`window.MergenPixelSprite.ciz`) applies a THEME-AWARE outline (dark in light theme, light in dark theme) and stays backward-compatible with old color/darkColor/lightColor-only data. BOTH the Bilge Yolaç thinking animation (`claude_code.js`) and the karsilama scene (`bilge_yolac_karsilama.js`) must render through this shared helper; load order is persona data → `pixel_sprite_render.js` → consumers (owned by the `bilge_yolac_calisma` frontend zone). Do not re-inline per-consumer pixel drawing or drop the extended palette.
- TOWERS + CAMPAIGN: alongside the 5 heroes (one-per-map), there are 3 buildable, upgradeable, sellable TOWERS (`gozetleme`/`veri_topu`/`kripto_isik`, 3 tiers) defined in `bilge_savunmasi_denge.js` (`kuleler`/`kuleAl`/`kuleIstatistik`/`kuleYatirim`). The tower SIM is a separate pure extension `bilge_savunmasi_sim_kuleler.js` (`BS.simKuleler.bagla(sim, ic)`) — no Math.random, no DOM; it attaches `kuleYerlestir`/`kuleYukselt`/`kuleSat`/`kuleTik`/`kuleVurusUygula`/`kuleSeriDurum`/`kuleSeriYukle` to `sim`. Tower kills increment the same `guncelDalga.olduruldu`, so the server per-wave cap still bounds score (no exploit); `kuleler` is added to checkpoint serialization (backward-compatible: old checkpoints without it still load) and `kullanilan_kuleler` to the summary (analytics only — server never validates it). The campaign is 6 maps (linear lock): `baglam_kapisi`(8) → `celiski_kavsagi`(10) → `bilgi_cekirdegi`(12) → `veri_labirenti`(12) → `sinyal_vadisi`(14) → `karar_zirvesi`(16), catalogued in BOTH `bs_harita_katalogu()` (R) and `BS.haritalar.liste` (JS) — the R `dalga_dusman_sayilari`/`dalga_patron_sayilari` MUST stay consistent with the JS `dalgaPlani` totals and the patron rule (`wave %% 4 == 0` or last wave), or the server score cap under/over-counts. Difficulty was rebalanced for the added tower power (`canCarpani` Normal 1.18 / Gelişmiş 1.7) with new elite threats (`veri_solucani`/`golge_istek`/`veri_hortumu`). Keep the map order derived from a single source (`siraliListe()` / `bs_harita_katalogu()`), not a hard-coded 3-map list.
- 2.5D RENDER SPLIT: the renderer is split into `bilge_savunmasi_varliklar.js` (local SVG asset loader for towers/decor/optional enemies with procedural fallback — never a broken image, no external URLs), `bilge_savunmasi_cizim_zemin.js` (one-time offscreen static ground painter), and `bilge_savunmasi_cizim.js` (dynamic layer). 2.5D depth is cheap-fake, not a 3D engine: entities are DEPTH-SORTED by grid Y (lower drawn on top → natural occlusion), tower/decor bodies rise from the cell base, plus ground shadows and a slight depth scale. Do not reintroduce Three.js/GPU dependency. Art lives in `www/assets/bilge_savunmasi/gorseller/{kuleler,dekor,dusmanlar}/` (repo ships original SVGs); missing art degrades to procedural.
...
Protected by: `tests/testthat/test-bilge-savunmasi-config-behavior.R`, `test-bilge-savunmasi-validation-behavior.R`, `test-bilge-savunmasi-db-behavior.R` (real SQLite), `test-bilge-savunmasi-lifecycle-contract.R`, `test-bilge-savunmasi-kule-muzik-contract.R` (towers/music/fullscreen/2.5D/map consistency), `test-persona-sprite-contract.R` (sprite palette + theme-aware renderer), plus the manifest/seam/zone/persona/frontend-ratchet contracts. VM-only proof: SQL Server Turkish at-rest values, portrait binaries, real multi-user SSO leaderboard flow, and the game-local music/art assets (operator-supplied `www/assets/bilge_savunmasi/muzik/*` tracks) which are not shipped in the cloud checkout.
...
- MCP SQL/chart DuckDB connections are sandboxed by `helpers_mcp_tools$open_sandboxed_duckdb()`
  ...
  tools, and never re-enable external access from SQL.
- Client-supplied paths are never trusted as filesystem targets: the analysis-file
  ...
  `www/` tree or the clicking user's own upload folder, the source-click `&&` hint
  ...
  (hidden entries count) — and the bug-report attachment flow
  (`R/module_destek_hata_bildir.R`) ignores the client `path` field entirely and
  only deletes/persists paths inside `destek_uploads`.
- `mergen_path_inside_root()` (`R/helpers_files_path.R`) FAILS CLOSED on an unknown
  ...
  of calling `get_user_upload_dir()` a second time.
...
- Settings: the Yapılandırma "Başlangıç Deneyimi" card (`startup_experience_lane`, in `R/module_settings_yapilandirma_advanced_ui.R`) is PENDING-until-save: the radio only updates `temp_startup_lane` in `R/module_settings_yapilandirma.R`; the saved configuration changes ONLY when "Ayarları Kaydet" runs `mergen_apply_saved_startup_lane()` (`R/module_startup_lane.R`), which sets `settings$startup_lane` and sends `applyStartupLane` (persistence rides the bulk `saveSettings` in `save_all_settings()`). A `settings$startup_lane` observer re-syncs the radio + pending value on boot resolution/restore/save so reload after an unsaved change shows the previously saved option. Do not reintroduce immediate `saveSettings`/`applyStartupLane` from the radio observer. The lane radios use a custom checkbox-consistent appearance (`.startup-lane-choices` rules in `www/css/settings_page.css`, dark + light themed, `:focus-visible` preserved). Kişiselleştirme hides the Deneyim Modu cards under `html.mergen-fast-lane` with an explanatory note; reset returns the lane to `ask_once` and clears the pending value.
- Deferred startup work (BOTH lanes): the persisted file inventory and the image gallery are no longer scanned/rendered on the boot critical path. `file_index_ready` is marked with `detail = list(deferred = TRUE)` (label "Dosyalar gerektiğinde yüklenecek") without touching the filesystem; the real scan runs when Dosya Yönetimi is first opened (`file_manager_module-page_opened` from the navigation observer, gated by `persisted_scan_pending`) or on manual/tool triggers. The image gallery activates lazily (`gallery_activated` in `R/module_image_gallery.R`); auth-ready refresh hooks are safe no-ops until the gallery page is first opened, and `gallery_content` must NOT be forced to render while hidden (`suspendWhenHidden` stays default TRUE — the old FALSE forced base64-encoding every gallery image into the boot websocket payload). `bootReadinessInit()` logs one `[STARTUP PERF] checkpoint=... elapsed_ms=...` line per checkpoint for cold-start phase attribution, plus one `[STARTUP PERF] first_flush elapsed_ms=...` line (first reactive flush) to separate pre-first-flush server-wiring cost from the file/identity checkpoints.
- Fast-lane saved-chat preview is DEFERRED PAST FIRST RENDER and dispatched with a LEAN worker contract (fixes the former ~9-14 s Windows VM post-auth stall + the stray `quantmod`/`zoo` S3 warning; latest 2026-07-11 VM logs show first server-start + first-user fast-lane deferred readiness around 13 s, the corresponding cold `welcome_client_ready` around 15.6 s, and warm `welcome_client_ready` around 3.1-3.6 s). The 6-chat "Son Konuşmalar" preview is NOT on the critical draw path: `R/server_observers_startup.R` marks `saved_chats_preview_ready` `deferred=TRUE` immediately ("Son konuşmalar arka planda yüklenecek") and starts the real DB dispatch only on `input$welcome_client_ready` (idempotent; `~10 s` `later::later` fallback via `options(mergen.startup_preview_fallback_secs)` if the client signal never arrives — the fallback never re-enters required readiness). Real hydration marks a SEPARATE `saved_chats_preview_hydrated` checkpoint (never reuse one key for both "deferred" and "loaded" — `bootReadinessInit()` logs only a key's first occurrence). Full-list deferral uses `saved_chats_full_deferred`; `saved_chats_full_loaded` fires ONLY on genuine completion. Dispatch goes through `mergen_startup_chat_preview_promise()` (`R/helpers_startup_chat_preview.R`) using `tracked_future_promise(..., dependency_mode = "explicit")`: the worker export set is narrow (DBI + `db_chat_preview_fetch_raw` + `db_chat_preview_query_sql` + scalar DSN/encoding), Turkish visible-text normalization stays in the MAIN process (`db_chat_preview_format_frame()`, byte-aligned with `load_chats_preview_from_db`), and NO `.GlobalEnv` scan / session-env serialization runs. Preserve all race/isolation guards: a stale/late preview must not overwrite a local mutation or a started full list, a closed-session callback must not mutate state, identity is re-resolved at hydration time, invalid ids never query, and a preview dispatch/reject failure only warns (never holds the overlay open). Do NOT regress the preview back to an eager `tracked_future_promise` on the critical path, and do NOT reuse a single checkpoint key for both deferral and completion. Protected by `tests/testthat/test-startup-observers-runtime-smoke.R`, `tests/testthat/test-startup-chat-preview-behavior.R`, and `tests/testthat/test-startup-lane-contract.R`.
- `tracked_future_promise()` (`R/helpers_worker_monitor.R`) is DEFAULT-`dependency_mode = "auto"` (unchanged auto-scan + recursive `.GlobalEnv` global expansion for every existing LLM/SSE/image/TTS/Ortak-Oturum caller). `dependency_mode = "explicit"` (opt-in) skips ALL auto-detection and re-binds `task_fn`'s environment to an isolated env of only the supplied `globals` (+ `packages`), so observer/session closures are never serialized to the worker — this is the required mode for boot-critical-path tasks. The auto-scan helpers are `worker_monitor_detect_task_deps` / `worker_monitor_expand_function_globals`; do not inline them back. Explicit-mode/auto-mode parity + no-leak-on-reject are protected by `tests/testthat/test-worker-monitor.R`.
...
- `tests/testthat/test-startup-observers-runtime-smoke.R`
- `tests/testthat/test-startup-chat-preview-behavior.R`
- `tests/testthat/test-worker-monitor.R`
- `tests/testthat/test-boot-readiness-behavior.R`
...
FAIL-CLOSED PERSISTENCE: `processAndSummarizeFile()` (`R/helpers_file_pipeline.R`) ABORTS when `effective_user_id <= 0L` — logging only was copying the file into a `user_0` bucket and indexing it with `user_id = 0` (IDOR). It also aborts when `global_register_file()` fails: the copy is removed through `fm_cleanup_orphan_upload()` and no session/table state is created, because swallowing that error reported a successful upload while the persistent index did not contain the file.
...
- File Manager marks `file_index_ready` for startup-style triggers (`initial`, `auth_ready`, `startup`) as a DEFERRED checkpoint (`detail = list(deferred = TRUE)`, no filesystem/index scan) when `boot_ready$mark` is available. The real persisted-folder scan is lazy: it runs on the first Dosya Yönetimi page open (`input$page_opened`, sent by the navigation observer, gated by `persisted_scan_pending`) or on non-startup triggers (`manual`, `page_open`, Bilge Yolaç refresh). Do not reintroduce a synchronous startup scan on the boot critical path — it blocked `welcome_client_ready` processing and dominated cold start (the old "Dosyalar hazırlanıyor" stall).
- Unsupported-extension files that physically exist in the user's persisted folder (e.g. agent-generated `.doc`) must stay VISIBLE in the Dosya Yönetimi table with the "Desteklenmeyen dosya türü" status and download/delete-only actions (`fm_build_unsupported_file_table_row()` in `R/helpers_file_manager_table.R`); they are never parsed/previewed/attachable and must not produce a login-time "cannot be saved" toast. Protected by `tests/testthat/test-file-manager-unsupported-file-behavior.R`.
...
### Bilge Yolaç Oturumlar (kalıcı oturum) sayfası contract

The Bilge Yolaç "Oturumlar" page persists agent sessions in `MB_ClaudeCode_Sessions` / `MB_ClaudeCode_Runs` (separate from `MB_Chats`/`MB_Messages`). Keep these boundaries intact:

- DB session helpers are split by responsibility and source order is protected: `R/helpers_db_claude_code_session_queries.R` (pure SQL/title/truncate builders), then `R/helpers_db_claude_code_sessions.R` (connection/tx orchestration: create/save-run/list/load/update/soft-delete), then `R/helpers_db_claude_code_session_lifecycle.R` (archive-restore + PERMANENT delete), then `R/helpers_database.R`. Do not move `cc_db_restore_session()` / `cc_db_hard_delete_session()` back into `R/helpers_db_claude_code_sessions.R`; that file is at its maintainability budget. The lifecycle file depends on the shared internals (`.cc_db_try`, `.cc_db_sessions_acquire/_release`, `normalize_db_params`) defined in the orchestration file loaded before it.
...
- A named-but-missing document is a BLOCKED run, never a silent substitution. When the prompt (or `explicit_files`) names a real document — a mention whose extension is in `get_claude_code_binary_doc_extensions()` — and nothing in the discovered set matches it, `cc_select_documents_for_request()` fails closed with `Açıkça istenen dokümanlar seçilen klasörde bulunamadı: ...`; `cc_prepare_run_workspace()` turns any `Açıkça istenen doküman` error into `blocked = TRUE` so the user sees the reason instead of a summary of a document they never asked for. Falling back to the automatic `order(candidate_key)` set in that situation is forbidden. The document-extension filter is what keeps this from over-blocking ordinary prose: dotted tokens such as `3.5` or `v1.2` are not document mentions and must never block a run.
...
- Truncation is NEVER silently treated as success. The pre-run snapshot (`prep$snapshot_truncated`) blocks the run in `cc_start_streaming_run()`, and the POST-run scan is propagated too: `diff_claude_code_workdir_snapshot()` attaches its `scan` metadata as an attribute, `cc_process_run_outputs()` returns `output_scan_truncated` / `output_scan_truncated_reason`, and `cc_report_output_scan_truncation()` finalizes the run as a failure instead of `Tamamlandı`. Do not drop the `scan` attribute or ignore the flag.
...
- `tests/testthat/test-claude-code-run-hardening-behavior.R`
- `tests/testthat/test-claude-code-runtime-workdir-contract.R`
- `tests/testthat/test-claude-code-workdir-scan-contract.R`
- `tests/testthat/test-claude-code-run-lifecycle-contract.R`
...
Windows test-parity rule for this boundary: these tests must RUN on the Windows VM, because Windows is the production platform for this pipeline. Do not reach for `skip_on_os("windows")` when a POSIX-only mechanism is merely inconvenient — use the platform-appropriate equivalent instead. Directory links use `Sys.junction()` on Windows (no administrator right required) and `file.symlink()` elsewhere; an unreadable directory is produced with `Sys.chmod("000")` on POSIX and with lexical `list.files`/`file.access`/`requireNamespace` mocks inside the sourced test environment on Windows (and under `root`, where chmod is ignored); a FILE symlink, which needs a privileged Windows session, falls back to lexically simulating the link report and the resolved target so the guarded decision itself is still exercised. The only legitimately platform-scoped tests here are contracts that describe non-Windows behavior by definition (for example `test-claude-code-runtime-path-pure-behavior.R`).
...
- `testthat::test_file("tests/testthat/test-claude-code-run-hardening-behavior.R")`
- `testthat::test_file("tests/testthat/test-claude-code-runtime-workdir-contract.R")`
- `testthat::test_file("tests/testthat/test-claude-code-workdir-scan-contract.R")`
- `testthat::test_file("tests/testthat/test-claude-code-run-lifecycle-contract.R")`
- `testthat::test_file("tests/testthat/test-maintainability-ratchet.R")`
...
safe_source("R/helpers_claude_code_dir_listing_async.R", encoding = "UTF-8")
...
* `R/helpers_claude_code_dir_listing_async.R`: the memoized worker-global bundle (`cc_dir_listing_worker_globals()`) and the future-plan check (`cc_dir_listing_async_available()`) used to run that enumeration off the Shiny event loop. Pure decision/data preparation only: no Shiny, reactive, DB or network dependency.
...
Do not move `list_directory_contents()` or its directory display-name/path-variant logic back into `R/helpers_claude_code.R`. Keeping this boundary prevents `R/helpers_claude_code.R` from returning to the 25+ function maintainability threshold. Do not merge the async helpers back into `R/helpers_claude_code_directory_listing.R` either; that file is at its 260-line / 19-function ratchet budget.
...
- BACKGROUND VALIDATION IS DISABLED — production crash protection (CRITICAL). `pool::release()` schedules a RECURRING background validation task (`scheduleTaskRecurring`, via `later`) for every returned connection whenever `validationInterval > 0`. That task periodically runs `checkObjectValid()`; on a dropped idle connection (08S01) it tries to `dbConnect()` a replacement, and a transient login timeout (08001 "Login timeout expired") throws OUT of the `later` callback UNCAUGHT, terminating the entire `runApp()` process (full app crash). App-level `tryCatch` cannot catch it because it is the pool's OWN background task outside any observer/handler. Therefore `.db_pool_build_default()` builds the ODBC pool with `validationInterval = 0` (background loop fully disabled); connections are validated ONLY at checkout via `validateQuery = "SELECT 1"`, where a failed reconnect surfaces SYNCHRONOUSLY into the caller's `tryCatch` (e.g. `save_message_to_db`) instead of crashing. `MERGEN_DB_POOL_VALIDATION_INTERVAL` is still read by `db_pool_config()` for backward-compat/status reporting but MUST NOT be wired back into `pool::dbPool(...)` (that reintroduces the crash). Locked by `tests/testthat/test-db-pool-behavior.R`.
...
Do not move this pure decision logic (the page-name `switch` maps, the frequency `switch` wrappers, or the idle context-parts assembly) back into `R/server_ai_expert_handlers.R`. After the hybrid speech rework, only the IDLE chat still calls the LLM (`tracked_future_promise(call_ai_expert_llm)`, task_type `ai_expert_idle_chat`). Protected by `tests/testthat/test-ai-expert-handlers-support-behavior.R` and `tests/testthat/test-ai-expert-handlers-support-contract.R`.

### AI Expert / TTS async dispatch and idle-talk priority contract

The speech-side async dispatches were a production event-loop hazard: `tracked_future_promise` auto mode serialized the observer/session closure chain to the PSOCK worker on every dispatch (6-8 s synchronous freezes per TTS/idle dispatch on the VM — delayed music start, blank Son Konuşmalar/Söyleşi Geçmişi, frozen quick actions). The following are now protected boundaries; do not regress them:

- `R/module_tts.R` `synthesize_speech()`, the idle-chat LLM dispatch in `R/server_ai_expert_handlers.R`, and the VoxCPM2 warmup dispatch in `R/server_speech_assets_runtime.R` use `dependency_mode = "explicit"` with complete enumerated globals (TTS: request values + `%||%`; idle chat: `call_ai_expert_llm` + `safe_trimws`/`safe_nzchar`/`normalize_utf8_text`/`sanitize_ai_expert_pronunciation` + captured values; warmup: `mergen_voxcpm2_synthesize_blocking` + `mergen_voxcpm2_redact` + `mergen_voxcpm2_ref_field_names` + `%||%` + values). When editing these worker bodies, keep the globals list in sync — explicit mode skips all auto-detection, so a missed helper only fails on the VM.
- Idle talk has the LOWEST priority in the whole app: `trigger_idle_chat()` skips the round (reschedules) when `future::nbrOfFreeWorkers()` reports no free worker, when a send/typing is active, or when speech state disallows it. Never let idle-chat work queue ahead of user-facing work, and never make its dispatch block the event loop.
- Reactive-context safety in speech callbacks is a crash boundary: `mergen_speech_chunk_dispatcher()$is_current()` reads `is_speaking` under `shiny::isolate()`, `can_speak()`/`start_speaking()` in `R/module_ai_expert.R` isolate all reactive reads, and the buffer-gate deadline `later::later` callback in `R/helpers_ai_expert_chunk_pipeline.R` is wrapped in `tryCatch`. Bare reactiveVal reads from promise/`later` callbacks previously threw `Operation not allowed without an active reactive context` at top level and killed the whole app. Do not remove these isolates/guards.
- The personal welcome prefix is prewarmed at the Başlayalım click (`explore_preheat_initial_greeting` -> `speech_runtime$prewarm_welcome`) while the character selection video plays; `VOXCPM2_PREFIX_DEADLINE_MS` defaults to 4000 ms and only bounds the residual wait at welcome dispatch time.
