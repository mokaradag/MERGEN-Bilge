...
- Bilge Yolaç büyük klasör / eşzamanlılık sınırları (opsiyonel, hepsi varsayılanlı): preflight taraması `CLAUDE_CODE_SCAN_MAX_FILES` (2000), `CLAUDE_CODE_SCAN_MAX_DIRS` (500), `CLAUDE_CODE_SCAN_MAX_DEPTH` (6), `CLAUDE_CODE_SCAN_MAX_TOTAL_MB` (512), `CLAUDE_CODE_SCAN_TIMEOUT_MS` (4000); "klasör sınır dışı" eşikleri `CLAUDE_CODE_PREFLIGHT_MAX_FILES` (1500), `CLAUDE_CODE_PREFLIGHT_MAX_DIRS` (400), `CLAUDE_CODE_PREFLIGHT_MAX_TOTAL_MB` (256); girdi aktarımı `CLAUDE_CODE_MAX_INPUT_FILES` (40), `CLAUDE_CODE_MAX_INPUT_FILE_MB` (25), `CLAUDE_CODE_MAX_INPUT_TOTAL_MB` (100), `CLAUDE_CODE_AUTO_SELECT_MAX_FILES` (25); çıktı `CLAUDE_CODE_OUTPUT_SCAN_MAX_FILES` (1000), `CLAUDE_CODE_OUTPUT_SCAN_MAX_DEPTH` (8), `CLAUDE_CODE_MAX_OUTPUT_FILE_MB` (100), `CLAUDE_CODE_MAX_OUTPUT_TOTAL_MB` (400); doküman `CLAUDE_CODE_MAX_DOCUMENTS` (10), `CLAUDE_CODE_MAX_DOCUMENT_MB` (25), `CLAUDE_CODE_MAX_DOCUMENTS_TOTAL_MB` (80); hazırlık/temizlik `CLAUDE_CODE_PREPARE_TIMEOUT_SEC` (180), `CLAUDE_CODE_FILE_SETTLE_TOTAL_MS` (1200), `CLAUDE_CODE_RUNTIME_RETENTION_SEC` (21600). Bilge Yolaç seçilen klasörün TAMAMINI asla kopyalamaz; pahalı tarama/kopyalama/çıkarım/çıktı işleri arka plan worker'ında yürütülür. Sahada tanılama için `[WORKDIR_PREFLIGHT]`, `[BOUNDED_SCAN]`, `[RUNTIME_PREPARE]`, `[INPUT_COPY]`, `[DOCUMENT_PREPARE]`, `[PROCESS_START]`, `[OUTPUT_DIFF]`, `[DOWNLOAD_STAGE]`, `[OUTPUT_SYNC]` log etiketlerine bakın. Ayrıntı: [`docs/technical-reference.md`](docs/technical-reference.md) "Bilge Yolaç bloklamayan çalıştırma hattı".
...
17 Ağustos 2026 tarihli son VM evidence gate koşumu başarılıdır: `Toplam: 13 passed, 0 failed, 0 skipped`. Son kanıt `artifacts/vm-evidence/20260817-065352/evidence.json` altında beklenir; genel düzen `artifacts/vm-evidence/<timestamp>/evidence.json` olarak kalır. Ayrıntılı son koşum kaydı için [`docs/vm-evidence-status.md`](docs/vm-evidence-status.md) izlenir.
...
2026-06-12 tarihinde Windows VM üzerinde tam evidence gate milestone'u kaydedildi; sonraki başarılı yeniden doğrulamalar 2026-06-15, 2026-07-21, 2026-07-26 ve 2026-07-28 tarihlerinde, en son başarılı yeniden doğrulama ise 2026-08-17 tarihinde alındı:
...
- En son başarılı koşum: `artifacts/vm-evidence/20260817-065352/evidence.json`.
